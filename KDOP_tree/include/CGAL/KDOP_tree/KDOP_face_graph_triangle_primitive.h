...
#ifndef KDOP_TREE_INCLUDE_CGAL_KDOP_TREE_KDOP_FACE_GRAPH_TRIANGLE_PRIMITIVE_H_
#define KDOP_TREE_INCLUDE_CGAL_KDOP_TREE_KDOP_FACE_GRAPH_TRIANGLE_PRIMITIVE_H_

#include <CGAL/disable_warnings.h>

#include <CGAL/KDOP_tree/KDOP_primitive.h>
 ...
 * while the KDOP tree holding the primitive is in use.
 ...
 *
 * \cgalModels `KDOPPrimitiveWithSharedData`
 *
 ...
 *\sa `KDOPPrimitive`
 *\sa `KDOP_primitive<Id,ObjectPropertyMap,PointPropertyMapPolyhedron,ExternalPropertyMaps,CacheDatum>`
...
class KDOP_face_graph_triangle_primitive
#ifndef DOXYGEN_RUNNING
  : public KDOP_primitive<typename boost::mpl::if_<OneFaceGraphPerTree,
...
public:
  ...
  */
  ...
  KDOP_face_graph_triangle_primitive(Iterator it, const FaceGraph& graph, VertexPointPMap_ vppm)
  ...
  /*!
    ...
    is available with `vppm` set to `get(vertex_point, graph)`.
  */
  KDOP_face_graph_triangle_primitive(face_descriptor fd, const FaceGraph& graph, VertexPointPMap_ vppm)
...
#ifndef DOXYGEN_RUNNING
  template <class Iterator>
  KDOP_face_graph_triangle_primitive(Iterator it, const FaceGraph& graph)
    : Base( Id_(make_id(*it, graph, OneFaceGraphPerTree())),
            Triangle_property_map(const_cast<FaceGraph*>(&graph)),
            Point_property_map(const_cast<FaceGraph*>(&graph)) )
  {}

  KDOP_face_graph_triangle_primitive(face_descriptor fd, const FaceGraph& graph)
    : Base( Id_(make_id(fd, graph, OneFaceGraphPerTree())),
            Triangle_property_map(const_cast<FaceGraph*>(&graph)),
            Point_property_map(const_cast<FaceGraph*>(&graph)) )
  {}
...
#endif /* KDOP_TREE_INCLUDE_CGAL_KDOP_TREE_KDOP_FACE_GRAPH_TRIANGLE_PRIMITIVE_H_ */
