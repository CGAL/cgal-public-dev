...
// w2: point-to-plane weight
// w3: rigid transformation weight
std::pair<Transform, PointSet> rigid_registration_with_triplets(const PointSet& source, const PointSet& target,
    double w1 = 0.1, double w2 = 0.1, double w3 = 1.0,
    int max_iterations = 100, int num_threads = 1) {
    Eigen::setNbThreads(num_threads);
    // resulting transformation is (R, t)
...
// w3: rigid transformation
// w4: as-rigid-as-possible transformation
std::pair<Transform, Mesh> nonrigid_registration_with_triplets(const Mesh& source, const PointSet& target,
    double w1 = 0.1, double w2 = 0.1, double w3 = 1.0, double w4 = 10.0,
    int max_iter = 100, int num_threads = 1) {
    Eigen::setNbThreads(num_threads);
    size_t N = source.number_of_vertices();
...
int main(int argc, char* argv[]) {
    ...
    Mesh mesh1 = readToscaMesh("../meshes/toscahires-asci/victoria0");
    Mesh mesh2 = readToscaMesh("../meshes/toscahires-asci/victoria1");
    //subsample(Mesh2PointSet(mesh1), Mesh2PointSet(mesh2), 10);
    CGAL::draw(merge_meshes(mesh1, mesh2));
    auto rigid_result = rigid_registration_with_triplets(Mesh2PointSet(mesh1), Mesh2PointSet(mesh2), 0.1, 0.1, 1.0, 20, 8);
    ...
    //CGAL::draw(merge_meshes(mesh1, mesh2));
    auto nonrigid_result = nonrigid_registration_with_triplets(mesh1, Mesh2PointSet(mesh2), 0.1, 0.1, 1.0, 20.0, 10, 8);
