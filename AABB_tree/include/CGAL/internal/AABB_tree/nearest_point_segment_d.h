...
namespace CGAL {
    ...
    namespace internal {
        ...
        */
        ...
        inline
            bool
            is_inside_segment_2(const typename K::Point_2& query,
            const typename K::Segment_2 & s,
            typename K::Point_2& closest_point_on_segment,
            const K&)
        {
            typedef typename K::FT FT;
        ...
        */
        ...
        template <class K>
               typename K::Point_2
                   nearest_point_2(const typename K::Point_2& query,
                   ...
                   const K& k)
               {
                   ...
                   typename K::Is_degenerate_2 is_degenerate =
                       k.is_degenerate_2_object();
                   ...
                   if(is_degenerate(segment)) {
                       const Point_2& p_on_seg = vertex(segment, 0);
                       if(compare_sq_distance(query,
                                              p_on_seg,
                                              bound_sq_dist) == CGAL::LARGER) {
                           return bound;
                   ...
                   // Project query on segment supporting line
                   const Point_2 proj = projection(segment.supporting_line(), query);

                   // If point is projected outside, return bound
                   if ( compare_sq_distance(query, proj, bound_sq_dist) == CGAL::LARGER )
                       return bound;

                   Point_2 closest_point_on_segment;
                   bool inside = is_inside_segment_2(proj,segment,closest_point_on_segment,k);
    ...
    template <class K>
        inline
            ...
            const Segment_2<K>& segment,
            const Point_2<K>& bound)
        {
            return internal::nearest_point_2(origin, segment, bound, K());
        }
