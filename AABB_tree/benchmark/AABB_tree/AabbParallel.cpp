...
{   
    ...
    std::vector<Ray> rays; /*Vector of rays for queries*/
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, numberOfRays), 
        [&](const tbb::blocked_range<size_t>& r){
                for(size_t i = r.begin(); i !=r.end(); ++i){
                    
                    Ray_intersection intersection = tree.first_intersection(rays[i]);
                }
        }
    );
