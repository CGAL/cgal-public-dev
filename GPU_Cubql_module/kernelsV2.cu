    ...
    int batchSize = 256000) 
{
    ...
    // Temporary device pointer for the current active slice of Mesh B
    ...
    for (int bStart = 0; bStart < NB; bStart += batchSize) {
        ...
        // Allocate batch intermediate storage
        ...
        // Get matching raw pointer iterators for the output device vectors to avoid pointer model collision
        ...
        if (totalGreen > 0) {
            ...
            CUBQL_CUDA_CALL(Memcpy(hGreenPairs.data() + oldSize, thrust::raw_pointer_cast(dev_green.data()), totalGreen * sizeof(IntersectionPair), cudaMemcpyDefault));
        }
        if (totalYellow > 0) {
            size_t oldSize = hYellowPairs.size();
            hYellowPairs.resize(oldSize + totalYellow);
            
            // Remap the ID back to global index space by adding bStart
        ...
        // Clean intermediate memory inside loop for next incoming batch chunk
