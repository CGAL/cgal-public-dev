...
namespace cuBQL {
  namespace gpuBuilder_v2_2 {
    ...
    struct CUBQL_ALIGN(16) TempNode {
      ...
      union {
        struct {
          ...
          uint32_t         count;
          uint32_t         level; // Tracks the absolute depth level of this node
        } openBranch;
        struct {
          uint32_t offset;
          int      dim;
                   ...
                   TempNode<T,D> *nodes)
    {
      ...
      nodes[0].openBranch.level = 0; // Root starts at level 0
    ...
    void selectSplits(BuildState    *buildState,
                      ...
                      uint32_t       targetLevel,             // Intercept tree descent at this level
...
#pragma unroll
          for (int side=0;side<2;side++) {
            ...
            child.count         = 0;
            child.level         = currentLevel + 1; // Increment child depth
    ...
    template<typename T, int D>
    __global__ void writePrimsAndLeafOffsets(
                                  TempNode<T,D> *nodes,
    ...
    void build_custom(BinaryBVH<T,D>    &bvh,
               ...
               int                numPrims,
               BuildConfig        buildConfig,
               uint32_t           targetLevel,              // The target depth level to monitor
