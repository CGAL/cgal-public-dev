...
uint64_t executeSingleTreeBatchedTraversalWithPredicates(Mesh& meshAcpu,
  ...
  // Instantiate CUDA Profiling Events
  cudaEvent_t evAllocStart, evAllocEnd;
  cudaEvent_t evAssemblyStart, evAssemblyEnd;
  cudaEvent_t evAabbStart, evAabbEnd;
  cudaEvent_t evEvalStart, evEvalEnd;
  cudaEvent_t evCompact1Start, evCompact1End;
  cudaEvent_t evDoubleStart, evDoubleEnd;
  cudaEvent_t evCompact2Start, evCompact2End;

  CUBQL_CUDA_CALL(EventCreate(&evAllocStart));
  CUBQL_CUDA_CALL(EventCreate(&evAllocEnd));
  ...
  // 2. Compute Exact Maximum Chunk Space Bounds & Preallocate Sandboxes (GPU Mode Profiling)
  CUBQL_CUDA_CALL(EventRecord(evAllocStart, stream));
  ...
  CUBQL_CUDA_CALL(EventRecord(evAllocEnd, stream));
  CUBQL_CUDA_CALL(EventSynchronize(evAllocEnd));

  float allocMs = 0.0f;
  CUBQL_CUDA_CALL(EventElapsedTime(&allocMs, evAllocStart, evAllocEnd));
  tracker.preallocateTimeMs = allocMs;

  // Dynamic Raw Host Buffer Management
  ...
  for (int i = 0; i < totalBatches; i += batchMultiplier) {
    ...
    CUBQL_CUDA_CALL(EventRecord(evAssemblyStart, stream));
    ...
    CUBQL_CUDA_CALL(EventRecord(evAssemblyEnd, stream));
    ...
    if (totalChunkPairs == 0) {
      ...
      CUBQL_CUDA_CALL(EventSynchronize(evAabbEnd));
      float emptyAabbMs = 0.0f;
      CUBQL_CUDA_CALL(EventElapsedTime(&emptyAabbMs, evAabbStart, evAabbEnd));
      tracker.executionPhaseMs += emptyAabbMs;
      continue;
    }
    ...
    // ----------------------------------------------------------------
    CUBQL_CUDA_CALL(EventRecord(evCompact1Start, stream));
    ...
    CUBQL_CUDA_CALL(EventRecord(evCompact1End, stream));
    ...
    if (totalYellow > 0) {
      if (exactPredicateComputeMode > 0) {
        ...
        CUBQL_CUDA_CALL(EventRecord(evCompact2Start, stream));
        ...
        CUBQL_CUDA_CALL(EventRecord(evCompact2End, stream));
      } else {
        // Track Yellow candidate D2H copy time when exact GPU mode is bypassed (mode == 0)
        CUBQL_CUDA_CALL(EventRecord(evCompact2Start, stream));
    ...
    // ----------------------------------------------------------------
    // METRICS RECORDING VIA CUDA EVENTS
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    // 2. PER-CHUNK GPU MEMORY DEALLOCATION
    // ----------------------------------------------------------------
    ...
    CUBQL_CUDA_CALL(StreamSynchronize(stream));
    tracker.cleanupTimeMs += (cuBQL::getCurrentTime() - tCleanStart) * 1000.0;
  }

  // Pass ownership directly out to caller
  outFinalExactPairs = h_finalBuffer;
  outFinalCount = hostCount;

  // Final Resource Cleanup Tracking
  ...
  CUBQL_CUDA_CALL(EventDestroy(evAllocEnd));
