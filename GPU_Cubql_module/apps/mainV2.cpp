...
// Replace your old typedef with this:
...
//   cuBQL::Triangle tri;
//   tri.a = {float(CGAL::to_double(pA.x())), float(CGAL::to_double(pA.y())), float(CGAL::to_double(pA.z()))};
//   tri.b = {float(CGAL::to_double(pB.x())), float(CGAL::to_double(pB.y())), float(CGAL::to_double(pB.z()))};
//   tri.c = {float(CGAL::to_double(pC.x())), float(CGAL::to_double(pC.y())), float(CGAL::to_double(pC.z()))};
//   return tri;
// }
...
int main(int ac, char** av) {
  if(ac < 7) {
    std::cout << "Usage: " << av[0]
              << " <A.off> <B.off> <gpu_fine_output.csv|none> <final_exact_output.csv|none> <pipelineMode "
                 "(0=Default/1/2)> <numThreads>\n";
...
// 2. DATA TRANSLATION
  ...
  tbb::concurrent_vector<IntersectionPair> finalExactPairs(confirmedIntersections.begin(),
                                                           confirmedIntersections.end());
  ...
  std::cout << "  |---> (CPU processed " << ambiguousPairs.size() << " elements instead of "
            << (confirmedIntersections.size() + ambiguousPairs.size()) << ")\n";
  std::cout << "----------------------------------------------------\n";
