...
namespace cuBQL {
  namespace gpuBuilder_v2 {
    ...
    struct CUBQL_ALIGN(16) TempNode {
      ...
      union {
        struct {
          ...
          uint32_t         count;
          uint32_t         alreadyMarked; // Repurposed from 'unused' to suppress downstream tracking
        } openBranch;
        struct {
          uint32_t offset;
          int      dim;
                   ...
                   TempNode<T,D> *nodes)
    {
      ...
      nodes[0].openBranch.alreadyMarked = 0;
                      ...
                      uint32_t      *tempMarkedCounts)
    {
      ...
      // Track flags to pass downstream safely within loop local scope
      bool currentMarkedFlag = false;
      bool parentWasMarked = false;
...
#pragma unroll
          for (int side=0;side<2;side++) {
            ...
            child.count         = 0;
            // Stop downstream tracking if this node or an ancestor already hit the target threshold
            child.alreadyMarked = (currentMarkedFlag || parentWasMarked) ? 1 : 0;
            nodeStates[childID] = OPEN_BRANCH;
          }
      }
#else
      // Path disabled by original macro configuration (#if 1)
#endif
    }
               ...
               GpuMemoryResource &memResource)
    {
      ...
      // Temporary counter tracking how many nodes hit thresholdX across runtime loops
      ...
      // Read back the total number of items recorded matching the constraint
      if(h_outMarkedCount) {
         CUBQL_CUDA_CALL(MemcpyAsync(h_outMarkedCount, d_globalMarkedCounter, 
