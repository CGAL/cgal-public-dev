...
// --------------------------------------------------------------------
// OPTIMIZED KERNEL DEFINITIONS (NO BINARY SEARCH)
// --------------------------------------------------------------------
...
// NEW LUT Generation Kernel: Maps every candidate combination space linearly to its batch index
...
// Evaluation Kernel: Replaced Binary Search loop with a direct O(1) LUT lookup
__global__ void evaluatePairsKernel_BSFree(
    int* d_pairValid,
    ...
    if (gIdx >= totalPairsToCheck) return;

    // Direct lookup execution instead of running sequential/binary branches
    ...
    bool overlaps = triA[primIdA].bounds().overlaps(triB[primIdB].bounds());
    d_pairValid[gIdx] = overlaps ? 1 : 0;
}

// Collection Kernel: Replaced Binary Search loop with a direct O(1) LUT lookup
...
uint64_t executeBatchedCrossIntersectionLoop(
    ...
    for (int i = 0; i < totalBatches; i += batchMultiplier) {
        ...
        // Allocation phase including the mapping lookup table (LUT)
        int* d_pairValid = nullptr;
        int* d_pairOutputPositions = nullptr;
        int* d_pairToBatchMap = nullptr; // New LUT mapping array
        ...
        // Generate the flat combination-to-batch indexing map
        ...
        // Compute completely free of binary search divergence
        evaluatePairsKernel_BSFree<<<evaluationGridSize, evaluationBlockSize>>>(
