...
int main(int ac, char** av) {
    if (ac < 5) {
        std::cout << "Usage: " << av[0] << " <A.off> <B.off> <gpu_fine_output.csv> <final_exact_output.csv>\n";
        return 1;
    }

    std::string gpuOutputPath   = av[3]; // Intermediate GPU output
    std::string exactOutputPath = av[4]; // Final verified output
    ...
    // --- NEW: INTERMEDIATE GPU OUTPUT WRITING ---
    ...
    std::cout << "[Step 6.5] Write GPU Fine CSV to Disk: " << cuBQL::prettyDouble(cuBQL::getCurrentTime() - t0) << "s\n";


    // --- TWO-LOOP CGAL FILTERING & CLEAN TIMING ---
    ...
    // 8. DISK I/O (Write the verified exact pairs)
    t0 = cuBQL::getCurrentTime();
    if (!exactPairs.empty()) {
    ...
    std::cout << "GPU AABB Overlaps:  " << totalAABBHits << " (Saved to " << gpuOutputPath << ")\n";
