...
__device__ inline __int128_t abs128_V2(__int128_t v) {
    return (v < 0) ? -v : v;
}
    ...
    const int3_128_V2& Ba, const int3_128_V2& Bb, const int3_128_V2& Bc) 
{
    ...
    __int128_t abs_nx = abs128_V2(nx), abs_ny = abs128_V2(ny), abs_nz = abs128_V2(nz);
...
{    
    ...
    int3_128_V2 qA0 = {0, 0, 0};
    int3_128_V2 qA1 = { llround(relA1.x * dynamic_scale), llround(relA1.y * dynamic_scale), llround(relA1.z * dynamic_scale) };
    int3_128_V2 qA2 = { llround(relA2.x * dynamic_scale), llround(relA2.y * dynamic_scale), llround(relA2.z * dynamic_scale) };

    int3_128_V2 qB0 = { llround(relB0.x * dynamic_scale), llround(relB0.y * dynamic_scale), llround(relB0.z * dynamic_scale) };
    int3_128_V2 qB1 = { llround(relB1.x * dynamic_scale), llround(relB1.y * dynamic_scale), llround(relB1.z * dynamic_scale) };
    int3_128_V2 qB2 = { llround(relB2.x * dynamic_scale), llround(relB2.y * dynamic_scale), llround(relB2.z * dynamic_scale) };
