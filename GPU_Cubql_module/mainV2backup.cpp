...
// Unified pair layout matching our new GPU performance schema
struct TaggedIntersectionPair {
    int idA;
    int idB;
    int status; // 1 = PAIR_GREEN (Definite), 2 = PAIR_YELLOW (Uncertain)
};
...
int main(int ac, char** av) {
    if (ac < 7) { 
        std::cout << "Usage: " << av[0] << " <A.off> <B.off> <gpu_fine_output.csv|none> <final_exact_output.csv|none> <pipelineMode (Unused but kept for signature)> <numThreads>\n";
        return 1;
    }
    ...
    // 3. BROAD-PHASE GPU INTERSECTION EXECUTION (Single Streaming Array Pass)
    std::vector<TaggedIntersectionPair> unifiedPairs;
    ...
    // 4. HOST SEGREGATION PASS (Filter streamed data into Green vs. Yellow structures)
    double t_split_start = cuBQL::getCurrentTime();
    std::vector<IntersectionPair> confirmedIntersections; // Green List
    std::vector<IntersectionPair> ambiguousPairs;         // Yellow List
    
    // Reserve optimization to eliminate vector reallocations
    confirmedIntersections.reserve(unifiedPairs.size() / 2);
    ambiguousPairs.reserve(unifiedPairs.size() / 8);

    for (const auto& pair : unifiedPairs) {
        if (pair.status == 1) {
            confirmedIntersections.push_back({pair.idA, pair.idB});
        } else if (pair.status == 2) {
            ambiguousPairs.push_back({pair.idA, pair.idB});
        }
    }
    double time_host_split = cuBQL::getCurrentTime() - t_split_start;

    // 5. WRITE INTERMEDIATE GPU BVH OUTPUT
    t0 = cuBQL::getCurrentTime();
    if (writeGpuCSV) {
        std::ofstream gpuFile(gpuOutputPath);
        gpuFile << "ID_A,ID_B,Type\n";
        for (const auto& p : unifiedPairs) {
            gpuFile << p.idA << "," << p.idB << "," << (p.status == 1 ? "Green" : "Yellow") << "\n";
        }
    ...
    // 6. HYBRID NARROW-PHASE FILTERING LOOP (Processes Only Yellow List)
    double t_inter_start = cuBQL::getCurrentTime();
    
    auto& coords1 = m1.points();
    auto& coords2 = m2.points();

    // Pre-populate with our definite GPU checks (Green values)
    ...
    std::cout << "  |---> Host Processing Segregation:    " << cuBQL::prettyDouble(time_host_split) << "s\n";
    ...
    std::cout << "  |---> (CPU processed " << ambiguousPairs.size() << " elements instead of " << unifiedPairs.size() << ")\n";
    std::cout << "----------------------------------------------------\n";

    // 7. FINAL EXACT OUTPUT GENERATION (Conditional)
    t0 = cuBQL::getCurrentTime();
    ...
    std::cout << "  |-- GPU Definite/Uncertain Track: " << unifiedPairs.size() << "\n";
