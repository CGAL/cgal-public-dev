...
public:
    ...
    ::CGAL::Comparison_result overlap_or_order(
        ...
        if (isolator1.right_bound(i1) < isolator2.left_bound(i2)) {
            return CGAL::SMALLER;
        }
        if (isolator2.right_bound(i2) < isolator1.left_bound(i1)) {
            return CGAL::LARGER;
        }
        return CGAL::EQUAL;
    }
    ...
    bool is_included(
        ...
        if (strict) {
            return 
                (isolator1.left_bound(i1) > isolator2.left_bound(i2)) &&
                (isolator1.right_bound(i1) < isolator2.right_bound(i2));
        } 
        // else 
        return 
            (isolator1.left_bound(i1) >= isolator2.left_bound(i2)) &&
    ...
    OutputIterator refined_overlaps(
        ...
        while (true) {
            ...
            case CGAL::EQUAL:
                ...
                if (isolator1.right_bound(i1) <=
                    isolator2.right_bound(i2)) {
                    i1++;
                } else if (isolator1.right_bound(i1) >=
                           isolator2.right_bound(i2)) {
                    i2++;
                }
                break;
