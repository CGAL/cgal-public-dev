 ...
 */
template <typename GeomTraits, typename T_Dcel = Arr_default_dcel<GeomTraits> >
...
private:
  ...
  struct Vertex_key_comparer {
    ...
    {
      return (m_traits->compare_y_on_boundary_2_object()(p1, p2) == SMALLER);
    }
  };
...
public:
  ...
  // The following member functions are used only in Arr_transform_on_sphere.h
  // The interface redesigned. (The implementation of Arr_transform_on_sphere
...
private:
  ...
  template <typename ExGeomTraits_,
            typename ArrangementA_, typename ArrangementB_>
