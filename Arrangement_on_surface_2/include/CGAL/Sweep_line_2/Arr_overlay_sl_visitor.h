  ...
  { 
    if (rev_iter != event->right_curves_rend())
    {
      if((*rev_iter)->color() == Traits_2::BLUE)
        (*rev_iter)->set_red_face(m_overlay_helper.red_face());
      else if((*rev_iter)->color() == Traits_2::RED)
        (*rev_iter)->set_blue_face(m_overlay_helper.blue_face());
...
//
template <typename OvlHlpr, typename OvlTr>
void Arr_overlay_sl_visitor<OvlHlpr, OvlTr>::update_event(Event* e, Subcurve* sc)
{
...
void Arr_overlay_sl_visitor<OvlHlpr, OvlTr>::after_sweep()
{
  ...
  m_overlay_traits->create_face(m_overlay_helper.red_face(),
                                m_overlay_helper.blue_face(),
                   ...
                   bool& new_face_created)
{
  ...
  if (new_face_created)
  {
    ...
    Face_handle     new_face =
      (! flipped_he) ? new_he->face() : new_he->twin()->face();
    Halfedge_handle he;
...
insert_isolated_vertex(const Point_2& pt, Status_line_iterator iter)
{
  ...
  else if (is_red_empty)
  {
    ...
    if (iter == this->status_line_end())
    {
      ...
      red_face = m_overlay_helper.red_face();
    }
    else
    {
  ...
  else
  {
    ...
    if (iter == this->status_line_end())
    {
      ...
      blue_face = m_overlay_helper.blue_face();
    }
    else
    {
