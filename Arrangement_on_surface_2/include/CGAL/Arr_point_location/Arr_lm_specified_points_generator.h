...
public: 
  ...
  void build_landmark_set ()
  {
    ...
    typename Traits_adaptor_2::Compare_xy_2 compare_xy_2 = this->m_traits->compare_xy_2_object();
    for (pt_it = m_points.begin(); pt_it != m_points.end(); ++pt_it)
    {
        for (pairs_it = lm_pairs.begin();
                   pairs_it != lm_pairs.end() && compare_xy_2((*pairs_it).first,(*pt_it)) != EQUAL;
                   ++pairs_it) {};

        if (compare_xy_2((*pairs_it).first,(*pt_it)) == EQUAL)
        {
