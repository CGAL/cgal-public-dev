...
// TODO which topol traits functions can be removed?
// .. e.g. are_equal, compare_y_at_x

// make generic?
// place_boundary_vertex, locate_around_boundary_vertex
...
remove_isolated_vertex(Vertex_handle v)
{
  ...
  // Notify the obsesrvers that we are abount to remove a vertex.
           ...
           const X_monotone_curve_2& cv)
{
  ...
  DCcb  *ic1 = (he3->is_on_inner_ccb()) ? he3->inner_ccb() : NULL;
  DCcb  *oc1 = (ic1 == NULL) ? he3->outer_ccb() : NULL;

  DCcb  *ic2 = (he4->is_on_inner_ccb()) ? he4->inner_ccb() : NULL;
  DCcb  *oc2 = (ic2 == NULL) ? he4->outer_ccb() : NULL;
  ...
  // merge does not change the pointing to the leftmost vertex ...
  if (oc1 != NULL && oc1->halfedge() == he3)
    oc1->set_halfedge (he1); // reset leftmost pointer
  else if (ic1 != NULL && ic1->halfedge() == he3)
    ic1->set_halfedge (he1); // reset leftmost pointer

  // .. infact he4 should never have been such a pointer
  if (oc2 != NULL && oc2->halfedge() == he4)
    oc2->set_halfedge (he2); // reset leftmost pointer
  else if (ic2 != NULL && ic2->halfedge() == he4)
    ic2->set_halfedge (he2); // reset leftmost pointer
...
remove_edge(Halfedge_handle e, bool remove_source, bool remove_target)
{
  ...
  DHalfedge   *he1 = _halfedge (e);
  DHalfedge   *he2 = he1->opposite();
  DCcb  *ic1 = (he1->is_on_inner_ccb()) ? he1->inner_ccb() : NULL;
  DCcb  *oc1 = (ic1 == NULL) ? he1->outer_ccb() : NULL;
  DFace       *f1 = (ic1 != NULL) ? ic1->face() : oc1->face();
  DCcb  *ic2 = (he2->is_on_inner_ccb()) ? he2->inner_ccb() : NULL;
  DCcb  *oc2 = (ic2 == NULL) ? he2->outer_ccb() : NULL;
...
#if 0 // TODO replace with new implementation
                    ...
                    Comparison_result cmp)
{
  ...
  DCcb  *ic = (prev->is_on_inner_ccb()) ? prev->inner_ccb() : NULL;
  DCcb  *oc = (ic == NULL) ? prev->outer_ccb() : NULL;
                    ...
                    bool& new_face)
{
  ...
  if (curr != curr->next()) {
    ...
    while (curr != prev1) {
      if (!curr->has_null_curve()) {
        std::cout << "curr1: " << curr->curve() << std::endl;
      } else {
        std::cout << "curr1: fictitious" << std::endl;
      }
      std::cout << "dir1: " 
  ...
  if (curr != curr->next()) {
    ...
    while (curr != prev2) {
      if (!curr->has_null_curve()) {
        std::cout << "curr2: " << curr->curve() << std::endl;
      } else {
        std::cout << "curr2: fictitious" << std::endl;
      }
      std::cout << "dir2: " 
                << (curr->direction() == CGAL::ARR_LEFT_TO_RIGHT ? "L2R" : "R2L") 
                << std::endl;
  ...
  DHalfedge           *he1 = _dcel().new_edge();
  DHalfedge           *he2 = he1->opposite();
  he1->set_curve (_new_curve(cv)); // and thus for he2
  ...
  DCcb  *ic1 = (prev1->is_on_inner_ccb()) ? prev1->inner_ccb() : NULL;
  DCcb  *oc1 = (ic1 == NULL) ? prev1->outer_ccb() : NULL;
  DFace       *f1 = (ic1 != NULL) ? ic1->face() : oc1->face();
  DCcb  *ic2 = (prev2->is_on_inner_ccb()) ? prev2->inner_ccb() : NULL;
  DCcb  *oc2 = (ic2 == NULL) ? prev2->outer_ccb() : NULL;
  DFace       *f2 = (ic2 != NULL) ? ic2->face() : oc2->face();
  ...
  // 2.2) Compute some information related to closing an inner ccb
  ...
  if (ic1 == ic2 && ic1 != NULL)
  {
    // In this case, where the two halfedges are reachable from one
    // another and are located on the same hole, a new face will be created
    // and form a hole inside their current incident face. Thus, we
    ...
    // first, we check the perimetricy
    // NOTE: ic1 and ic2 are actually already invalided, but they are still 
    //       represented by prev1 and prev2 - but as we throw in he1 and he2,
    //       we are actually have two different loops at hand
    if (ic1 != NULL) {
      _determine_ccb_perimetricy(ic1);
    }
    if (ic2 != NULL) {
      _determine_ccb_perimetricy(ic2);
    }
  
    // We use the direction of ccbs to determine whether we have
    // to create a new face by splitting f, and if so - whether the new face is
    // contained in the existing face or adjacent to it.

    // if one loop is non-perimetric, the default values are the right ones, but ...
    if (ic1->is_perimetric() && ic2->is_perimetric()) {
      // ... if both are perimetric
      
      // there is a special case for two identified sides (torus-like surface)
        // TODO only if two identifications (or is this already implied by #outer_ccbs == 0?)
      if (false && // TODO EBEF TOP_FACE m_topol_traits.top_face() == f && 
          f1->number_of_outer_ccbs() == 0) {
        // the special case is when the first perimetric loop of curves is
        // found, this just creates two outer ccbs for the face
        // that contained a "perimetric" hole before, not new face is created
        split_new_face = false;
        is_split_face_contained = false;
      } else {
    ...
    // but it is still open which halfedge forms the outer ccbs of a possibly new face

    // TODO do contraction/identification have special cases as previously in is_inside_new_face?
    if (ic1->is_perimetric()) {
  ...
  // 3) Check the various cases of insertion (in the design document: the
  // various sub-cases of "insert_at_vertices" - "case 3 in the technical document);
  // i.e. split face if needed and set ccbs properly

  // first, save some leftmost pointers
  DHalfedge *ic1_he = (ic1 != NULL ? ic1->halfedge() : NULL);
  DHalfedge *ic1_he_rev = (ic1 != NULL ? ic1->halfedge()->next()->opposite() : NULL);
  ...
  if ((ic1 != NULL || ic2 != NULL) && ic1 != ic2)
  {
    ...
    if (ic1 != NULL && ic2 != NULL)
    {
      ...
      _notify_before_merge_inner_ccb (fh1,
      ...
      _notify_after_merge_inner_ccb (fh1, (Halfedge_handle(he1))->ccb());
    ...
    std::cout << "INSERT 3.2: Extend outer ccb by inner ccb" << std::endl;
      ...
      _notify_after_remove_inner_ccb (fh1);
  ...
  else if (! split_new_face)
  {
    ...
    // we form two outer CCBs of the same face (e.g., on torus)
    Face_handle       fh1 (f1);

    // Notify the obserers we are about to remove an inner CCB from f.
    _notify_before_remove_inner_ccb (fh1, (Halfedge_handle(he1))->ccb());
    ...
    // Notify the obserers we are about to add an outer CCB to f.
    _notify_before_add_outer_ccb (fh1, Halfedge_handle (he2));

    // Create a new outer CCB that for the face f, and make 
    // he2 (implied choice) the representative halfedge of this component.
    DCcb  *new_oc2 = _dcel().new_outer_ccb();
  ...
  else if (ic1 == ic2 && oc1 == oc2)
  {
    ...
    _notify_before_split_face (fh1, Halfedge_handle (he1));
    ...
    if (ic1 != NULL)
    {
      ...
      else
      {
        ...
        DCcb_iter  oc_it = f1->outer_ccbs_begin();
        DCcb_iter  oc_to_move;

        while (oc_it != f1->outer_ccbs_end())
        {
          // Use the direction of the perimetric outer ccbs to determine 
          // whether the representative of the current outer CCB should 
          // belong to the same face as he2 (which is GGG and 
          ...
          if (*oc_it != he1 &&
            ...
            _move_outer_ccb (f1, new_f, *oc_to_move);
          }
          else
          {
            ++oc_it;
    ...
    else
    {
      ...
      if (new_f->is_unbounded())
        f1->set_unbounded (m_topol_traits.is_unbounded (f1));
    }

    // Notify the observers that we have split the face.
    _notify_after_split_face (fh1, Face_handle (new_f), is_hole);
  }
  else
  {
    ...
    _notify_before_merge_outer_ccb (fh1,
                                    (Halfedge_handle(prev1))->ccb(),
                                    (Halfedge_handle(prev2))->ccb(),
                                    Halfedge_handle (he1));
    ...
    _notify_after_merge_outer_ccb (fh1,
  ...
  // 4) Finally return the halfedge directed from v1 to v2.
  return (he2);
}
...
_relocate_inner_ccbs_in_new_face (DHalfedge *new_he)
{
  ...
  DCcb_iter    ic_it = old_face->inner_ccbs_begin();
  DCcb_iter    ic_to_move;
...
_remove_edge (DHalfedge *e, bool remove_source, bool remove_target)
{
  ...
  if (f1 == f2)
  {
    ...
    else if (he1->next() == he2 || he2->next() == he1)
    {
      ...
      if (oc1 != NULL && (oc1->halfedge() == he1 ||
                          oc1->halfedge() == he2))
      {
        // either prev1 is pointing to leftmost now
        _update_ccb_with(oc1, prev1);
        // or he2->next(), which is prev1->next()
        _update_ccb_with(oc1, prev1->next());
      }
      else if (ic1 != NULL && (ic1->halfedge() == he1 ||
                               ic1->halfedge() == he2))
      {
        // either prev1 is pointing to leftmost now
        // TODO do no compare with he1/he2 in update!
        _update_ccb_with(ic1, prev1);
        // or he2->next(), which is prev1->next()
        // TODO do no compare with he1/he2 in update!
        _update_ccb_with(ic1, prev1->next());
      }
    ...
    // not remove from face and not an antenna:
    ...
    // Note from here we can check perimetricy of the new (implicit) CCBs 
    ...
    // The only task left now is to update the ccbs:
    if (ic1 != NULL && ic1 == ic2)
    {
      ...
      // inner CCB.
      // TODO update leftmost pointer of ic1
      ic1->set_halfedge (prev1);

      // Create a new component that represents the new hole we split.
      DCcb     *new_ic2 = _dcel().new_inner_ccb();

      f1->add_inner_ccb (new_ic2, prev2); // sets prev2 as initial leftmost pointer
      new_ic2->set_face (f1);
    ...
    else if (oc1 != oc2)
    {
      ...
      // but lie on different (perimetic) outer CCBs of this face, removing this pair of
      // halfedge causes the two components to merge and to become an
      // inner CCB in the face. 
      // We first remove the outer CCB oc1 from f, and inform the observers
      // on doing so.
      Face_handle               fh1 (f1);

      _notify_before_remove_outer_ccb (fh1, (Halfedge_handle(he1))->ccb());

      f1->erase_outer_ccb (oc1);
      _dcel().delete_outer_ccb (oc1);

      _notify_after_remove_outer_ccb (fh1);

      // We now remove the outer CCBs oc2 from f, and inform the observers
      // on doing so.
      _notify_before_remove_outer_ccb (fh1, (Halfedge_handle(he2))->ccb());
      ...
      // In this case we now have to create a new inner CCB inside the face:
      ...
      // Initiate a new inner CCB inside the given face.
      DCcb   *new_ic1 = _dcel().new_inner_ccb();

      f1->add_inner_ccb (new_ic1, prev1); // sets prev1 as initial leftmost pointer
      new_ic1->set_face (f1);

      // Set the innser CCB of the halfedges along the component boundary.
      DHalfedge   *curr = prev1;
      
      // TODO set leftmost pointer of new_ic1
      do
      {
        curr->set_inner_ccb (new_ic1);
        curr = curr->next();
      } while (curr != prev1);
    ...
    else
    {
      ...
      // As the old outer CCB of f1 may be represented by any of the
...
#if 0
      // TODO unify CCB classes and combine/simplify the following two cases:
      // TODO new ccb (NOT INNER/OUTER)
      ...
      {
        // If both are perimetric, we have to split 
        // the outer CCB into two outer components
        // (case 3.2.2), such that the number of outer 
        // CCBs of the face is incremented.
        ...
        // If one is not perimetric, we have to create 
        // a new hole in the interior of the incident face
        // (case 3.2.1):
        //
        //    +-----------------------------+
        //    | prev1                       |
      ...
      if (m_topol_traits.hole_creation_after_edge_removal (he1))
      {
        ...
        f1->add_inner_ccb (new_ic2, he1->next()); // sets he1->next as initial leftmost pointer
        new_ic2->set_face (f1);
  ...
  if (ic1 == NULL && ic2 == NULL)
  {
    ...
    DCcb_iter    ic_it = f2->inner_ccbs_begin();
    DCcb_iter    ic_to_move;
    ...
    if (add_inner_ccb)
    {
      ...
      // Initiate a new inner CCB inside the given face.
      DCcb   *new_ic = _dcel().new_inner_ccb();

      f1->add_inner_ccb (new_ic, prev1); // sets prev1 as initial leftmost pointer
      new_ic->set_face (f1);
  ...
  while (ic_it != f2->inner_ccbs_end())
  {
    // We increment the holes iterator before moving the hole, because
    // this operation invalidates the iterator.
    ic_to_move  = ic_it;
    ++ic_it;
...
template<class GeomTraits, class TopTraits>
bool Arrangement_on_surface_2<GeomTraits, TopTraits>::
_is_outer_ccb_valid (const DCcb *oc, const DHalfedge *first) const
{
...
template<class GeomTraits, class TopTraits>
bool Arrangement_on_surface_2<GeomTraits, TopTraits>::
_is_inner_ccb_valid (const DCcb *ic, const DHalfedge *first) const
{
  // Make sure that all halfedges along the CCB refer to the same component.
  const DHalfedge   *curr = first;
  bool               found_rep = false;
