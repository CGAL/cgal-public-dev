...
request, which can be a rather costly operation. A constructor for points using a rational
function and its $x$-coordinate is provided by the Traits class. A constructor
for points taking as parameters two rational coordinates is also provided, 
internally it constructs a supporting rational function its $x$-coordinate.
...
\ccMethod{Construct_point_2 construct_point_2_object() const;}
  {Returns an instance of \ccc{Construct_point_2}.}\ccGlue
...
\subsection*{Class   Arr\_rational\_function\_traits\_2$<$AlgebraicKernel\_d\_1$>$::Construct\_point\_2}
\begin{ccClass}{Arr_rational_function_traits_2<AlgebraicKernel_d_1>::Construct_point_2}

Functor to construct a \ccc{Point_2}. To enable caching the class
is not default constructible and must be obtained via the function
\ccc{construct_point_2_object()}, which is a member of the traits. 
...
\ccTypes \ccThree{}{xxxxxxxxxxxxxxxxxxxxxx}{x}
\ccTypedef{typedef Arr_rational_function_traits_2<Algebraic_kernel_d_1> Traits;}{}\ccGlue
\ccTypedef{typedef CGAL::Arr_rational_arc::Cache<Algebraic_kernel_d_1>  Cache;}{}\ccGlue
...
% Operators that accept polynomials:
\ccMethod{Point_2 operator()(const Rational_function& rational_function,
                       const Algebraic_real_1& x_coordinate);}
  {Constructs an algebraic point supported by a rational function and its $x$-coordinate.}\ccGlue
\ccMethod{Point_2 operator()(const Rational& x, const Rational& y);}
  {Constructs an algebraic point from two Rational coordinates, internally it constructs a supporting rational function
  and its $x$-coordinate.}\ccGlue
\ccMethod{Point_2 operator()(const Algebraic_real_1& x, const Rational& y);}
  {Constructs an algebraic point from an algebraic $x$-coordinate and a rational $y$-coordinate, internally it constructs a supporting rational function
  and its $x$-coordinate.}\ccGlue
\ccMethod{Point_2 operator()(int x, int y);}
  {Constructs an algebraic point from two integer coordinates, internally it constructs a supporting rational function
  and its $x$-coordinate.}\ccGlue
\end{ccClass}
