...
namespace implicit {
  ...
  inline bool intersect(const VBBox<T> &bbox, const Ray<T> &ray, std::size_t i) {
    ...
    double xmin = (bbox.bounds()[ray.sign.x].get().x[i] - ray.origin.x) * ray.inv_direction.x;
    double xmax = (bbox.bounds()[1 - ray.sign.x].get().x[i] - ray.origin.x) * ray.inv_direction.x;
    double ymin = (bbox.bounds()[ray.sign.y].get().y[i] - ray.origin.y) * ray.inv_direction.y;
    double ymax = (bbox.bounds()[1 - ray.sign.y].get().y[i] - ray.origin.y) * ray.inv_direction.y;
    double zmin = (bbox.bounds()[ray.sign.z].get().z[i] - ray.origin.z) * ray.inv_direction.z;
    double zmax = (bbox.bounds()[1 - ray.sign.z].get().z[i] - ray.origin.z) * ray.inv_direction.z;
