...
namespace xsimd {
  ...
  inline bool intersect(const BBox<T> &bbox, const Ray<T> &ray) {
    ...
    xsimd::batch<double, 4> min(
            bbox.bounds()[ray.sign().x()].x(),
            bbox.bounds()[ray.sign().y()].y(),
            bbox.bounds()[ray.sign().z()].z(),
            std::numeric_limits<double>::max()
    );

    // Determine intermediate value for maximum bounds
    xsimd::batch<double, 4> max(
            bbox.bounds()[1 - ray.sign().x()].x(),
            bbox.bounds()[1 - ray.sign().y()].y(),
            bbox.bounds()[1 - ray.sign().z()].z(),
