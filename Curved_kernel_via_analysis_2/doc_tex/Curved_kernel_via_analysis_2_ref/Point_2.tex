...
\begin{ccRefClass}[CGAL::internal::]{Point_2}  %% add template arg's if necessary
\ccRefLabel{Point_2}
...
class \ccc{Rep_} = \ccc{CGAL::internal::Point_2_rep<CurvedKernelViaAnalysis_2>}$>$   \\
class \ccc{Point_2};
...
\ccGlue
\ccConstructor{Point_2(const Coordinate_1& x, const Curve_analysis_2& c, int arcno);}
{
...
\item[Returns:]The constructed point \end{description}
\ccGlue
\ccConstructor{Point_2(const Coordinate_1& x, const Curve_analysis_2& c, Arr_curve_end inf_end);}
{
[protected] \\
...
\ccGlue
\ccMethod{const Coordinate_2& xy() const;}
{
Access to underlying \ccc{Coordinate_2} object.
}
\ccGlue
\begin{description}
