...
\begin{ccRefFunctionObjectClass}[CGAL::internal::Curved_kernel_via_analysis_2_Functors::]{Compare_y_at_x_2}  %% add template arg's if necessary
\ccRefLabel{Compare_y_at_x_2}
...
\end{description}
\begin{description}
\item[Returns:]SMALLER if y(p) $<$ cv(x(p)), i.e., the point is below the arc; LARGER if y(p) $>$ cv(x(p)), i.e., the point is above the arc; EQUAL if p lies on the arc\end{description}
\begin{description}
