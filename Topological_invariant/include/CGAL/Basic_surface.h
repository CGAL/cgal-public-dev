#ifndef BASIC_SURFACE_H
#define BASIC_SURFACE_H

#include "Graph_on_surface.h"

namespace CGAL
{
    template<class Items_, class Alloc_>
    class Basic_surface
    {
    public:
        //Constructor
        ...
        template<class GMap>
        Basic_surface(const GMap& gmap): mGMap(gmap) {
            mSignature = get_new_mark();
        }
        ...
        typedef Basic_surface<Items, Alloc> Self;
        
        //GMap Types
        typedef typename Items::template Wrapper<Self>::GMap GMap;
        ...
        //Graph Types
        typedef Graph_on_surface<Self>                              Graph;
        typedef typename Alloc::template rebind<Graph>::other       Graph_allocator;
        typedef Compact_container<Graph,Graph_allocator>            Graph_container;
        typedef typename Graph_container::iterator                  Graph_handle;
        typedef typename Graph_container::const_iterator            Graph_const_handle;
      ...
      /*  Arc_occurence_order_range arc_occurence_order(Halfedge_handle he){
        ...
        Graph_handle create_Graph(){
            Graph_handle g = mGraphs.insert(Graph(*this));
            return g;
        }
        
        void erase_Graph (Graph_handle g){
            mGraphs.erase(g);
        }
