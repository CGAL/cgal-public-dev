...
\cgalRefines `GeneralizedMap`

Refines GeneralizedMap in order to handle non-orientable surfaces. Also adds  an interface to simulate the structure of signed half-edges as formalized by Mohar and Thomassen. This concept allows create paths and embedded graph.
...
public:
    ...
    bool sign(Dart_handle d) const;
    
    /// return the sign of a dart.  
    bool sign(Dart_const_handle d) const;
