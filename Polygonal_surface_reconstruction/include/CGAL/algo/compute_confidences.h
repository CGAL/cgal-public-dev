...
namespace CGAL {
	...
	*/
	...
	void Candidate_confidences<Kernel>::compute(const Point_set& point_set, Mesh& mesh) {
		...
		Mesh::Property_map<Face_descriptor, std::size_t> face_num_supporting_points =
			mesh.add_property_map<Face_descriptor, std::size_t>("f:num_supporting_points").first;

		// the area of each face
		Mesh::Property_map<Face_descriptor, FT> face_areas =
			mesh.add_property_map<Face_descriptor, FT>("f:face_area").first;

		// the point covered area of each face
		Mesh::Property_map<Face_descriptor, FT> face_covered_areas =
			mesh.add_property_map<Face_descriptor, FT>("f:covered_area").first;
		...
		BOOST_FOREACH(Face_descriptor f, mesh.faces()) {
			...
			if (area > degenerate_face_area_threshold) {
				...
				if (alpha_mesh.extract_mesh(radius * radius, covering_mesh)) {
					...
					const Mesh::Property_map<Vertex_descriptor, Point>& coords = covering_mesh.points();
