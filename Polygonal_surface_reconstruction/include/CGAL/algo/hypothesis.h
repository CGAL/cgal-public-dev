...
namespace CGAL {
	...
	*/
	...
	public:
		...
		Adjacency extract_adjacency(const Mesh& mesh);
	...
	private:
		...
		bool do_intersect(const Mesh& mesh, Face_descriptor face, const Plane* plane);

		// cuts face using the cutting_plane and returns the new faces
		std::vector<Face_descriptor> cut(Face_descriptor face, const Plane* cutting_plane, Mesh& mesh);

		// collect all faces in 'mesh' that intersect 'face'. Store in std::set() so easier to erase
		std::set<Face_descriptor> collect_intersecting_faces(Face_descriptor face, const Mesh& mesh);
		...
		void compute_intersections(const Mesh& mesh,
		...
		Halfedge_descriptor split_edge(Mesh& mesh, const EdgePos& ep, const Plane* cutting_plane);
	...
	namespace details {
		...
		typename BBox::FT bbox_radius(const BBox& box) {
			BBox::FT dx = box.xmax() - box.xmin();
			BBox::FT dy = box.ymax() - box.ymin();
			BBox::FT dz = box.zmax() - box.zmin();
	...
	void Hypothesis<Kernel>::merge(Planar_segment* s1, Planar_segment* s2, FT max_allowed_dist) {
		...
		std::vector< Planar_segment* >::iterator pos = std::find(segments.begin(), segments.end(), s1);
		if (pos != segments.end()) {
	...
	void Hypothesis<Kernel>::construct_bbox_mesh(Mesh& mesh) {
		...
		Mesh::Property_map<Face_descriptor, const Plane*> face_supporting_planes =
			mesh.add_property_map<Face_descriptor, const Plane*>("f:supp_plane").first;

		// the supporting planes of each edge
		Mesh::Property_map<Edge_descriptor, std::set<const Plane*> > edge_supporting_planes
			= mesh.add_property_map<Edge_descriptor, std::set<const Plane*> >("e:supp_plane").first;

		// the supporting planes of each vertex
		Mesh::Property_map<Vertex_descriptor, std::set<const Plane*> > vertex_supporting_planes
			= mesh.add_property_map<Vertex_descriptor, std::set<const Plane*> >("v:supp_plane").first;
		...
		BOOST_FOREACH(Edge_descriptor ed, mesh.edges()) {
			...
			CGAL_assertion(f1 != Mesh::null_face()); // the bbox mesh is closed
			CGAL_assertion(f2 != Mesh::null_face()); // the bbox mesh is closed
		...
		BOOST_FOREACH(Vertex_descriptor vd, mesh.vertices()) {
			...
			CGAL::Halfedge_around_target_circulator<Mesh> hbegin(vd, mesh), done(hbegin);
			do {
	...
	void Hypothesis<Kernel>::construct_proxy_mesh(const Mesh& bbox_mesh, Mesh& mesh) {
		...
		Mesh::Property_map<Face_descriptor, const Plane*> bbox_face_supporting_planes
			= bbox_mesh.property_map<Face_descriptor, const Plane*>("f:supp_plane").first;
		Mesh::Property_map<Edge_descriptor, std::set<const Plane*> > bbox_edge_supporting_planes
			= bbox_mesh.property_map<Edge_descriptor, std::set<const Plane*> >("e:supp_plane").first;
		Mesh::Property_map<Vertex_descriptor, std::set<const Plane*> > bbox_vertex_supporting_planes
			= bbox_mesh.property_map<Vertex_descriptor, std::set<const Plane*> >("v:supp_plane").first;
		...
		Mesh::Property_map<Face_descriptor, Planar_segment*> face_supporting_segments =
			mesh.add_property_map<Face_descriptor, Planar_segment*>("f:supp_segment").first;
		...
		const Mesh::Property_map<Vertex_descriptor, Point>& coords = bbox_mesh.points();
		...
		for (std::size_t i = 0; i < segments.size(); ++i) {
			...
			if (intersecting_points.size() >= 3) {
				...
				for (std::list<Point>::iterator it = hull.begin(); it != hull.end(); ++it) {
				...
				if (ch.size() >= 3) {
					...
					for (std::size_t j = 0; j < ch.size(); ++j) {
						Vertex_descriptor vd = mesh.add_vertex(ch[j]);
						descriptors.push_back(vd);
						vertex_supporting_planes[vd] = ch_source_planes[j];
						CGAL_assertion(vertex_supporting_planes[vd].size() == 3);
					}

					Face_descriptor fd = mesh.add_face(descriptors);
					face_supporting_segments[fd] = g;
					face_supporting_planes[fd] = cutting_plane;

					// assign each edge the supporting planes
					CGAL::Halfedge_around_face_circulator<Mesh> hbegin(mesh.halfedge(fd), mesh), done(hbegin);
					do {
						Halfedge_descriptor hd = *hbegin;
						Edge_descriptor ed = mesh.edge(hd);

						Vertex_descriptor s_vd = mesh.source(hd);
						Vertex_descriptor t_vd = mesh.target(hd);
	...
	typename Hypothesis<Kernel>::Halfedge_descriptor Hypothesis<Kernel>::split_edge(Mesh& mesh, const EdgePos& ep, const Plane* cutting_plane) {
		// the supporting planes of each edge
		Mesh::Property_map<Edge_descriptor, std::set<const Plane*> > edge_supporting_planes =
			mesh.property_map<Edge_descriptor, std::set<const Plane*> >("e:supp_plane").first;

		// the supporting planes of each vertex
		Mesh::Property_map<Vertex_descriptor, std::set<const Plane*> > vertex_supporting_planes
			= mesh.property_map<Vertex_descriptor, std::set<const Plane*> >("v:supp_plane").first;
		...
		if (h == Mesh::null_halfedge()) // failed splitting edge
			return h;
		...
		Mesh::Property_map<Vertex_descriptor, Point>& coords = mesh.points();
		coords[v] = ep.pos;
	...
	std::vector<typename Hypothesis<Kernel>::Face_descriptor> Hypothesis<Kernel>::cut(Face_descriptor face, const Plane* cutting_plane, Mesh& mesh) {
		...
		// the supporting plane of each face
		Mesh::Property_map<Face_descriptor, const Plane*> face_supporting_planes =
			mesh.property_map<Face_descriptor, const Plane*>("f:supp_plane").first;
		...
		// the supporting planar segment of each face
		Mesh::Property_map<Face_descriptor, Planar_segment*> face_supporting_segments =
			mesh.property_map<Face_descriptor, Planar_segment*>("f:supp_segment").first;
		...
		Halfedge_descriptor h0 = Mesh::null_halfedge();
		Halfedge_descriptor h1 = Mesh::null_halfedge();
		...
		CGAL_assertion(h0 != Mesh::null_halfedge());
		CGAL_assertion(h1 != Mesh::null_halfedge());
	...
	template <typename Kernel>
	void Hypothesis<Kernel>::compute_intersections(const Mesh& mesh,
		Face_descriptor face, const Plane* cutting_plane,
		std::vector<Vertex_descriptor>& existing_vts,
		std::vector<EdgePos>& new_vts)
	{
	...
	template <typename Kernel>
	bool Hypothesis<Kernel>::halfedge_exists(Vertex_descriptor v1, Vertex_descriptor v2, const Mesh& mesh) {
	...
	template <typename Kernel>
	bool Hypothesis<Kernel>::do_intersect(const Mesh& mesh, Face_descriptor f, const Plane* plane) {
		std::vector<Vertex_descriptor> existing_vts;
		std::vector<EdgePos> new_vts;
	...
	void Hypothesis<Kernel>::pairwise_intersection(Mesh& mesh) 
	{
		...
		// const Mesh::Face_range& all_faces = mesh.faces();
		...
		for (std::size_t i = 0; i < all_faces.size(); ++i) {
			...
			std::set<Face_descriptor> intersecting_faces = collect_intersecting_faces(face, mesh);
			...
			while (!intersecting_faces.empty()) {
				...
				for (std::size_t j = 0; j < faces_to_be_cut.size(); ++j) {
					...
					std::vector<Face_descriptor> tmp = cut(current_face, cutting_plane, mesh);
	...
	typename Hypothesis<Kernel>::Adjacency Hypothesis<Kernel>::extract_adjacency(const Mesh& mesh)
	{
		...
		BOOST_FOREACH(Halfedge_descriptor h, mesh.halfedges()) {
			...
			Vertex_descriptor sd = mesh.source(h);
			Vertex_descriptor td = mesh.target(h);
