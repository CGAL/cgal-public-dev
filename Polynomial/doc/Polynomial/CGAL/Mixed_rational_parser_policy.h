...
`Mixed_rational_parser_policy` allows mixing rational and integer coefficients in the same expression. `Polynomial_d` must be a polynomial whose `Innermost_coefficient_type`
is `ExplicitInteroperable` with integer and rational coefficients used in the same expression. `Integer` and `Rational` number types are provided as template parameters.
...
template < class Polynomial_d, class Integer, class Rational >
class Mixed_rational_parser_policy {
public:
...
returns <b>true</b> since `Mixed_rational_parser_policy` does not perform exponent check.
*/ 
...
}; /* end Mixed_rational_parser_policy */
} /* end namespace CGAL */
