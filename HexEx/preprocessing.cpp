...
Aff_transformation HexExtr::extract_transition_function(Dart_handle dh, const std::vector<Aff_transformation>& G){ //the function returns the tranformation g applied to one tet to get to the vertices of the adjacent tet.
  Aff_transformation id(1,0,0,0,1,0,0,0,1,1);
    if(input_tet_mesh.is_free(dh, 3)){//if the tet is on the boundary, it doesn't have a neighbouring tet. Returns identity.
      ...
      Dart_const_handle dh2 = input_tet_mesh.beta<3>(dh1); //dart handle corresponding to the same edge but in the adjacent tet
      std::vector<Point> face1, face2; 
      int i=0;
     
//adding the vertices of face1 (face of the first tet)
      face1.push_back(input_tet_mesh.point(dh1));
      face1.push_back(input_tet_mesh.point(input_tet_mesh.beta(dh1,1)));//(lcc.beta(dh1,1,0)));
      face1.push_back(input_tet_mesh.point(input_tet_mesh.beta(dh1,1,1)));//(lcc.beta(dh1,1,0,1,0)));
...
itend = input_tet_mesh.one_dart_per_cell<2>().end(); it != itend; it++){
    Aff_transformation at = extract_transition_function(it, G);
