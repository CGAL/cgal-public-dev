...
#ifndef CGAL_SURFACE_MESH_SIMPLIFICATION_DETAIL_WORKSHARING_DATA_STRUCTURES_H
#define CGAL_SURFACE_MESH_SIMPLIFICATION_DETAIL_WORKSHARING_DATA_STRUCTURES_H
...
namespace CGAL { namespace Surface_mesh_simplification {
...
// Load-based
#ifdef CGAL_SURFACE_MESH_SIMPLIFICATION_LOAD_BASED_WORKSHARING
...
/*
 * ==============
 * class MeshSimplificationWorkItem
 * Concrete class for a piece of work in the simplification process.
 * ==============
 */
...
{
public:
  MeshSimplificationWorkItem(const Func& func, const Priority &quality)
    : m_func(func), m_quality(quality)
  {}

  virtual ~MeshSimplificationWorkItem()
  {}

  void run()
  {
    m_func();
    tbb::scalable_allocator<MeshSimplificationWorkItem<Func, Priority> >().deallocate(this, 1);
  }

  bool less_than (const WorkItem &other) const
  {
    const MeshSimplificationWorkItem& other_cwi = static_cast<const MeshSimplificationWorkItem<Func,Priority>&>(other);
...
public:
  ...
  Load_based_worksharing_ds(const Bbox_3 &bbox)
    : NUM_WORK_ITEMS_PER_BATCH(
        Concurrent_simplifier_config::get().num_work_items_per_batch),
      m_num_cells_per_axis(
        Concurrent_simplifier_config::get().work_stats_grid_num_cells_per_axis),
  ...
  {
    WorkItem *p_item = new MeshSimplificationWorkItem<Func, Quality>(f, quality);
...
public:
  ...
  {
    WorkItem *p_item =
      tbb::scalable_allocator<MeshSimplificationWorkItem<Func, Quality> >()
      .allocate(1);
    new (p_item) MeshSimplificationWorkItem<Func, Quality>(f, quality);
    WorkBatch &workbuffer = m_tls_work_buffers.local();
    workbuffer.add_work_item(p_item);
    if (workbuffer.size() >= NUM_WORK_ITEMS_PER_BATCH)
    {
...
#endif // CGAL_SURFACE_MESH_SIMPLIFICATION_DETAIL_WORKSHARING_DATA_STRUCTURES_H
