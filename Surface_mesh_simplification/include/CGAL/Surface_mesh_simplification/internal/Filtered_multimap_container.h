...
#ifndef CGAL_SURFACE_MESH_SIMPLIFICATION_DETAIL_FILTERED_MULTIMAP_CONTAINER_H
#define CGAL_SURFACE_MESH_SIMPLIFICATION_DETAIL_FILTERED_MULTIMAP_CONTAINER_H
  ...
  ************************************************/
  ...
  protected:
    ...
    bool no_longer_local_element_to_refine_impl(const Predicate &test)
    {
      ...
      while( !is_empty && !test(m_local_lists.local().front().second, m_local_lists.local().front().first) )
      {
  ...
  ***********************Class Filtered_multimap*************************/
  ...
  public:
    ...
    bool no_longer_element_to_refine_impl()
    {
      ...
      while( !is_empty && !test(container.begin()->second, container.begin()->first) )
      {
    ...
    bool is_zombie(const Element& e, const Quality& q) const {
      return !test(e, q);
    }
...
#endif // CGAL_SURFACE_MESH_SIMPLIFICATION_DETAIL_FILTERED_MULTIMAP_CONTAINER_H
