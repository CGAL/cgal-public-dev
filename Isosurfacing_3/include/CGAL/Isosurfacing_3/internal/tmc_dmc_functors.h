// exactly the private functions in topologically_coorect_marching_cubes_functors.h
// but instead standalone
#ifndef CGAL_ISOSURFACING_3_INTERNAL_TMC_DUAL_MARCHING_CUBE_H
#define CGAL_ISOSURFACING_3_INTERNAL_TMC_DUAL_MARCHING_CUBE_H
...
inline void face_intersections(const std::array<FT, 8>& values, const std::size_t idx, const FT i0, FT& a, FT& b, FT& c, FT& d) {
...
template<typename FT>
inline bool calc_coordinates(const std::array<FT, 8>& values, const std::size_t idx, const FT i0, const FT a, const FT b, const FT d, const std::vector<bool> &f_flag, unsigned char &q_sol, FT ui[2], FT vi[2], FT wi[2]) {
    const int* remap = internal::Cube_table::asymptotic_remap[idx];
...
template<typename Domain, typename Point_3, typename FT>
inline bool p_slice(const Domain& domain,
             ...
             std::vector<std::array<Point_3, 3>>& triangles) 
{
    ...
    if(numberOfSetBits(q_sol) == 6)
    {
        ...
        for(int t=0; t < (int)cnt_; ++t)
        {
          if(get_cnt_size(t, c_) == 3)
          {
            ...
            if(uc_min > umax || uc_max < umin)
            {
              ...
              triangles.push_back({vertices[e0], vertices[e1], vertices[e2]});
            }
          }
        }
      }
      ...
      for(int i=0; i<(int)cnt_; ++i)
      {
        if(_not_tunnel != i)
        {
          ...
          for(int r=0; r<cnt_sz; ++r)
          {
            ...
            {
              case 0:
                triangles.push_back({vertices[tid1], vertices[tid2], tg_idx[cid1]});
              break;
              case 1:
              {
                // measure diagonals
                ...
                if(l1 < l2)
                {
                  triangles.push_back({vertices[tid1], vertices[tid2], tg_idx[cid2]});
                  triangles.push_back({vertices[tid1], tg_idx[cid2], tg_idx[cid1]});
                }
                else
                {
                  triangles.push_back({vertices[tid1], vertices[tid2], tg_idx[cid1]});
                  triangles.push_back({vertices[tid2], tg_idx[cid2], tg_idx[cid1]});
                }
              }
              break;
              case 2:
              {
                ...
                triangles.push_back({vertices[tid1], vertices[tid2], tg_idx[cidm]});
                triangles.push_back({vertices[tid1], tg_idx[cidm], tg_idx[cid1]});
                triangles.push_back({vertices[tid2], tg_idx[cid2], tg_idx[cidm]});
              }
              break;
            } // switch
      ...
      if(cnt_ == 1)
      {
        ...
        if(!of_)
        {
          triangles.push_back({tg_idx[0], tg_idx[2], tg_idx[1]});
          triangles.push_back({tg_idx[2], tg_idx[4], tg_idx[3]});
          triangles.push_back({tg_idx[0], tg_idx[5], tg_idx[4]});
          triangles.push_back({tg_idx[0], tg_idx[4], tg_idx[2]});
        }
        else
        {
          triangles.push_back({tg_idx[0], tg_idx[1], tg_idx[2]});
          triangles.push_back({tg_idx[2], tg_idx[3], tg_idx[4]});
          triangles.push_back({tg_idx[0], tg_idx[4], tg_idx[5]});
          triangles.push_back({tg_idx[0], tg_idx[2], tg_idx[4]});
        }
      }
    }
    else
    {
      ...
      if(nr_t == nr_u || nr_t == nr_v || nr_t == nr_w)
      {
        ...
        for(int i=0; i<(int)cnt_; ++i)
        {
          switch (get_cnt_size(i, c_))
          {
            case 3:
            {
              triangles.push_back({vertices[get_c(i, 0, c_)], vertices[get_c(i, 1, c_)],
            ...
            case 4:
            {
              triangles.push_back({vertices[get_c(i, 0, c_)], vertices[get_c(i, 1, c_)],
                           vertices[get_c(i, 2, c_)]});
              triangles.push_back({vertices[get_c(i, 0, c_)], vertices[get_c(i, 2, c_)],
            ...
            case 5:
            {
              ...
              triangles.push_back({vertices[get_c(i, 0, c_)], vertices[get_c(i, 2, c_)],
                           vertices[get_c(i, 3, c_)]});
              triangles.push_back({vertices[get_c(i, 0, c_)], vertices[get_c(i, 3, c_)],
            ...
            case 6:
            {
              triangles.push_back({vertices[get_c(i, 0, c_)], vertices[get_c(i, 1, c_)],
                           vertices[get_c(i, 3, c_)]});
              triangles.push_back({vertices[get_c(i, 1, c_)], vertices[get_c(i, 2, c_)],
                           vertices[get_c(i, 3, c_)]});
              triangles.push_back({vertices[get_c(i, 0, c_)], vertices[get_c(i, 3, c_)],
                           vertices[get_c(i, 4, c_)]});
              triangles.push_back({vertices[get_c(i, 0, c_)], vertices[get_c(i, 4, c_)],
      ...
      else // there are no saddle points
      {
        ...
        for(int i=0; i<int(cnt_); ++i)
        {
          ...
          if(cnt_sz == 3)
          {
            triangles.push_back({vertices[get_c(i, 0, c_)], vertices[get_c(i, 1, c_)], vertices[get_c(i, 2, c_)]});
          }
          else
          {
            for(int t=0; t<cnt_sz; ++t)
              triangles.push_back({vertices[get_c(i, t, c_)],
