...
#ifndef CGAL_URBAN_AREA_PROCESSING_INTERNAL_LEAST_SQUARES_LINE_FIT_SORTING_2_H
#define CGAL_URBAN_AREA_PROCESSING_INTERNAL_LEAST_SQUARES_LINE_FIT_SORTING_2_H
...
namespace internal {
  ...
  typename PointMap>
  class Least_squares_line_fit_sorting_2 {
  ...
  public:
    ...
    Least_squares_line_fit_sorting_2(
    ...
    void sort() {  
      ...
      compute_scores();
      CGAL_assertion(m_scores.size() > 0);
    ...
    void compute_scores() {
      ...
      for (std::size_t i = 0; i < m_input_range.size(); ++i) {
        ...
        for (std::size_t j = 0; j < neighbors.size(); ++j) {
          ...
          CGAL_assertion(neighbors[j] >= 0);
          CGAL_assertion(neighbors[j] < m_input_range.size());

          const auto& key = *(m_input_range.begin() + neighbors[j]);
          points.push_back(m_to_local_converter(get(m_point_map, key)));
        }
        CGAL_assertion(points.size() == neighbors.size());
...
#endif // CGAL_URBAN_AREA_PROCESSING_INTERNAL_LEAST_SQUARES_LINE_FIT_SORTING_2_H
