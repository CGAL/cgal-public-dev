...
int main(int argc, char* argv[]) //arguments : 1.input mesh file name, 2.output .xyz file name, 3.lvl
{
  ...
  //initialization and subdivision
  size_t lvl = atoi(argv[3]);
  size_t nb_vert = new_mesh.vertices().size();
  Subdivision_method_3::Loop_subdivision(new_mesh, params::number_of_iterations(1));
  ...
  size_t nb_faces = new_mesh.faces().size();
  ...
  //generate clusters
  size_t b = 0;
  size_t nb_clusters = ceil(lvl/3.0);
  std::cout << nb_clusters << std::endl;
  ...
  for (size_t i = 0; i < nb_clusters ; ++i) {
    size_t face_rdm_cluster = CGAL::get_default_random().uniform_int(b,nb_faces-1);
  ...
  //remove points
  size_t mult = ((lvl - 1)%3+1);
  size_t dist_th = mult * ceil(dmax / 100.0)  ;
  std::cout << (dist_th) << std::endl;
