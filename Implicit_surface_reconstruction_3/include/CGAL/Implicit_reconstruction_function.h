...
private:
                      ...
                      double laplacian)
  {
    ...
    dihedral_angle_per_cell("bad_tet_octree_bimba.off");
    perturb_sliver(4);
    perturb_sliver(3, true);
    dihedral_angle_per_cell("bad_tet_octree_bimba_perturb.off");
...
/*
  ...
  void get_rainbow_color(const double ratio, Color& color)
  {
   ...
   void perturb_sliver(double sliver_lb, bool force_empty=false)
   {
       // 1. found all slivers, construct vertices and add to a priority queue
       double tan_ub = tan(sliver_lb / 180.0 * boost::math::constants::pi<double>()); // TODO: is double ok?
       //int count = 0;
       ...
       std::cout << "total_slivers:" << PVertex_buffer_map.size() << std::endl;
       ...
       while (!pqueue.empty())
       {
           ...
           while (iter<5)
           {
               ...
               if (pertubation_vec != CGAL::NULL_VECTOR)
               {
                   ...
                   if (force_empty)
                   {
                       if (new_slivers.size() != 0) // if not 0
                       {
                           pv.next_perturbation();
                           pv.set_vertex(m_tr->move(pv.vertex(), old_pos));
                           //pv.vertex()->index() = v_idx;
                           //pv.set_id(v_idx);
                           //update_priority_queue(pv, pqueue);
                       }
                   }
                   else if (new_slivers.size() >= slivers.size()) // if not better
                   {   // revert and reinsert
                       pv.next_perturbation();
                       pv.set_vertex(m_tr->move(pv.vertex(), old_pos));
                       //pv.vertex()->index() = v_idx;
