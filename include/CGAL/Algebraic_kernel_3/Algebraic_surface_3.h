...
#ifndef CGAL_ALGEBRAIC_KERNEL_3_ALGEBRAIC_SURFACE_3_H
#define CGAL_ALGEBRAIC_KERNEL_3_ALGEBRAIC_SURFACE_3_H 1

/*!\file include/CGAL/Algebraic_kernel_3/Algebraic_surface_3.h
...
public:
  ...
  //! the class itself
  typedef Algebraic_surface_3_rep< Coefficient, Unify > Self;
  ...
  typedef typename 
  CGAL::Polynomial_type_generator<Coefficient,3>::Type Polynomial_3;
  ...
  // friends
  friend class Algebraic_surface_3< Coefficient, Unify, Self >;
};
...
class Rep_ = 
CGAL::internal::Algebraic_surface_3_rep< Coefficient_, Unify_ >
>
...
public:
  ...
  //! the class itself
  typedef Algebraic_surface_3< Coefficient, Unify, Rep > Self;
...
private:
  ...
  typedef typename CGAL::Polynomial_traits_d< Polynomial_3 > PT_3;
...
public:
  ...
  Algebraic_surface_3(const Polynomial_3& p, bool z_regular_check = false) :
    ...
    CGAL_assertion_code(int n = CGAL::degree(p_canon);)
...
public:
  ...
  Polynomial_3 f(int index) const {
    ...
    CGAL_precondition(index <= CGAL::degree(f()));
    // TODO use polynomial traits
  ...
  Coefficient coefficient(int i, int j, int k) const {
    ...
    CGAL_precondition(j >= 0);
    CGAL_precondition(k >= 0);
    CGAL_precondition(i + j + k <= CGAL::total_degree(f()));
    // TODO use polynomial traits
    if (k <= this->f().degree()) {
  ...
  Polynomial_3 fz() const {
    ...
    if (!this->ptr()->_m_fz ) {
      typename PT_3::Differentiate diff;
      this->ptr()->_m_fz = diff(this->ptr()->_m_f);
    }
    ...
    if (! this->ptr()->_m_resultant_f_fz) {
      if(cofactors) {
        ...
        if (this->ptr()->_m_resultant_f_fz->is_zero()) {
          throw CGAL::internal::Zero_resultant_exception< Polynomial_3 >(f());
        }
      }
    }
...
private:
  ...
  void compute_sturm_habicht(int index = 0) const {
    ...
    if(! this->ptr()->_m_stha_f[index]) {
      ...
      Polynomial_3 truncated_f = f(CGAL::degree(f())-index);
...
public:
  ...
  Polynomial_3 sturm_habicht_polynomial(int i, 
    ...
    int n = CGAL::degree(f()), index = n - degree;
    if (!this->ptr()->_m_stha_f[index]) {
      if (cofactors) {
    ...
    CGAL_assertion(i >= 0);
    CGAL_assertion(i < static_cast<int>
  ...
  Polynomial_3 sturm_habicht_polynomial( int i, 
    ...
    return sturm_habicht_polynomial(i, CGAL::degree(f()), cofactors);
  }
  ...
  Polynomial_2 principal_sturm_habicht_coefficient( 
    ...
    return 
      principal_sturm_habicht_coefficient(i,CGAL::degree(f()), cofactors);
  }
  ...
  Polynomial_3 cofactor_for_f(int i) const {
    return cofactor_for_f( i, CGAL::degree(f()));
  }
   ...
   */
  Polynomial_3 cofactor_for_fz(int i, int degree) const {
  ...
  Polynomial_3 cofactor_for_fz(int i) const {
    return cofactor_for_fz(i, CGAL::degree(f()));
  }
...
std::ostream& operator<<(
    std::ostream& os, 
    const Algebraic_surface_3< Coefficient_, Unify_, Rep_ >& surface) {
...
#endif // CGAL_ALGEBRAIC_KERNEL_3_ALGEBRAIC_SURFACE_3_H
// EOF
