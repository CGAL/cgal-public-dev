...
#ifndef CGAL_ALGEBRAIC_KERNEL_3_ALGEBRAIC_SURFACE_3_Z_AT_XY_ISOLATOR_TRAITS_BASE_H
#define CGAL_ALGEBRAIC_KERNEL_3_ALGEBRAIC_SURFACE_3_Z_AT_XY_ISOLATOR_TRAITS_BASE_H 1

/*!\file include/CGAL/Algebraic_kernel_3/Algebraic_surface_3_z_at_xy_isolator_traits_base.h
...
public:    
    ...
    typedef CGAL::internal::Wrap_ckva_for_z_at_xy_isolator_traits_2 <
...
public:
    ...
    public:
        ...
        int operator()(const Surface_3& surface1, const Surface_3& surface2,
            ...
            CGAL_precondition(CGAL::gcd(surface_f1, surface_f2).degree() < 1);
            
            // compute correct polynomial // TODO is CGAL::resultant best?
            Polynomial_2 f = CGAL::resultant(surface_f1, surface_f2);
    ...
    public:
        ...
        OutputIterator out) const {
            ...
            if( n == 0 ) {
                ...
                *out++ = isolator.left_bound(0) - Rational(1);

                for( int i = 1; i < n; i++ ) {
                    
                    *out++ = (isolator.left_bound(i) + 
                              isolator.right_bound(i-1)) / 2 ;
                    
                    // Note that even if both are equal, this value is always
                    // an intermediate one
                }
                
            *out++ = isolator.right_bound(n-1)+Rational(1);
    ...
    public:
        ...
        OutputIterator operator() (Z_at_xy_isolator isolator,
            ...
            while( (it != bound_end) || i!=index_end) {
                ...
                while( isolator.left_bound(*i) <= *it &&
                       *it <= isolator.right_bound(*i) ) {
                
                    isolator.refine_interval(*i);

                }
                
                if( isolator.left_bound(*i) > *it ) {
                    *out++ = bucket_roots;
                    bucket_roots=0;
                    it++;
                    continue;
...
#endif // CGAL_ALGEBRAIC_KERNEL_3_ALGEBRAIC_SURFACE_3_Z_AT_XY_ISOLATOR_TRAITS_BASE_H
// EOF
