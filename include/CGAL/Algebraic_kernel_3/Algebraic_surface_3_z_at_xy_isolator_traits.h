...
#ifndef CGAL_ALGEBRAIC_KERNEL_3_ALGEBRAIC_SURFACE_3_Z_AT_XY_ISOLATOR_TRAITS_H
#define CGAL_ALGEBRAIC_KERNEL_3_ALGEBRAIC_SURFACE_3_Z_AT_XY_ISOLATOR_TRAITS_H 1

/*!\file include/CGAL/Algebraic_kernel_3/Algebraic_surface_3_z_at_xy_isolator_traits.h
...
#include <CGAL/Algebraic_kernel_3/Algebraic_surface_pair_3.h>
...
*/
...
#endif      
            Polynomial_2 gcd12 = CGAL::gcd(f1,f2);
            f2 = CGAL::integral_division(f2,gcd12);
#if !NDEBUG
            std::cout << "2.." << std::flush;
#endif
            Polynomial_2 gcd13 = CGAL::gcd(f1,f3);
            f3 = CGAL::integral_division(f3,gcd13);
#if !NDEBUG
            std::cout << "3.." << std::flush;
#endif            
            Polynomial_2 gcd23 = CGAL::gcd(f2,f3);
...
#endif
                        } catch (CGAL::internal::Non_generic_position_exception 
                                 Ex) {
#if !NDEBUG
    ...
    static Interval evaluate_polynomial_2_at_approximated_point(Poly_2 f, 
        ...
        typedef typename 
	  CGAL::Polynomial_traits_d<Poly_2>::Innermost_coefficient_type 
	  Coefficient;
	typedef typename 
	  CGAL::Polynomial_traits_d<Poly_2>::Coefficient_type 
	  Poly_1;
	
        typename CGAL::internal::Interval_evaluate_1<Poly_1,Rational> interval_evaluate_1;
        CGAL_assertion(f.degree()>=0);
        int n=f.degree();
        Interval ret(0,0);
        for(int i=n;i>=0;i--) {
            ret *= y_iv;
	    std::pair<Rational,Rational> curr_pair 
	      = interval_evaluate_1(f[i],std::make_pair(x_iv.lower(),
							x_iv.upper()));
            ret += Interval(curr_pair.first,curr_pair.second);
        }
        return ret;
    }
...
public:    
        ...
        bool operator()(
            ...
            if (k == 1 || !on_silhouette1 || !on_silhouette2) {
                ...
                Polynomial_rat_2 local_gcd_at_left1_with_denom
                    = local_gcd_p.evaluate(isolator1.left_bound(i1));
                ...
                Polynomial_rat_2 local_gcd_at_right1_with_denom
                    = local_gcd_p.evaluate(isolator1.right_bound(i1));
                ...
                Rational left_bound = isolator1.left_bound(i1);
                Rational right_bound = isolator1.right_bound(i1);
                
                for(int i = 0; i < gcd_isol.number_of_real_roots(); i++) {
                    
                    if (gcd_isol.left_bound(i) > right_bound ||
                        gcd_isol.right_bound(i) < left_bound ) {
                        continue;
                    } else {
                        if (gcd_isol.left_bound(i) >= left_bound &&
                            gcd_isol.right_bound(i) <= right_bound) {
    ...
    public:
        ...
        typedef typename Restricted_cad_3::Coordinate_1 Algebraic_real_1;
        ...
        typedef typename Restricted_cad_3::Arrangement_traits_2::Curve_kernel_2
            ::Algebraic_kernel_d_1::Solve_1 Solve_1;

        typedef typename Restricted_cad_3::Arrangement_traits_2::Curve_kernel_2
            ::Algebraic_kernel_d_1::Bound_between_1 Bound_between_1;
    ...
    private:
        ...
        void _merge_real_roots( InputIterator curves_begin,
            ...
            for(InputIterator it = curves_begin; it!=curves_end; it++) {
                ...
                if(! pol_1.is_zero()) {
                    ...
                    if(! CGAL::is_square_free(pol_1) ) {
                        throw Box_boundary_square_full_exception();
                    }

                    std::vector< std::pair< Algebraic_real_1, int > > curr_roots;
                    
                    solve_1(pol_1, std::back_inserter(curr_roots) );
                    ...
                    for(int i = 0; 
                        ...
                        curr_roots_with_info.push_back
                            (std::make_pair(curr_roots[i].first, root_info));
                    }
        ...
        void _edge_adjacency_with_curve_analysis 
            ...
            if(is_vertical) {            
                ...
                std::vector< std::pair< Algebraic_real_1, int > > y_roots;
                ...
                he_it->second.sample_value = y_roots[arcno].first;
            }
        ...
        int _find_arcno(Algebraic_real_1 ar) const {
            ...
            solve_1(ar.polynomial(), std::back_inserter(roots));
            ...
            while(i < n) {
                ...
                Algebraic_real_1 root = roots[i].first;

                if( root.high() < ar.low() ) {
                    i++;
                } else if( root.high() >= ar.high() ) {
                    break;
                } else {
                    ar.refine();
                }
        ...
        int _exclude_nk_boundary_points_around_vertex
            ...
            for( int i=0; 
                ...
                while( (!(he_handle->source()->is_at_open_boundary())) && 
                ...
                if( he_handle->source()->is_at_open_boundary() ) {
                    continue;
                }
        ...
        CGAL::Object _locate_point_on_vertex_incident_path(
            ...
            for (typename Arr_2::Halfedge_const_iterator eit =
                ...
                if (eit->curve().equal_y_at_x(p)) {
                    if (!eit->target()->is_at_open_boundary() &&
                        eit->target()->point() == p) {
#if !NDEBUG
            ...
            case(LEFT): {
                ...
                if( box_info.second == y_iv.upper() ) {
                    ...
                    if(n == 1) {
                      ...
                      rat_x = Bound_between_1()(box_roots_top[0], box_roots_top[1]);
                    }
                ...
                if(root_it_succ == box_roots_left.end() ) {
                    ...
                    std::cout << "root_it_succ: " 
                              ...
                              << std::endl;
#endif
                    rat_y = Bound_between_1()(*root_it, *root_it_succ);
                }
            ...
            case(RIGHT): {
                if( box_info.second == y_iv.lower() ) {
                    ...
                    if(n == 1) {
                      ...
                      rat_x = Bound_between_1()(box_roots_bottom[n-2], box_roots_bottom[n-1]);
                    }
                ...
                if(root_it == box_roots_right.begin() ) {
                    ...
                    root_it_prec--;                        
                    rat_y = Bound_between_1()(*root_it_prec, *root_it);
                }
            ...
            case(BOTTOM) : {
                if( box_info.second == x_iv.lower() ) {
                    ...
                    if(n == 1) {
                      ...
                      rat_y = Bound_between_1()(box_roots_left[0], box_roots_left[1]);
                    }
                ...
                if(root_it == box_roots_bottom.begin() ) {
                    ...
                    root_it_prec = root_it;
                    root_it_prec--;                        
                    rat_x = Bound_between_1()(*root_it_prec, *root_it);
                }
            ...
            case(TOP) : {
                if( box_info.second == x_iv.upper() ) {
                    ...
                    if(n == 1) {
                      ...
                      rat_y = Bound_between_1()(box_roots_right[n-2], box_roots_right[n-1]);
                    }
                ...
                if(root_it_succ == box_roots_top.end() ) {
                  ...
                  rat_x = Bound_between_1()(*root_it, *root_it_succ);
                }
...
// NOTE NOTE NOTE: here is something wrong..
            if( v_handle->is_isolated() ) {
                ...
                typename Curve_kernel_2::Approximate_relative_y_2
                  approx_y = 
                  Arrangement_traits_2::instance().kernel().
                  approximate_relative_y_2_object();

		typename Curve_kernel_2::Bound_between_y_2 bound_between
		  = Arrangement_traits_2::instance().kernel().
		...
		typedef typename Curve_kernel_2::Bound Bound;
			...
			Bound lower_i, upper_i;
			
			while (i < vl.number_of_events()) {
                        
			    std::pair<Bound,Bound> pair_i 
			      = approx_y(vl.algebraic_real_2(i),prec);
			    upper_i = pair_i.first;
			    lower_i=pair_i.second;
			  
			    if (upper_i < y_iv.lower()) {
                                i++;
                            } else if (lower_i > y_iv.upper()) {
                                break;
                            } else {
          ...
          Map1& sample_point_map
            ...
            for(int i = 0; i < no_bucket_arcs; i++,oedge_it2++) {
                ...
                for( j = 0; j < n; j++ ) {
                    ...
                    if(bucket_isol.left_bound(j) <= z &&
                       bucket_isol.right_bound(j) >= z) {
                        break;
                    }
...
#endif // CGAL_ALGEBRAIC_KERNEL_3_ALGEBRAIC_SURFACE_3_Z_AT_XY_ISOLATOR_TRAITS_H
// EOF
