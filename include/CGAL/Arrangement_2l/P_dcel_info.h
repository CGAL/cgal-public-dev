...
::CGAL::Handle_with_policy< internal::P_dcel_info_rep< SurfaceZAtXyIsolatorTraits > > {
public:
    ...
    //! type of rep;
    typedef internal::P_dcel_info_rep< Surface_z_at_xy_isolator_traits > Rep;
...
private:
    ...
    Point_2 _sample_point_for_vertex_handle(VertexHandle vh) const {
        CGAL_precondition(!vh->is_at_open_boundary());
...
private:
    ...
    Point_2 _rational_point_in_face(FaceHandle fh, 
        ...
        if (he == invalid_he) {
            ...
            type = 1;
            Bound xr1 = he->curve().boundary_in_x_range_interior();
            x0 = Coordinate_1(xr1);
...
//             std::cout << "type:" << type << "; prec: " << prec << "\n";
            ...
            case 2: {
                ...
                CGAL_assertion(he->curve().is_vertical());
                Coordinate_1 x1 = he->curve().x();
                Bound xt2 = (he->direction() == CGAL::ARR_LEFT_TO_RIGHT ?
                      x1.low() : x1.high());
                if (x1.is_rational()) {
                    xr = xt2 + 
                        (he->direction() == CGAL::ARR_LEFT_TO_RIGHT ? 
                         Bound(-1) : Bound(1))/
                        CGAL::ipower(Bound(2),count);
                } else {
                    // TODO use ak
		    xr = bound_between_x(x1,Coordinate_1(xt2));
		}
                x0 = Coordinate_1(xr);
