...
private:
    ...
    //! type of cache
    typedef internal::Restricted_cad_3_cache< Surface_z_at_xy_isolator_traits >
...
private:
    ...
    void _remove_edge(Halfedge_handle he, bool remove_source,
             bool remove_target) {
        ///CGAL::remove_edge(*(this->ptr()), he);
        this->ptr()->remove_edge(he, remove_source, remove_target);
    }
...
private:
     ...
     */
    static
    Point_2 _construct_point_with_rational_y(Coordinate_1 x0, Bound y0) {
        ...
        if (cv.is_vertical()) {
            ...
            if (ps_y_min == CGAL::ARR_BOTTOM_BOUNDARY) { // -oo
                ...
                if (ps_y_max == CGAL::ARR_TOP_BOUNDARY) { // +oo
                    ...
                    y0 = bound_between_y(min_sl.algebraic_real_2(min_i),
            ...
            Coordinate_1 x0(cv.boundary_in_x_range_interior());
...
public:
    ...
    Z_at_xy_isolator
    isolator_at_point_on_halfedge(const Point_2& pt, const Surface_3& surface,
            Halfedge_const_handle heh) const {
        
        CGAL_precondition(_point_on_dcel_handle(pt, heh));
        CGAL::Nk nk = heh->data()->_nk(surface);
        typename Surface_z_at_xy_isolator_traits::Construct_isolator cisl;

        return cisl(surface, pt, nk, CGAL::EDGE);
    }

    Z_at_xy_isolator
    isolator_at_point_in_face(const Point_2& pt, const Surface_3& surface,
            Face_const_handle fh) const {
        
        CGAL_precondition(_point_on_dcel_handle(pt, fh));
        CGAL::Nk nk = fh->data()->_nk(surface);
        typename Surface_z_at_xy_isolator_traits::Construct_isolator cisl;

        return cisl(surface, pt, nk, CGAL::FACE);
    }
...
public:
    ...
    friend class CGAL::internal::Construct_nk_decomposition_2< 
    Surface_z_at_xy_isolator_traits >;
