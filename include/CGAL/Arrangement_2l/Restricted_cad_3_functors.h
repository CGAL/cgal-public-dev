        ...
        if (begin != end) {
            ...
            for (typename Restricted_cad_3::Edge_const_iterator 
                ...
                if (tmp.nk(eit, surface).feature1() == CGAL::FACE) {
                    ...
                    bool remove_src = true, remove_tgt = true;
                    //if(!eit->is_fictitious()) {
                    if (!ti->source()->is_at_open_boundary ())
                        remove_src = (tmp.nk(ti->source(),
                             surface).feature1() != CGAL::VERTEX);

                    if(!ti->target()->is_at_open_boundary())
                        remove_tgt = (tmp.nk(ti->target(),
                             surface).feature1() != CGAL::VERTEX);
                    //}
                    acc.remove_edge(acc.non_const_handle(ti),
                                   remove_src, remove_tgt);
                } else {
                    eit++;
                }
            }
...
private:
    ...
    Restricted_cad_3 operator()(const Surface_3& surface, 
        ...
        typedef CGAL::internal::Construct_nk_decomposition_2< 
            Surface_z_at_xy_isolator_traits >
...
private:
    ...
    void _set_silhouette(const Surface_3& surface, 
        ...
        for (typename Restricted_cad_3::Vertex_const_iterator vit =
            ...
            CGAL_assertion(!vit->is_at_open_boundary());
            if (!vit->data()) {
...
private:
    ...
    friend class 
    CGAL::internal::Restricted_cad_3_cache< Surface_z_at_xy_isolator_traits >;
    
}; // Create


// TODO move Overlay_restricted_cad_3 to namespace internal?!?!
/*!\brief
...
private:
    ...
    friend class 
    CGAL::internal::Construct_nk_decomposition_2< Surface_z_at_xy_isolator_traits >;
