...
public:
  ...
  public:
    ...
    Rational_point_2 source()
    {
      return source_p;
    }

    Rational_point_2 target()
    {
      return target_p;
    }
...
private:
  ...
  const Rational_segment_3 * m_S1, * m_S2;
...
public:
  ...
  void add_element_to_arrangement_S1S2_on_the_same_plane(const Rational_segment_3& S3)
  {
    if (S3.source() == S3.target())
    {
      ...
      {
        Rational p0_x_diff = 
          (S3.source().x() - m_intersection_point_S1S2.x());
        Rational p0_y_diff = 
          (S3.source().y() - m_intersection_point_S1S2.y());
        Rational p0_z_diff = 
          (S3.source().z() - m_intersection_point_S1S2.z());
        ...
        if (S3.has_on(m_intersection_point_S1S2))
        {
          ...
          std::cout << change_color(CGAL_YELLOW,
                    ...
                    << std::endl;
#endif
          if (m_num_of_intersection == 0)
          {
            m_S1_S2_concurrent_S3 = &S3;
          }
          else if (m_num_of_intersection == 1)
          {
            m_S1_S2_concurrent_S4 = &S3;
          }
        ...
        else
        {
          m_isolated_points_on_sphere.add_element
            (Point_on_sphere_and_line_pair(Point_on_sphere_2(p0_x_diff,
                                                             p0_y_diff,
                                                             p0_z_diff),
                                           &S3));
        }
  ...
  void add_element_to_plane_arrangement(const Rational_segment_3& S3)
  {
        ...
        m_S1->supporting_line().has_on(S3.target()))
    {
      Rational_point_3 ipoint;
      Rational S2_t;
      CGAL::Object result = m_rat_kernel->intersect_3_object()(m_S1->supporting_line(),m_S2->supporting_line());
  ...
  void add_element_to_one_dimensional_arrangement(const Rational_segment_3& S3)
  {
    ...
    if (S3.source() == S3.target() && m_S1_S2_plane.has_on(S3.source()))
    {
      ...
      Rational_point_3 ipoint;
      CGAL::Object result = m_rat_kernel->intersect_3_object()(m_S2->supporting_line(),
                                                               S3_S1_line);
      if (CGAL::assign(ipoint, result))
      {
  ...
  void add_element_to_arrangement(const Rational_segment_3& S3)
  {
    ...
    else if (are_overlap(*m_S1,S3))
    {
      ...
      if (S1_t_min > S1_t_max)
      {
        std::swap(S1_t_max,S1_t_min);
      }

      if (S1_t_min < Rational(0))
      {
        S1_t_min = Rational(0);
      }

      if (S1_t_max > Rational(1))
      {
        S1_t_max = Rational(1);
      }
    ...
    else if (are_overlap(*m_S2,S3))
    {
      ...
      if (S2_t_min > S2_t_max)
      {
        std::swap(S2_t_max,S2_t_min);
      }

      if (S2_t_min < Rational(0))
      {
        S2_t_min = 0;
      }
    ...
    else
    {
                                                     ...
                                                     S3.supporting_line()))
      {
        ...
        if (m_arr_g_func.calc_parallel_segments(*m_S1, *m_S2, S3, ipoint_S2, m_rat_kernel,S2_t,true))
        {
          std::list<Rational_arc_2> arcs;
                                                     ...
                                                     S3.supporting_line()))
      {
        ...
        if (m_arr_g_func.calc_parallel_segments(*m_S2, *m_S1, S3, ipoint_S1, m_rat_kernel,S1_t, true))
        {
          std::list<Rational_arc_2> arcs;
          m_arr_g_func.add_segs_to_arr_S1_is_a_point(*m_S2, 
                                                     S3, 
...
private:
      ...
      */
      ...
      CGAL::Object result =
        m_rat_kernel->intersect_3_object()(x_y_plane,
                                           Rational_line_3(Rational_point_3(p0_x_diff, p0_y_diff,
                                                                            p0_z_diff),
                                                           Rational_point_3(p1_x_diff, p1_y_diff,
                                                                            p1_z_diff)));
...

  {
    ...
    if (l.has_on(Rational_point_3(0,0,0)))
      return true;
...
private:
  ...
  void find_all_lines_sphere(OutputIterator out)
  {
    ...
    else
    {
      ...
      for (vit = m_arr_on_sphere->vertices_begin(); vit != 
             m_arr_on_sphere->vertices_end(); ++vit)
      {
#if ARR_ON_SUR_DEBUG
