...
#include <CGAL/license/Spectral_surface_reconstruction_3.h>
...
#include <CGAL/spectral_refine_triangulation.h>
...
/// \cond SKIP_IN_MANUAL
struct Spectral_visitor {
  void before_insertion() const
  {}
};

struct Spectral_skip_vertices { 
  double ratio;
  Random& m_random;
  Spectral_skip_vertices(const double ratio, Random& random)
...
/*!
\ingroup PkgSpectralSurfaceReconstruction
...
*/
template <class Gt>
class Spectral_reconstruction_function
{
// Public types
public:
...
private:
  ...
  // Note: spectral_refine_triangulation() requires a robust circumcenter computation.
...
private:
    ...
    Visitor visitor)
  {
    ...
    CGAL_TRACE_STREAM << "Creates Spectral triangulation...\n";
    ...
    // Prints status
    CGAL_TRACE_STREAM << "Creates Spectral triangulation: " << task_timer.time() << " seconds, "
                                                           << std::endl;
  }
...
public:
  ...
  /*! 
    Creates a Spectral implicit function from the  range of points `[first, beyond)`. 
  ...
  */ 
                                 ...
                                 double average_spacing_ratio = 5) 
  {
    ...
    internal::Spectral::Constant_sizing_field<Triangulation> sizing_field(CGAL::square(cell_radius_bound));
    ...
    if(approximation_ratio > 0. && 
      ...
      // triangulation, is based on the result of a spectral function with a
      ...
      //
      //   - the minimum of the square of 'coarse_spectral_function' and the
      ...
      typedef Filter_iterator<typename Triangulation::Input_point_iterator,
                              Spectral_skip_vertices> Some_points_iterator;
      //make it deterministic
      Random random(0);
      Spectral_skip_vertices skip(1.-approximation_ratio,random);
      ...
      Spectral_reconstruction_function<Geom_traits> 
        coarse_spectral_function(Some_points_iterator(m_tr->input_points_end(),
                                                     skip,
      ...
      Special_wrapper_of_two_functions_keep_pointers<
        internal::Spectral::Constant_sizing_field<Triangulation>,
        Spectral_reconstruction_function<Geom_traits> > sizing_field2(&min_sizing_field,
                                                                     &coarse_spectral_function);
...
private:
                                   ...
                                   Second_sizing_field second_sizing_field)
  {
    ...
    unsigned int nb_vertices_added = spectral_refine_triangulation(*m_tr,radius_edge_ratio_bound,sizing_field,second_sizing_field,max_vertices,elarged_bsphere);
...
#endif // CGAL_SPECTRAL_RECONSTRUCTION_FUNCTION_H
