...
public:
    ...
    Point_2_rep(const Curved_kernel_via_analysis_2* ckva, 
                const Coordinate_2& xy) : 
        _m_ckva(ckva),
        _m_xy(xy), 
        _m_location(CGAL::ARR_INTERIOR) {
    }

    //! constructs a point at +/-oo in x
    Point_2_rep(const Curved_kernel_via_analysis_2* ckva, 
                CGAL::Arr_curve_end inf_end, const Curve_analysis_2& c,
                int arcno) :
        _m_ckva(ckva),
        _m_curve(c), 
        _m_arcno(arcno) {
...
public:
     ...
     */
    Point_2(const Curved_kernel_via_analysis_2* ckva, 
            const Coordinate_1& x, const Curve_analysis_2& c, int arcno) :
        Base(Rep(ckva, Coordinate_2(x, c, arcno))) {
    }
    ...
    Point_2(const Curved_kernel_via_analysis_2* ckva, 
            const T& x, const T& y) :
        Base(Rep(ckva,
                 ckva->kernel().construct_algebraic_real_2_object()(x,y)))
    {
    }
