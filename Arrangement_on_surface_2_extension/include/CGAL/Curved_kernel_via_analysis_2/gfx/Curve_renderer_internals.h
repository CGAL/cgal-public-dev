...
//
// $URL: svn+ssh://asm@scm.gforge.inria.fr/svn/cgal/branches/features/Symbolic-mpi/Arrangement_on_surface_2/include/CGAL/Curved_kernel_via_analysis_2/gfx/Curve_renderer_internals.h $
// $Id: Curve_renderer_internals.h 59002 2010-10-04 11:00:27Z lrineau $
//
...
{
    Clip_point_() { }
    Clip_point_(AlgebraicReal alpha_, int arcno_=-1) :
        alpha(alpha_), arcno(arcno_) { } 
    
    AlgebraicReal alpha;   // this clip point coordinate
 ...
 */
template <class AlgebraicKernel_1, class Coeff_>
...
public:
    ...
    typedef typename AK_1::Polynomial_1 Poly_dst_1;
    typedef CGAL::Polynomial< Poly_dst_1 > Polynomial_2;

    //! exact integer number type
    typedef typename AK_1::Coefficient Coefficient;

    typedef typename CGAL::Get_arithmetic_kernel< Coefficient> ::
...
public:
    ...
    Polynomial_2 poly_in, poly_in_swapped;
...
//! substitutes the inner-most variable with \c num/denom
//! \c in_degree - degree in this variable 
Poly_dst_1 subst_homogeneous_x(const Polynomial_2& poly, Coefficient num, 
        Coefficient denom, int in_degree) {

    typename Polynomial_2::const_iterator ci2;
    typename Poly_dst_1::Vector v(poly.degree() + 1);

    int i;
    for(ci2 = poly.begin(), i = 0; ci2 != poly.end(); ci2++, i++) {
    
        const Poly_dst_1& pp = *ci2;
        typename Poly_dst_1::const_iterator ci = pp.end() - 1;
    ...
    return Poly_dst_1(v.begin(), v.end());
}
...
int evaluate_rational(int var, const NT& c, const NT& key)
{
    ...
    Rational res = substitute_xy(*rat, key_r, c_r);
    return CGAL_SGN(res);
}
...
void precompute(const Polynomial_2& in) {
    ...
    poly_in = in;
    poly_in_swapped = typename Polynomial_traits_2::Swap()(in, 0, 1);

    typedef Convert_and_normalize< Polynomial_2, Arithmetic_kernel > C_and_n;
    Rational factor(1);
    *rational_y  = C_and_n()(in, factor, true);
