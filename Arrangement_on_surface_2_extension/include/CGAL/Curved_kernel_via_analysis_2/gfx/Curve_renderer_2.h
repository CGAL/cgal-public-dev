...
public: 
    ...
    Curve_renderer_2() : 
      cache_id(-1), IA_method(0), initialized(false), one(1) {
        arcno = -1;
        setup(Bbox_2(-1.0, -1.0, 1.0, 1.0), 640, 480);
    }

  // TODO Curve_renderer_2(Ckva)
...
void draw(const Arc_2& arc, 
    ...
    typename Curve_kernel_2::Algebraic_kernel_d_1::Approximate_relative_1
        approximate_x = arc.ckva()->kernel().approximate_relative_1_object();
...
//         while(1) {
//             if(lbound_x(x0) >= engine.x_max_r) {
//                 x_outside_window = true;
//                 lower = ubound_x(x0);
//                 break;
//             }
//             Rational bnd = ubound_x(x0);
//             if(bnd < engine.x_min_r) 
//                 break;
//             else if(bnd - lbound_x(x0) <= ref_bound) {
//                 lower = bnd, clip_src = false;
//                 break;
//             }
//             refine_x(x0);      
//         }
...
//         while(1) {
//             if(ubound_x(x0) <= engine.x_min_r) {
//                 x_outside_window = true;
//                 upper = lbound_x(x0);
//                 break;
//             }
//             Rational bnd = lbound_x(x0);
//             if(bnd > engine.x_max_r) 
//                 break;
//             else if(ubound_x(x0) - bnd <= ref_bound) {
//                 upper = bnd, clip_tgt = false;
//                 break; 
//             }
//             refine_x(x0);      
//         }
...
bool draw(const Point_2& pt, Coord_2& coord) {
    ...
    typename Curve_kernel_2::Algebraic_kernel_d_1::Approximate_relative_1
        approximate_x = pt.ckva()->kernel().approximate_relative_1_object();
    Bounds bnd = approximate_x(x0, 0); // do not approximate
    while(bnd.second - bnd.first > ref_bound) {
        bnd = approximate_x(x0, 1);
    }
        ...
        if(r > x_lower && l < x_upper) {
            ...
            if(it->arcno == -1) {
                ...
                for(i = 0; i < n_arcs; i++) {
                    ...
                    Bounds _ = 
                      xy.kernel()->approximate_absolute_y_2_object()(xy, -2);
//                     low = lbound_y(xy);
//                     high = ubound_y(xy);
