...
//
// $URL: svn+ssh://asm@scm.gforge.inria.fr/svn/cgal/branches/features/Symbolic-mpi/Arrangement_on_surface_2/include/CGAL/Curved_kernel_via_analysis_2/gfx/Curve_renderer_2.h $
// $Id: Curve_renderer_2.h 59002 2010-10-04 11:00:27Z lrineau $
//
...
extern CGAL::Timer tm_refine, tm_clip_pts;
#endif
...
#define CGAL_REFINE_X        1000   // was 1000 previously

// refine factor for intervals in y-direction (in pixel size) 
#define CGAL_REFINE_Y        100000 // was 100000

// refine factor for clip-points 
#define CGAL_REFINE_CLIP_POINTS  1000 // was 1000
 ...
 */
//!@{
template <class LiteCurveAnalysis_2, class Coeff_>
...
public: 
    ...
    typedef typename Lite_curve_analysis_2::Arc_2 Arc_2;
...
private:    
    ...
    typedef internal::Curve_renderer_internals< AK_1, Coeff>
        Renderer_internals;

    //! exact integer number type
    typedef typename Renderer_internals::Coefficient Coefficient;
    ...
    typename AK_1::Algebraic_real_traits::Refine refine;
    ...
    typedef internal::Clip_point_< Coordinate_1 > Clip_point_entry;
...
private:
    ...
    void refine_coord_1(const Coordinate_1& x, const Rational& bound,
            Rational& low, Rational& up) {

        Rational lb, ub;
        while(1) {
            lb = lower_bnd(x), ub = upper_bnd(x);   
            if(ub - lb < bound)
                break;
            refine(x);
        } 
...
private:
    ...
    Pixel_2 isolated_l, isolated_r; //! isolating rectangles for lower  and
                                    //! upper end-points
                                    
    //! an instance of rendering engine
    internal::Curve_renderer_internals< AK_1, Coeff > engine;
...
template < class Coord_2, template < class, class > class Container,
        class Allocator >
void draw_isolate(const Arc_2& arc, 
    ...
    Rational y_at_rational_x(const Rational& x, const Rational& bound) {
...
void draw(const Arc_2& arc, 
    ...
    Rational lower = engine.x_min_r, upper = engine.x_max_r, y_lower = 0,
             y_upper = 0;
    Coordinate_1 x_low_coord, x_up_coord;
    ...
    Rational refine_bnd_x = engine.pixel_w_r / CGAL_REFINE_X,
             refine_bnd_y = engine.pixel_h_r / CGAL_REFINE_Y;
    if(loc_p1 != CGAL::ARR_LEFT_BOUNDARY) {

        Gfx_OUT("refining left point\n");
        x_low_coord = arc.curve_end_x(CGAL::ARR_MIN_END);
        typename AK_1::Approximate_relative_1 approximate_x;
        typedef typename AK_1::Approximate_relative_1::result_type Bounds;

        Bounds bnd = approximate_x(x_low_coord, 0); 
        while(1) {
            ...
            if(bnd.second < engine.x_min_r)
                break;
            else if(bnd.second - bnd.first <= refine_bnd_x) {
                lower = bnd.second, clip_src = false;
                break;
            }
            bnd = approximate_x(x_low_coord, 1);
        }
    }   
    ...
    if(loc_p2 != CGAL::ARR_RIGHT_BOUNDARY) {
        ...
        x_up_coord = arc.curve_end_x(CGAL::ARR_MAX_END);

        typename AK_1::Approximate_relative_1 approximate_x;
        typedef typename AK_1::Approximate_relative_1::result_type Bounds;

        Bounds bnd = approximate_x(x_up_coord, 0); // do not approximate
        while(1) {
            if(bnd.second <= engine.x_min_r) {
                x_outside_window = true;
                upper = bnd.first;
            ...
            bnd = approximate_x(x_up_coord, 1);
        }
    } 
    ...
    arcno = (arc.is_vertical() ? -1u : arc.arcno());

    // these flags indicate if left or right end-points are interior points
    // w.r.t. the current drawing box
    bool left_pt_interior = false, right_pt_interior = false;
    if(!x_outside_window) {
        ...
        if(clip_src) {
            y_lower = y_at_rational_x(lower, refine_bnd_y);
            x_low_coord = Coordinate_1(engine.x_min_r);

        } else if(loc_p1 == CGAL::ARR_BOTTOM_BOUNDARY) {
            y_lower = (arc.is_vertical() ? engine.y_min_r :
                engine.y_min_r - height_r);
        } else if(loc_p1 == CGAL::ARR_INTERIOR) {
            left_pt_interior = true;
            y_lower = engine.y_min_r + height_r * Rational(1000);
        } else {  // endpoints must be sorted lexicographical
            CGAL_precondition(!arc.is_vertical());
            y_lower = engine.y_max_r + height_r;
        }
        if(clip_tgt) {
            y_upper = y_at_rational_x(upper, refine_bnd_y);
            x_up_coord = Coordinate_1(engine.x_max_r);

        } else if(loc_p2 == CGAL::ARR_BOTTOM_BOUNDARY) {
            CGAL_precondition(!arc.is_vertical());
            y_upper = engine.y_min_r - height_r;
    
        } else if(loc_p2 == CGAL::ARR_INTERIOR) {
            right_pt_interior = true;
            y_upper = engine.y_min_r + height_r * Rational(1000);            
        } else {
    ...
    if(!clip_pts_computed) {
        ...
        horizontal_clip(engine.y_max_r, top_clip, top_poly);
        horizontal_clip(engine.y_min_r, btm_clip, btm_poly);
    ...
    segment_clip_points(lower, upper, engine.y_min_r, btm_poly, arc.arcno(),
         btm_clip, btm_idx);
        
    Gfx_OUT("checking top clip: y = " << engine.y_max << "\n");
    segment_clip_points(lower, upper, engine.y_max_r, top_poly, arc.arcno(),
         top_clip, top_idx);
    ...
    while(1) { 
        ...
        if(it1 != btm_idx.end()) {
            if(it2 != top_idx.end())
                push_1 = ((btm_clip)[*it1].alpha < (top_clip)[*it2].alpha);
...
/** **************************************************************************/
    isolated_l.level = -1u; // this indicates that real end-points are
    isolated_r.level = -1u; // not available yet
/** **************************************************************************/
    if(left_pt_interior) {
        pix_1.y = 1000000;
        isolated_l = pix_1;
    ...
    if(seg_pts.size() == 0) {
        mid = x_low_coord.rational_between(x_up_coord);
        if(!y_inside_at_rational_x(mid, engine.y_min_r, engine.y_max_r)) {
            Gfx_OUT("segment is outside: " << bfi(x_low_coord) << " - " <<
                 bfi(mid) << " - " << bfi(x_up_coord) << "\n");
            return;
        }
    ...
    /// the window, we can get into the troubles..
        if(pix_2.x - pix_1.x <= 1) { // it goes away right here
            ...
            Rational x = mid, y = y_at_rational_x(x, refine_bnd_y);
            get_pixel_coords(x, y, pix_mid);

            if(left_pt_interior) {
                x = x_low_coord.rational_between(Coordinate_1(mid));
                y = y_at_rational_x(x, refine_bnd_y);
                get_pixel_coords(x, y, pix_1);
            }    
            if(right_pt_interior) {
                x = x_up_coord.rational_between(Coordinate_1(mid));
                y = y_at_rational_x(x, refine_bnd_y);
                get_pixel_coords(x, y, pix_2);
            }
                
            CGAL_CKVA_STORE_COORDS(rev_points, pix_1);
            CGAL_CKVA_STORE_COORDS(rev_points, pix_mid);
        ...
        // if arc is almost vertical - start from a middle point
        if(pt > mid) 
            pt = mid; 
    ...
    if(seg_pts.front().second == 0) {
        alpha = btm_clip[seg_pts.front().first].alpha;
    } else {
        alpha = top_clip[seg_pts.front().first].alpha;
    }
    l = x_low_coord.rational_between(alpha);
    
    if(seg_pts.back().second == 0) {
        alpha = btm_clip[seg_pts.back().first].alpha;
    } else {
        alpha = top_clip[seg_pts.back().first].alpha;
    }
    r = x_up_coord.rational_between(alpha);

    Gfx_OUT("l = " << bfi(l) << "; r = " << bfi(r) << "\n");
    bool pt1_inside = y_inside_at_rational_x(l, engine.y_min_r,
                 engine.y_max_r),
         pt2_inside = y_inside_at_rational_x(r, engine.y_min_r,
                 engine.y_max_r);
        
    Gfx_OUT("segment is not completely inside the window: " <<
            pt1_inside << " and " << pt2_inside << "\n");
    ...
    // save these values
    int lvl1 = isolated_l.level, lvl2 = isolated_r.level;
    ...
    while(it != eend) {
        ...
        l = lower_bnd(pclip->alpha), r = upper_bnd(pclip->alpha);
        ...
        isolated_l.level = lvl1;
        isolated_r.level = lvl2;
        ...
        if(it == seg_pts.begin() && pt1_inside) { 
            ...
            mid = (lower + l)/2;
            if(pt > mid)
                pt = mid; // for almost straight arcs
...
//                 refine_alg_point(l, r, *ppoly, lower, 1);
            ...
            //! mark that right end-point is a clip-pt
            //! this is required to handle asymptotes correctly
            isolated_r.level = -1u; 
...
//                 refine_alg_point(l, r, *ppoly, upper, 1);
            ...
            pt = pclip->alpha.rational_between(Coordinate_1(pt));
            Rational tmp_left = lower_bnd(ptmp->alpha);

            get_pixel_coords(l, y_clip, pix_beg);  
            get_pixel_coords(tmp_left, it->second ? engine.y_max_r :
                engine.y_min_r, pix_end); 
            if(CGAL_ABS(tmp_left - l) <= engine.pixel_w_r*2) {
                
                mid = y_at_rational_x(pt, engine.pixel_h_r/CGAL_REFINE_Y);
                CGAL_CKVA_STORE_COORDS(rev_points, pix_beg);
                get_pixel_coords(pt, mid, pix_beg); 
                straight_segment = true;
            } 
        
            isolated_l.level = -1u;  // both are clip-points
            isolated_r.level = -1u; 
        }
...
bool draw(const Point_2& pt, Coord_2& coord) {
    ...
    typename AK_1::Approximate_relative_1
      approximate_x;
...
void draw_lump(std::vector< Coord_2 >& rev_points, int& last_x,
    ...
    while(1) {
        ...
        if(pix.x < 0 || pix.x > engine.res_w || pix.y < -CGAL_WINDOW_ENLARGE||
            ...
            branches_coincide = false;
            Gfx_OUT("breaking the loop on line " << __LINE__ << "\n");
            break;
        } 
...
//         }
        ...
        bool test_n = test_neighbourhood(pix, back_dir, new_dir);
        bool is_term = is_terminating_pixel(pix, direction_taken);
...
Back:
        if(!test_n) {
            ...
            if(witness == pix) { // witness subpixel is a pixel itself
                if(!subdivide(pix, back_dir, new_dir, is_term)) {
                    Gfx_OUT("### cannot sbd witness subpixel: " <<
                        branches_coincide << " " << is_term << "\n");
                    if(branches_coincide && is_term)
                        branches_coincide = false;
                    is_exit = true;
                    break;
                }
                ...
                Gfx_OUT("### advancing witness pix itself: " <<
                    pix << "\n");
            } else {
                back_dir = stored_dir;
                pix = witness;
...
//                 }
                ...
                is_term = is_terminating_pixel(pix, direction_taken);
...
//                         stored_dir = -1;
//                         failed = true;
//                         continue; // restore the subdivision level back
//                     }
                    if(!subdivide(pix, back_dir, new_dir, is_term)) {
                        is_exit = true;
                        break;
                ...
                if(is_term && check_terminating_pixel(pix, direction_taken, 
                         need_subdiv)) {
                    branches_coincide = false;
                    is_exit = true;
                    break;
                ...
                if(need_subdiv) {
                    Gfx_OUT("Subdivision required at: " << pix <<
                         std::endl);
                    if(!subdivide(pix, back_dir, new_dir, true)) {
                        is_exit = true;
                        break;
                    }
                }
                ...
                advance_pixel(pix, new_dir);
/** ************************************************************************/
                Gfx_OUT(pix << " NEW dir = " << new_dir << std::endl);
/** ************************************************************************/
            ...
            if(need_subdiv) {
                Gfx_OUT("Subdivision required at: " << pix <<
                         std::endl);
                // set flag that test_neighbourhood failed
                test_n = false; goto Back;
            }
            advance_pixel(pix, new_dir);
            witness = pix;
            Gfx_OUT("next pix: " << pix << "\n");
        }
        back_dir = (new_dir+4)&7;
    }
    ...
    if(branches_coincide) { // oops, need another seed point
        ...
        Gfx_OUT("\nLooking for the next seed point\n"); 
...
Lexit:  
        // take only clip-points but not isolated pts
        if(direction_taken == 0 && isolated_l.level == -1u) {
            pix = pix_1;
            Gfx_OUT("saving pix_1: " << pix_1 << "\n");
        } else if(direction_taken == 1 && isolated_r.level == -1u) {
            pix = pix_2;
            Gfx_OUT("saving pix_2: " << pix_2 << "\n");
        }        
    ...
    if(pix.y > -2 && pix.y < engine.res_h + 2)
        CGAL_CKVA_STORE_COORDS(ppoints, pix);
...
bool subdivide(Pixel_2& pix, int back_dir, int& new_dir, 
    ...
    //Gfx_DETAILED_OUT("subpixel index: " << idx << " (" << pix.sub_x << "; "
        // << pix.sub_y << ")" << std::endl);
    if(!test_neighbourhood(pix, back_dir, new_dir))
        return subdivide(pix, back_dir, new_dir, is_terminating);
...
bool get_seed_point(const Rational& seed, Pixel_2& start, int *dir, 
    ...
    Rational x_s = seed, y_s, _;
        
    Coordinate_1 y_coord;
    isolate_at_rational_x(x_s, y_coord);
    refine_coord_1(y_coord, engine.pixel_h_r/CGAL_REFINE_Y, _, y_s);

    Gfx_OUT("get seed point: " << bfi(x_s) << "; " << 
            bfi(y_s) << "\n");
...
//         CGAL::to_double(y_seed) << ")" << std::endl);
    ...
    while(!test_pixel(start, (int *)dir, (int *)b_taken, b_coincide)) {
        ...
        if(lvl > CGAL_REFINE_Y) {
            Rational _;
            refine_coord_1(y_coord, engine.pixel_h_r/(lvl*2), _, y_s);
...
void isolate_at_rational_x(const Rational& x, Coordinate_1& alpha) {

    typedef CGAL::Fraction_traits< Rational > FT;
    typename FT::Decompose decompose;
        
    Coefficient num, denom;
    decompose(x, num, denom);

    Poly_dst_1 fiber = engine.subst_homogeneous_x(engine.poly_in, num, denom,
             engine.poly_in_swapped.degree());
...
//     solve(fiber, true, std::back_inserter(roots));
    ...
    int n_roots = isl.number_of_real_roots(); // roots.size();
    if(n_roots <= arcno) {
        std::cerr << "FATAL: # of roots less than arcno: " << n_roots <<
            " and " << arcno << "\n";
        throw -1;
    }
//     alpha = roots[arcno];
    alpha = Coordinate_1(fiber, isl.left_bound(arcno),
                isl.right_bound(arcno));
}


//! returns an y-coordinate at rational x wiht this arc's \c arcno
//! refined to the \c bound
Rational y_at_rational_x(const Rational& x, const Rational& bound) {

    Coordinate_1 r;
    isolate_at_rational_x(x, r);

    Rational u, l;
    while(1) {
        u = upper_bnd(r), l = lower_bnd(r); 
        if(u - l < bound)
            break;
        refine(r);
    }
...
//! checks if y lies between [lb; ub]
bool y_inside_at_rational_x(const Rational& x, const Rational& y_low,
            const Rational& y_up) {        
...
void horizontal_clip(const Rational& y_clip,
        Clip_points& container, Poly_dst_1& poly) {
    ...
    decompose(y_clip, num, denom);

    poly = msf(engine.subst_homogeneous_x(engine.poly_in_swapped, num, denom,
             engine.poly_in.degree()));
//     poly = msf(poly_in.evaluate_homogeneous(num, denom));

    typename AK_1::Solve_1 solve;
    std::vector< Coordinate_1 > roots;
    solve(poly, true, std::back_inserter(roots));

    int n_roots = roots.size(), i;
    Rational lb, rb, criteria = engine.pixel_w_r/CGAL_REFINE_CLIP_POINTS;
    
    for(i = 0; i < n_roots; i++) {
        const Coordinate_1& r = roots[i];
        bool pt_found = false;
        while(1) {
//             lb = DS.left_bound(i), rb = DS.right_bound(i);   
            lb = lower_bnd(r), rb = upper_bnd(r);
            if(lb > engine.x_max_r || rb < engine.x_min_r)
                break;
            if(rb - lb <= criteria) {
                pt_found = true; break;
            }
        ...
        if(pt_found) {
            container.push_back(Clip_point_entry(r));
        }
    }
}
...
void segment_clip_points(const Rational& x_lower, const Rational& x_upper,
    ...
    typedef internal::Bitstream_coefficient_kernel_at_alpha
            < AK_1 > BCK;

    AK_1 ak_1;

    typename Clip_points::iterator cit;
    for(cit = clip_points.begin(); cit != clip_points.end(); cit++, j++) {

        const Coordinate_1& alpha = cit->alpha;
        l = lower_bnd(alpha), r = upper_bnd(alpha);
...
// std::cout << "[" << bfi(alpha.lower()) << "; " << bfi(alpha.upper()) << "] " <<
//         alpha.polynomial() << "\n";

            CGAL::internal::Bitstream_descartes< BCK >
                DS(engine.poly_in, bck);

            Gfx_OUT("processing clip-point: " << j << ": " <<
                    bfi(alpha) << "\n");
            for(i = 0; i < DS.number_of_real_roots(); i++) {
                ...
                while(1) {
                    lb = DS.left_bound(i), rb = DS.right_bound(i);
                    if(lb > y_clip || rb < y_clip)
                        break;
                    if(rb - lb <= engine.pixel_h_r/CGAL_REFINE_Y) {
                        pt_found = true; break;
                    }

                    DS.refine_interval(i);
                }
                if(!pt_found)
                    continue;
                cit->arcno = i;
                Gfx_OUT("clip point assigned to arcno: " << i << "\n");
                break;
            }
        }
        if(cit->arcno == arcno) {
            Gfx_OUT("clip-point set\n");
...
void select_cache_entry(const Arc_2& arc) {
    ...
    if(new_entry) {
        engine.precompute(arc.curve().polynomial_2());
    }
}
...
inline bool is_terminating_pixel(const Pixel_2& pix, bool direction) {

    Pixel_2& stop = (direction == 0 ? isolated_l : isolated_r);
    if(stop.level == -1u) // stop pixel is not set
        return false;

    if(direction == 0) {
        // protection from overrrun
        if(pix.x < stop.x || pix.x - stop.x  <= 1) {
            return true;
        }
    }
    if(direction == 1) {
        // protection from overrrun
        if(pix.x > stop.x || stop.x - pix.x <= 1) {
            return true;
        }
    }
...
//! (stopping criteria)
inline bool check_terminating_pixel(const Pixel_2& pix, bool direction,
            bool& need_subdiv) {

// NOTE NOTE: we can refine x-intervals with greater precision
// and compare on subpixel level

    // pixel we wish to reach
    Pixel_2& stop = (direction == 0 ? isolated_l : isolated_r);
    need_subdiv = false;

    // if terminating pixel is not set or the current pixel is not yet
    // withing the x-range of the terminating pixel => quit
//     if(stop.level == -1u)
//         return false;
// 
    // protection from overrrun
    if((direction == 0 && pix.x < stop.x) ||
        (direction == 1 && pix.x > stop.x))
        return true;

    // we believe that this is a terminating pixel
    // even if one pixel before
    if(std::abs(pix.x - stop.x) == 1 && pix.level >= 6)
        return true;

    if(pix.x != stop.x) // the test is only for terminating pixel
        return false;

    Gfx_OUT("checking for terminating pixel: " << pix << "\n");

    // check if horizontal boundaries of this pixel are not cut by the curve
    NT b = NT(pix.y), t = b + NT(1);

    Poly_1 poly_X;
    engine.get_precached_poly(CGAL_X_RANGE, b, 0, poly_X);
    
    Integer lvl = one << pix.level;
    NT inv = NT(1) / NT(lvl);
    make_exact(inv);

    NT l = NT(pix.x) + NT(pix.sub_x) * inv;
    if(get_range_1(CGAL_X_RANGE, l, l + inv, b, poly_X) &&
            !engine.zero_bounds) { 
            
    } else {
        engine.get_precached_poly(CGAL_X_RANGE, t, 0, poly_X);
        if(!get_range_1(CGAL_X_RANGE, l, l + inv, t, poly_X)
                || engine.zero_bounds) {
            std::cout << " found terminating subpixel: " << pix << std::endl;
            return true;
        }
    }

//     b = NT(pix.y) + NT(pix.sub_y) * inv;
//     engine.get_precached_poly(CGAL_Y_RANGE, l, 0, poly_X);
//     // test additional boundaries nearby terminating pixel
//     need_subdiv = get_range_1(CGAL_Y_RANGE, b, b + inv, l, poly_X);
// 
//     if(!need_subdiv) {
//         engine.get_precached_poly(CGAL_Y_RANGE, l + inv, 0, poly_X);
//         need_subdiv = get_range_1(CGAL_Y_RANGE, b, b + inv, l + inv, poly_X);
//     }
