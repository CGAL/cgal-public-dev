...
int main(int argc, char const *argv[])
{
    ...
    Point segmentTarget(0.1f, 0.1f, 1.0f);
    Vector rayDirection(0.0f, 0.0f, 1.0f); /*Direction need not be normalized.*/
    Ray ray(rayOrigin, rayDirection);
    Segment segment(rayOrigin, segmentTarget);

    boost::optional<Tree::Intersection_and_primitive_id> intersection = tree.first_intersection(segment);
