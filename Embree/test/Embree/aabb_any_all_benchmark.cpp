...
std::size_t intersect(ForwardIterator b, ForwardIterator e, const Tree& tree, long& counter) {
      typedef boost::optional<typename Tree::Intersection_and_primitive_id>Obj_type;
  ...
  for(; b != e; ++b) {
    ...
    Obj_type o = tree.any_intersection(*b);
    if(o)
      ++counter;
  }
...
template<typename K>
boost::tuple<std::size_t, std::size_t, long> test(const char* name) {
  ...
  boost::tuple<std::size_t, std::size_t, long> tu;

    {
      ...
      {
        long counter = 0L;
        tu = boost::make_tuple(intersect(rays.begin(), rays.end(), tree, counter),
                               intersect(segments.begin(), segments.end(), tree, counter),
                               // cant use counter here
                               0);
        boost::get<2>(tu) = counter;
      }
      std::cout << t.time();
    }
...
int main()
{
  ...
  boost::tuple<std::size_t, std::size_t, long> t1 = test<CGAL::Simple_cartesian<float> >(filename);
  std::cout << " | " << std::endl;

  std::cout << "| Cartesian float kernel | ";
  boost::tuple<std::size_t, std::size_t, long> t2 = test<CGAL::Cartesian<float> >(filename);
  std::cout << " | " << std::endl;

  std::cout << "| Simple cartesian double kernel |";
  boost::tuple<std::size_t, std::size_t, long> t3 = test<CGAL::Simple_cartesian<double> >(filename);
  std::cout << " | " << std::endl;

  std::cout << "| Cartesian double kernel |";
  boost::tuple<std::size_t, std::size_t, long> t4 = test<CGAL::Cartesian<double> >(filename);
  std::cout << " | " << std::endl;

  std::cout << "| Epic kernel |";
  boost::tuple<std::size_t, std::size_t, long> t5 = test<CGAL::Exact_predicates_inexact_constructions_kernel>(filename);
  std::cout << " | " << std::endl;
