...
// struct for polyhedron.
template <typename TriangleMesh, typename GeomTraits>
struct Triangle_mesh_geometry_poly {
  ...
  const TriangleMesh* Polyhedron;
  RTCGeometry rtc_geometry;
  unsigned int rtc_geomID;
  Vertex_point_map vpm;
  // added a vector for faceDescriptors.
  std::vector<face_descriptor> faceDescriptors;

  Triangle_mesh_geometry_poly()
  {}
  ...
  static void bound_function(const struct RTCBoundsFunctionArguments* args)
  {
    Triangle_mesh_geometry_poly* self = (Triangle_mesh_geometry_poly*) args->geometryUserPtr;
    RTCBounds* bounds_o = args->bounds_o;
    unsigned int primID = args->primID;
  ...
  void insert_primitives()
  {
    rtcSetGeometryUserPrimitiveCount(rtc_geometry, num_faces(*Polyhedron));
    rtcSetGeometryUserData(rtc_geometry, this);
  ...
  Primitive_id primitive_id(unsigned int primID) const
  {
    return std::make_pair(face_descriptor(primID), const_cast<TriangleMesh*>(Polyhedron));
  }
};
