	...
	namespace LOD {
		...
		public:
			...
			void get_mock_basic(Container &input) const {
				...
				point = Point(-0.95172, -0.48047, 0.2); it = input.insert(point, normal); colors[*it] = blue; labels[*it] = facade; planes[*it] = plane; indices[*it] = 6;
				point = Point(-0.89196, -0.58007, 0.8); it = input.insert(point, normal); colors[*it] = blue; labels[*it] = facade; planes[*it] = plane; indices[*it] = 6;
				point = Point(-0.73891, -0.83516, 0.3); it = input.insert(point, normal); colors[*it] = blue; labels[*it] = facade; planes[*it] = plane; indices[*it] = 6;
				...
				point = Point(-0.7    , -0.9    , 0.1); it = input.insert(point, normal); colors[*it] = blue; labels[*it] = facade; planes[*it] = plane; indices[*it] = 7;
				point = Point(-0.39871, -0.72783, 0.5); it = input.insert(point, normal); colors[*it] = blue; labels[*it] = facade; planes[*it] = plane; indices[*it] = 7;
				point = Point(0.0     , -0.5    , 0.7); it = input.insert(point, normal); colors[*it] = blue; labels[*it] = facade; planes[*it] = plane; indices[*it] = 7;

				// (Facade 3) Follows the second one:
				normal = Normal(0.145876, 0.175048, -9.419e-7); plane = Plane(-154874.0, -185846.0, 1.0, -92923.7);
				
				point = Point(-0.05418, -0.45485, 0.7); it = input.insert(point, normal); colors[*it] = blue; labels[*it] = facade; planes[*it] = plane; indices[*it] = 8;
				point = Point(-0.30487, -0.24594, 0.3); it = input.insert(point, normal); colors[*it] = blue; labels[*it] = facade; planes[*it] = plane; indices[*it] = 8;
				point = Point(-0.4918 , -0.09016, 0.7); it = input.insert(point, normal); colors[*it] = blue; labels[*it] = facade; planes[*it] = plane; indices[*it] = 8;
				...
				point = Point(-0.6    , 0.0     , 0.23); it = input.insert(point, normal); colors[*it] = blue; labels[*it] = facade; planes[*it] = plane; indices[*it] = 9;
				point = Point(-0.64175, -0.29111, 0.88); it = input.insert(point, normal); colors[*it] = blue; labels[*it] = facade; planes[*it] = plane; indices[*it] = 9;
				point = Point(-1.0    , -0.4    , 0.23); it = input.insert(point, normal); colors[*it] = blue; labels[*it] = facade; planes[*it] = plane; indices[*it] = 9;

				// (Roof) This one is not horizontal :
				normal = Normal(0.053372, 0.052358, 0.120235); plane = Plane(0.443898, 0.435464, 1.0, -0.683335);
				  
				point = Point(-0.79882, -0.37218, 1.2); it = input.insert(point, normal); colors[*it] = red; labels[*it] = roof; planes[*it] = plane; indices[*it] = 10;
				point = Point(-0.53703, -0.63904, 1.2); it = input.insert(point, normal); colors[*it] = red; labels[*it] = roof; planes[*it] = plane; indices[*it] = 10;
				point = Point(-0.45596, -0.26240, 1.0); it = input.insert(point, normal); colors[*it] = red; labels[*it] = roof; planes[*it] = plane; indices[*it] = 10;
			}
