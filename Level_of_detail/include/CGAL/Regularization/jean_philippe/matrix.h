#ifndef JEAN_PHILIPPE_MATRIX_H
#define JEAN_PHILIPPE_MATRIX_H
		...
		for (uint c = 0 ; c < channels ; c++) {
			...
			for (uint i = 0; i < rows; i++) {
				...
				if (std::is_same<T, uchar>::value) {
					...
					im_band->RasterIO(GF_Read, 0, i, cols, 1, im_values[i], cols, 1, GDT_Float64, 0, 0);
				}
			}
		...
		for (uint c = 0; c < channels; c++) {
			...
			for (uint i = 0; i < rows; i++) {
				...
				if (std::is_same<T, uchar>::value) {
					...
					im_band->RasterIO(GF_Read, offset_j, offset_i + i, cols, 1, im_values[i], cols, 1, GDT_Float64, 0, 0);
				}
			}
