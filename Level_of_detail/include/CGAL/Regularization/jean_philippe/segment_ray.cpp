...
Segment::Segment(uint _index, double x1, double y1, double x2, double y2, double _width, double _log_nfa, double _grad_magnitude, double _grad_theta, bool _is_artificial)
{
    index = _index;
	...
	width = _width;
	grad_magnitude = _grad_magnitude;
	grad_theta = _grad_theta;
#endif
...
#if NOT_MEASURING_PERFORMANCES
    // No brothers for the moment
    brothers = set<Segment *>();
#endif
...
{
    dalpha = _dalpha;
    cv::Mat R = cv::Mat(2, 2, CV_64F);

    R.at<double>(0, 0) = cos(dalpha * (PI / 180)); R.at<double>(0, 1) = -sin(dalpha * (PI / 180));
    R.at<double>(1, 0) = sin(dalpha * (PI / 180)); R.at<double>(1, 1) = cos(dalpha * (PI / 180));

    finalBarycenter = interBarycenter = barycenter;
    finalEnd1 = interEnd1 = barycenter + Point2d(cv::Mat(R * cv::Mat(end1 - barycenter)));
    finalEnd2 = interEnd2 = barycenter + Point2d(cv::Mat(R * cv::Mat(end2 - barycenter)));
	...
	theta = atan2(finalDirection[1], finalDirection[0]);

    is_dalpha_set = true;
    update_line_coefficients();
}
#endif
...
#if NOT_MEASURING_PERFORMANCES
void Segment::add_brother(Segment* t)
{
    brothers.insert(t);
}


void Segment::remove_brother(Segment *t)
{
    set<Segment *>::iterator it_s = brothers.begin();
    while (it_s != brothers.end()) {
        if ((*it_s) == t) {
            brothers.erase(it_s);
            break;
        }
...
void Segment::merge_brothers(Segment *t)
{
    for (set<Segment *>::iterator it_s = t->brothers.begin() ; it_s != t->brothers.end() ; it_s++) {
        this->add_brother(*it_s);
        (*it_s)->add_brother(this);
    }
}


void Segment::clear_brothers()
{
    for (set<Segment *>::iterator it_s = brothers.begin() ; it_s != brothers.end() ; it_s++) {
        (*it_s)->remove_brother(this);
    }
    brothers.clear();
}


bool Segment::is_brother(Segment* t)
{
    for (set<Segment *>::iterator it_s = brothers.begin() ; it_s != brothers.end() ; it_s++) {
        if ((*it_s) == t) return true;
    }
    return false;
}
#endif
...
void Segment::draw_segments(Matrix<uchar> & I, Matrix<uchar> & J, std::vector<Segment *> &T)
{
    ...
    /*double min_log_nfa = FLT_MAX;
    double max_log_nfa = -FLT_MAX;
    for (int i = 0 ; i < int(T.size()) ; i++) {
        if (T[i]->log_nfa < min_log_nfa) min_log_nfa = T[i]->log_nfa;
        if (T[i]->log_nfa > max_log_nfa) max_log_nfa = T[i]->log_nfa;
    }*/
    //std::cout << "Log NFA : [" << min_log_nfa << ", " << max_log_nfa << "]" << std::endl;
    //std::cout << "NFA : [" << pow(10, -max_log_nfa) << ", " << pow(10, -min_log_nfa) << "]" << std::endl;

    for (int i = 0; i < int(T.size()); i++) {
        uchar rgb[3] = { 255, 0, 0 };

        /*double nfa = pow(10, -T[i]->log_nfa);
        cv::Vec3b color = ColorMap::blue_white_red(1 - nfa);
        uchar rgb[3] = { color[0], color[1], color[2] };*/
...
Ray::Ray(uint _index, Segment* _parent, Point2d & _O, Point2d & _A, Vec2d & _OA, double _incidence_angle, uint _ttl)
{
    index = _index;
    parent = _parent;
    parent->add_ray(this);
...
void Ray::build_rays(std::vector<Segment*> & segments, std::vector<Ray*> & rays, unsigned int ttl)
{
    clear_rays(rays);
    uint index = 0;
    ...
    for (unsigned int i = 0; i < segments.size() ; i++) {
        ...
        if (!s_i->is_disabled) {
			...
			if (same_orientation) {
				r_OA = new Ray(index + 0, s_i, O, A, minusu, (s_i->theta - 180) * PI / 180, ttl);
				r_OB = new Ray(index + 1, s_i, O, B, u, s_i->theta * PI / 180, ttl);
			} else {
				r_OA = new Ray(index + 0, s_i, O, A, u, s_i->theta * PI / 180, ttl);
				r_OB = new Ray(index + 1, s_i, O, B, minusu, (s_i->theta - 180) * PI / 180, ttl);
			}
            rays.push_back(r_OA);
            rays.push_back(r_OB);
            index += 2;
