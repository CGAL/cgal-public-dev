#ifndef CGAL_LEVEL_OF_DETAIL_LOD_0_BUILDER_H
#define CGAL_LEVEL_OF_DETAIL_LOD_0_BUILDER_H
...
namespace CGAL {
	...
	namespace Level_of_detail {
        ...
        class WallFaces,
        class RoofFaces>
		class Lod_0_builder : public Modifier_base<InputHDS> {
		...
		public:
            ...
            Lod_0_builder(const Buildings &buildings, const Ground &ground, Ground_face &ground_face, Wall_faces &wall_faces, Roof_faces &roof_faces) :
        ...
        private:
			...
			inline size_t estimate_number_of_faces() {
				return m_buildings.size() * 2 + 1;
			}

			void add_new_building(const Building &building, Builder &builder) {

				const Floor_faces &floor_faces = building.floor_faces();
				add_roof_faces(floor_faces, builder);
			}

            void add_roof_faces(const Floor_faces &floor_faces, Builder &builder) {
                ...
                for (Const_floor_faces_iterator triangle = floor_faces.begin(); triangle != floor_faces.end(); ++triangle, ++i) {
                    ...
                    const Point_3 a = Point_3(p1.x(), p1.y(), base_z + m_z_offset);
                    const Point_3 b = Point_3(p2.x(), p2.y(), base_z + m_z_offset);
                    const Point_3 c = Point_3(p3.x(), p3.y(), base_z + m_z_offset);
...
#endif // CGAL_LEVEL_OF_DETAIL_LOD_0_BUILDER_H