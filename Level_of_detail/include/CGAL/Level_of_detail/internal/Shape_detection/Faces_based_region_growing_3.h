#ifndef CGAL_LEVEL_OF_DETAIL_FACES_BASED_REGION_GROWING_3_H
#define CGAL_LEVEL_OF_DETAIL_FACES_BASED_REGION_GROWING_3_H
...
namespace CGAL {
	...
	namespace Level_of_detail {
		...
		template<class InputKernel, class InputMesh, class InputFaces>
		class Faces_based_region_growing_3 {

		public:
			using Kernel = InputKernel;
			using Mesh   = InputMesh;
			using Faces  = InputFaces;
			...
			Faces_based_region_growing_3(const Faces &building_faces) : 
			...
			void find_regions(Regions &regions) {
				CGAL_precondition(!m_building_faces.empty());
			...
			int get_number_of_regions() const {
				...
				CGAL_precondition(m_number_of_regions >= 0);
		...
		private:
			...
			void grow_regions(Regions &regions) {
				...
				CGAL_precondition(regions.size() == number_of_buildings);
			...
			void grow_building_regions(const size_t building_index, Building_regions &building_regions) {
				...
				CGAL_precondition(building_index >= 0 && building_index < m_building_faces.size());
...
#endif // CGAL_LEVEL_OF_DETAIL_FACES_BASED_REGION_GROWING_3_H