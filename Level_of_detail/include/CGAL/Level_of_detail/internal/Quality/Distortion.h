...
namespace CGAL {
	...
	namespace Level_of_detail {
		...
		public:
			...
			Distortion(const Container &input, const Reconstruction &reconstruction) : 
			m_input(input), 
			m_reconstruction(reconstruction),
			m_distortion(-FT(1)), 
			m_num_roofs_points(-FT(1)), 
			m_num_walls_points(-FT(1))
			{ }
		...
		private:
			...
			FT m_distortion;
			FT m_num_roofs_points;
			FT m_num_walls_points;
			...
			void compute_updated_mesh_points_l2_distances_to_roofs(const Mesh &mesh, const std::vector<Point_3> &points) {
				...
				CGAL_precondition(m_num_roofs_points > FT(0));
				CGAL_precondition(points.size() <= static_cast<size_t>(CGAL::to_double(m_num_roofs_points)));
			...
			void compute_mesh_points_l2_distances_to_walls(const Mesh &mesh, const std::vector<Point_3> &points) {
				...
				CGAL_precondition(m_num_walls_points > FT(0));
				CGAL_precondition(points.size() <= static_cast<size_t>(CGAL::to_double(m_num_walls_points)));
				...
				CGAL_precondition(m_walls_metrics.size() == points.size());
			...
			FT fit_data(const Quality_data_type type) const {
				...
				switch (type) {
					...
					default:
						CGAL_precondition(!"Wrong fitting type!");
						return -FT(1);
				}
			}
