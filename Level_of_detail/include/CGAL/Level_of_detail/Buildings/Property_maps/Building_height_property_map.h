#ifndef CGAL_LEVEL_OF_DETAIL_BUILDING_HEIGHT_PROPERTY_MAP_H
#define CGAL_LEVEL_OF_DETAIL_BUILDING_HEIGHT_PROPERTY_MAP_H
...
#include <CGAL/Level_of_detail/Buildings/Tools/Height_fitters/Average_height_fitter.h>
#include <CGAL/Level_of_detail/Buildings/Tools/Height_fitters/Fixed_10_meters_height_fitter.h>
...
namespace CGAL {
	...
	namespace Level_of_detail {
		...
		template<class InputKernel, class InputTriangulation, class InputBuilding>
		class Building_height_property_map {
        ...
        public:
            ...
            using Roof_points          = std::list<Point_3>;
            ...
            template<class Point_map>
            Building_height_property_map(
            ...
            using key_type = Building;
            using Self     = Building_height_property_map<Kernel, Triangulation, Building>;

            friend void put(const Self &self, key_type &building) {
                building.local_ground_height() = self.get_local_ground_height();

                CGAL_precondition(building.index() >= 0);
                building.height() = self.get_building_height(building.index());
        ...
        private:
            ...
            template<class Point_map>
            void collect_building_roof_points(const Point_map &point_map) {
            ...
            void fit_roofs() {
                ...
                for (Building_roof_points_iterator bp_it = m_building_roof_points.begin(); bp_it != m_building_roof_points.end(); ++bp_it) {
...
#endif // CGAL_LEVEL_OF_DETAIL_BUILDING_HEIGHT_PROPERTY_MAP_H