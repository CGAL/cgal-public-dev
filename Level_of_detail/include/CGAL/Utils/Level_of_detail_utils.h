...
namespace CGAL {
	...
	namespace LOD {
		...
		public:
			...
			void fit_building_roofs(const Input &input, Buildings &buildings) const {
				...
				for (typename Buildings::iterator bit = buildings.begin(); bit != buildings.end(); ++bit) {
					...
					building.roofs.resize(shapes.size());

					for (size_t i = 0; i < shapes.size(); ++i) {
						const auto &indices = shapes[i];

						std::vector<Point_3ft> tmp_points(indices.size());
						for (size_t j = 0; j < indices.size(); ++j) {
					
							const Point_3 &p = input.point(indices[j]);

							const double x = CGAL::to_double(p.x());
							const double y = CGAL::to_double(p.y());
							const double z = CGAL::to_double(p.z());

							tmp_points[j] = Point_3ft(x, y, z);
						}

						Plane_3ft tmp_plane;
						CGAL::linear_least_squares_fitting_3(tmp_points.begin(), tmp_points.end(), tmp_plane, CGAL::Dimension_tag<0>());
						final_plane = Plane_3(static_cast<FT>(tmp_plane.a()), static_cast<FT>(tmp_plane.b()), static_cast<FT>(tmp_plane.c()), static_cast<FT>(tmp_plane.d()));

						Vector_3 m;
						set_plane_normal(final_plane, m);

						Vector_3 n;
						set_ground_normal(n);

						FT rad_angle; Vector_3 axis;
						compute_angle_and_axis(m, n, rad_angle, axis);
						...
						for (size_t j = 0; j < indices.size(); ++j) {
							...
							const Point_3 &p = input.point(indices[j]);
							Point_3 projected = final_plane.projection(p);

							rotate_point(rad_angle, axis, projected);
						...
						for (size_t j = 0; j < indices.size(); ++j) {
							...
							const Vector_3 m2 = Vector_3(Point_3(FT(0), FT(0), 0), Point_3(bx, by, 0));
							const Vector_3 n2 = Vector_3(Point_3(FT(0), FT(0), 0), Point_3(FT(0), FT(1), 0));
							
							FT rad_angle_2; Vector_3 axis_2;
							compute_angle_and_axis(m2, n2, rad_angle_2, axis_2);
							...
							projected = Point_3(x, y, projected.z());

							x = projected.x() * cos(rad_angle_2) - projected.y() * sin(rad_angle_2);
							y = projected.y() * cos(rad_angle_2) + projected.x() * sin(rad_angle_2);
						...
						Point_3 p1 = Point_3(minx, miny, z);
						Point_3 p2 = Point_3(maxx, miny, z);
						Point_3 p3 = Point_3(maxx, maxy, z);
						Point_3 p4 = Point_3(minx, maxy, z);

						rotate_point(-rad_angle, axis, p1);
						rotate_point(-rad_angle, axis, p2);
						rotate_point(-rad_angle, axis, p3);
						rotate_point(-rad_angle, axis, p4);

						building.roofs[i].boundary.resize(4);

						building.roofs[i].boundary[0] = p1;
						building.roofs[i].boundary[1] = p2;
						building.roofs[i].boundary[2] = p3;
						building.roofs[i].boundary[3] = p4;
					}
				}

				Log log;
				log.export_points(debug, "tmp" + std::string(PSR) + "debug_points");
			}
			...
			void compute_angle_and_axis(const Vector_3 &m, const Vector_3 &n, FT &rad_angle, Vector_3 &axis) const {
				...
				rad_angle = static_cast<FT>(std::atan2(CGAL::to_double(length), CGAL::to_double(dot)));
				axis = cross / length;
			}

			void rotate_point(const FT rad_angle, const Vector_3 &axis, Point_3 &e) const {
				...
				e = Point_3((x * x * C + c)     * e.x() + (x * y * C - z * s) * e.y() + (x * z * C + y * s) * e.z(),
