...
namespace CGAL {
	...
	namespace LOD {
        ...
        public:
            ...
            using Angles          = map<int, FT>;
            using Angles_iterator = typename Angles::const_iterator;

            Level_of_detail_segment_regularizer_tree(const Regular_segments &segments, const Orientations &orientations, const QP_problem_data &qp_data, const Parameters &parameters) 
            : m_segments(segments), m_orientations(orientations), m_qp_data(qp_data), m_parameters(parameters), m_tolerance(FT(1) / FT(1000000)) { 
        ...
        private:
            ...
            void assign_to_parallel_node(const FT angle, const Regular_segment &segment) {

                if (m_parallel_segments.find(angle) != m_parallel_segments.end())
                    m_parallel_segments[angle].add(segment);
            }
		};
	}
}
