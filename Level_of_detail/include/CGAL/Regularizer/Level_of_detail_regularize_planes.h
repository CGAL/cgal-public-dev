#ifndef CGAL_LEVEL_OF_DETAIL_REGULARIZE_PLANES_H
#define CGAL_LEVEL_OF_DETAIL_REGULARIZE_PLANES_H
...
template <typename Traits>
typename Traits::Vector_3 regularize_normal(const typename Traits::Vector_3 &n,
...
template <typename Traits>
typename Traits::Vector_3 regularize_normals_from_prior(const typename Traits::Vector_3 &np,
			...
			do
			{
				...
				for (std::size_t k = 0; k < index_container_former_ring_parallel.size(); ++k)
				{
					...
					for (std::size_t l = 0; l < parallel_planes[plane_index].size(); ++l)
					{
						...
						if (is_available[it] && std::fabs(normal_it * clu.normal) > 1. - tolerance_cosangle)
						{
							propagation = true;
								 ...
								 typename Traits::FT tolerance_cosangle_ortho)
{
	...
	for (std::size_t i = 0; i < clusters.size(); ++i)
	{
		if (list_cluster_index[i] == static_cast<std::size_t>(-1))
		{
			...
			{
				if (list_cluster_index[j] == static_cast<std::size_t>(-1) && std::fabs(clusters[j].cosangle_symmetry -
										   ...
										   const typename Traits::Vector_3 &symmetry_direction)
{
	...
	for (std::size_t i = 0; i < subgraph_clusters_prop.size(); ++i)
	{
		...
		Vector vec_current = regularize_normal<Traits>(clusters[index_current].normal,
													   symmetry_direction,
			...
			for (std::size_t k = 0; k < index_container_former_ring.size(); k++)
			{
				...
				for (std::size_t j = 0; j < clusters[cluster_index].orthogonal_clusters.size(); ++j)
				{
					...
					if (clusters[cluster_index_2].is_free)
					{
						...
						Vector new_vect = regularize_normals_from_prior<Traits>(clusters[cluster_index].normal,
					   ...
					   typename Kernel::Vector_3 symmetry_direction = typename Kernel::Vector_3(0., 0., 1.))
{
    ...
    */
	...
	internal::PlaneRegularization::compute_parallel_clusters<Kernel>(planes, plane_map, clusters, areas,
	...
	if (regularize_axis_symmetry)
	{
		...
		// list_cluster_index
		internal::PlaneRegularization::cluster_symmetric_cosangles<Kernel>(clusters, tolerance_cosangle, tolerance_cosangle_ortho);
	}
	...
	if (regularize_orthogonality || regularize_axis_symmetry)
		internal::PlaneRegularization::subgraph_mutually_orthogonal_clusters<Kernel>(clusters, (regularize_axis_symmetry ? symmetry_direction : CGAL::NULL_VECTOR));
	...
	if (regularize_coplanarity)
	{
		...
		for (std::size_t i = 0; i < clusters.size(); ++i)
		{
			...
			for (std::size_t j = 0; j < clusters[i].planes.size(); ++j)
			{
				...
				if (get(plane_map, *(planes.begin() + index_prim)).orthogonal_vector() * plane_reg.orthogonal_vector() < 0)
...
void regularize_planes(const PointRange &points,
					   ...
					   double tolerance_angle = 25.0,
					   double tolerance_coplanarity = 0.01,
					   typename Kernel_traits<typename boost::property_traits<PointMap>::value_type>::Kernel::Vector_3 symmetry_direction = typename Kernel_traits<typename boost::property_traits<PointMap>::value_type>::Kernel::Vector_3(0., 0., 1.))
{
...
#endif // CGAL_LEVEL_OF_DETAIL_REGULARIZE_PLANES_H