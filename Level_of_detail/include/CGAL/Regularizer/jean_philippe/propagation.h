...
struct Propagation
{
    ...
    void schedule_events_at_boundaries(Kinetic_Model *model, int rows, int cols, vector<double> & maximal_sizes, IndexedEvent** events_at_boundaries);

	void schedule_events_between_colinear_segments(Kinetic_Model *model, vector<IndexedEvent *> & events_colinear);

    //void schedule_events_between_colinear_segments(Kinetic_Model *model, list<pair<int, int> > & colinear_indices, vector<IndexedEvent *> & events_colinear);
    ...
    void schedule_events(Kinetic_Model *model, IndexedEvent** schedule, IndexedEvent** intersectants, IndexedEvent** intersected, IndexedEvent** events_at_boundaries, vector<double> & maximal_sizes, vector<IndexedEvent *> & events_colinear, double t_0, double t_1);
	...
	void remove_indexed_event(IndexedEvent** schedule, IndexedEvent* event_at_boundary, IndexedEvent** events_at_boundaries, double t_0, double t_1, bool destroy);

	void remove_references_to_intersectant_ray(IndexedEvent* upcoming_event, int r_index, IndexedEvent** schedule, IndexedEvent** intersectants, IndexedEvent** intersected, IndexedEvent** events_at_boundaries, double t_0, double t_1);

    void prune_events_and_build_edges(Kinetic_Model *model, Size2i & size, IndexedEvent** first_schedule, IndexedEvent** simplified_schedule, IndexedEvent** last_event, IndexedEvent** intersectants, IndexedEvent** intersected, IndexedEvent** events_at_boundaries, 
    ...
    IndexedEvent* pop_event(Kinetic_Model* model, Size2i & size, IndexedEvent** schedule, IndexedEvent** intersectants, IndexedEvent** intersected, IndexedEvent** events_at_boundaries,
