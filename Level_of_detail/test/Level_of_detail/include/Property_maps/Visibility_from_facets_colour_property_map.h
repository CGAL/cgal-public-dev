	...
	namespace Level_of_detail {
		...
		public:
			...
			Colour generate_visibility_colour(const Facet &facet) const {
				...
				const Visibility_label visibility_label = visibility(facet);
      ...
      Visibility_label visibility (const Partition_element<Kernel, Container>& facet) const
      {
        return facet.visibility_label();
      }
      
      template <typename FaceHandle>
      Visibility_label visibility (const FaceHandle& facet) const
      {
        return facet.info().visibility_label();
      }
		};
