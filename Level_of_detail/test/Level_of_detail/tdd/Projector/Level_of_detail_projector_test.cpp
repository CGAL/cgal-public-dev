...
public:
	...
	using Output		 = std::map<int, Point>;
	...
	void create_test_input(Container &input, Planes &indices, Plane &ground) const {
		...
		Iterator 
		it = input.insert(Point(0.2, 0.3, 0.22));
		it = input.insert(Point(0.4, 0.6, 0.22));
		it = input.insert(Point(0.8, 0.1, 0.22));

		it = input.insert(Point(1.00, 0.2, 0.9));
		it = input.insert(Point(1.05, 0.5, 0.3));
		it = input.insert(Point(1.00, 0.8, 0.9));
...
TEST_F(LOD_ProjectorTest, MakesZCoordinateZero) {
	...
	ASSERT_THAT(output[indices[0][0]], Eq(Point(0.2, 0.3, 0.0)));
	ASSERT_THAT(output[indices[1][0]], Eq(Point(1.0, 0.2, 0.0)));
}
