...
template <class SearchTraits, 
          ...
          class Query_type = typename internal::K_neighbor_search<SearchTraits,Distance,Splitter,Tree>::Query_item>
...
//Partial specialization for spheres
template <class SearchTraits, 
          class Distance,
          class Splitter,
          class Tree>
class Orthogonal_k_neighbor_search<SearchTraits, Distance, Splitter, Tree, typename SearchTraits::Sphere_d >
  : public internal::K_neighbor_search<SearchTraits,Distance,Splitter,Tree> {
  typedef  internal::K_neighbor_search<SearchTraits,Distance,Splitter,Tree> Base;
...
public:
  ...
  Orthogonal_k_neighbor_search(const Tree& tree, const typename SearchTraits::Sphere_d& q,  
                               unsigned int k=1, FT Eps=FT(0.0), bool Search_nearest=true, const Distance& d=Distance(),bool sorted=true)
    : Base(typename SearchTraits::Construct_center_d()(q),k,Eps,Search_nearest,d)
  {
    if (tree.empty()) return;
    FT squared_radius = typename SearchTraits::Compute_squared_radius_d()(q);
    ...
    FT distance_to_root;
    if (this->search_nearest){ 
      distance_to_root = this->distance_instance.min_distance_to_rectangle(this->query_object, tree.bounding_box(),dists);
      compute_nearest_neighbors_orthogonally(tree.root(), distance_to_root,dists);
    }
    else {
      distance_to_root = this->distance_instance.max_distance_to_rectangle(this->query_object, tree.bounding_box(),dists);
      compute_furthest_neighbors_orthogonally(tree.root(), distance_to_root,dists);
    }
    ...
    this->queue.substract_radius(squared_radius);
    if (sorted) this->queue.sort();
  }
private:
...
}; // End of partial specialization for spheres

//Partial specialization for iso rectangles
template <class SearchTraits, 
          class Distance,
          class Splitter,
          class Tree>
class Orthogonal_k_neighbor_search<SearchTraits, Distance, Splitter, Tree, typename SearchTraits::Iso_box_d >
  : public internal::K_neighbor_search<SearchTraits,Distance,Splitter,Tree> {
  typedef  internal::K_neighbor_search<SearchTraits,Distance,Splitter,Tree> Base;

  typename SearchTraits::Cartesian_const_iterator_d qmax_it;
  typename SearchTraits::Cartesian_const_iterator_d qmin_it;
    
public:
  typedef typename Base::FT FT;

  Orthogonal_k_neighbor_search(const Tree& tree, const typename SearchTraits::Iso_box_d& q,  
                               unsigned int k=1, FT Eps=FT(0.0), bool Search_nearest=true, const Distance& d=Distance(),bool sorted=true)
    : Base(q,k,Eps,Search_nearest,d)
  {
    if (tree.empty()) return;
    ...
    qmax_it = construct_it(construct_max_vertex(q));
    qmin_it = construct_it(construct_min_vertex(q));

    int dim = static_cast<int>(std::distance(qmax_it, construct_it(construct_max_vertex(q),0)));
    std::vector<FT> dists(dim);
    for(int i=0;i<dim;i++)
        dists[i]=0;
