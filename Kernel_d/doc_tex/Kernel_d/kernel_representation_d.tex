...
\subsection{Lifting kernel}

Some applications require computations with lifted points. The last
coordinate of a \(d\)-dimensional point can be seen as lifting coordinate.
There exist applications, such as the computation of many regular
triangulations, where computations on lifted points are repeated several
times, after changing the lifting coordinates~\cite{EFKP12}. The repeated
computations on this scenario are predicates that involve determinants.

The \ccc{Lifting_kernel<Kernel_d>} takes advantage of this situation and
stores computed determinants. It inherits all member functions and
geometric objects from the parameterizing \ccc{Kernel_d}, but it reimplements
some predicates and add some functions to the interface for changing
lifting.
...
representation class, i.e., \ccc{CGAL/Cartesian_d.h},
\ccc{CGAL/Homogeneous_d.h} or \ccc{CGAL/Lifting_kernel_d.h}
