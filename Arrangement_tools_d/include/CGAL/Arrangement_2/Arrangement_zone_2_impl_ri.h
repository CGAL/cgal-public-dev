...
class Arr_face_split_observer : public CGAL::Arr_observer< Arrangement_ > {
public:
    ...
    //! reset to no face has split
    void reset() const {
        _m_face_split = false;
    }

    //! is \c true if a face has split since attaching
    bool face_split() const {
        //return false;
        return _m_face_split;
    }
...
private:
    ...
    //! indicates a face splitting since attachment
    mutable bool _m_face_split;
};
...
init_with_hint(const X_monotone_curve_2& _cv, const Object& _obj)
{
  ...
  if (m_geom_traits->is_bounded_2_object()(cv, ARR_MIN_END))
  {
  ...
  if (m_geom_traits->is_bounded_2_object()(cv, ARR_MAX_END))
  {
     ...
     X_monotone_curve_2 xcv)
{
  ...
  {
    // In case there is just a single incident halfedge around v,
    // we should insert xcv right after this halfedge.
    he = he_curr;

    // Note that xcv extends to the right of v. In case the single
    // halfedge also extends to the right of v (its source is to
    // the right), check if an overlap occurs.
    if (he->direction() == ARR_RIGHT_TO_LEFT &&
        (m_geom_traits->compare_y_at_x_right_2_object() (he->curve(), xcv,
                                                         v->point()) == EQUAL))
    {
      // Mark that an overlap occurs:
      //intersect_he = he_curr;
  ...
  // Find the face containing xcv around the vertex.
  typename Traits_adaptor_2::Is_between_cw_2  is_between_cw =
    m_geom_traits->is_between_cw_2_object();

  do
  {
    // Check if it is possible to insert xcv in between the current curve
    // and the next curve, going in a clockwise direction around v.
    is_between = is_between_cw (xcv, true,
                                he_curr->curve(), 
                                (he_curr->direction() == ARR_RIGHT_TO_LEFT),  
                                he_next->curve(),
                                (he_next->direction() == ARR_RIGHT_TO_LEFT),
    ...
    if (is_between)
    {
      // We can conclude that xcv should be placed between he_curr and
      // he_next (in a clockwise order), and no overlap occurs.
      he = he_curr;
      return (false);
    }
  ...
  if (iter != inter_map.end())
  {
    ...
    while (! inter_list.empty())
    {
      ...
      else
      {
        ...
        if (m_geom_traits->is_bounded_2_object()(*icv, ARR_MIN_END))
        {
...
_zone_in_face (Face_handle face, bool on_boundary)
{
  ...
  typedef Arr_face_split_observer< Arrangement_2 > Observer;
  Observer observer;
  observer.reset();
  observer.attach(arr);
...
#if !NDEBUG
  if (observer.face_split()) {
      //std::cout << "OBSERVER DETECTED FACE-SPLIT" << std::endl;
  }
#endif
  ...
  if (inserted_he != invalid_he)
  {
    ...
    if (right_he != invalid_he)
    {
        if (observer.face_split()) {
            CGAL_assertion_code(
                    bool overlap = 
                    _find_prev_around_vertex (left_v, left_he, cv);
            );
            CGAL_assertion(!overlap);
  ...
  {
    CGAL_assertion(!observer.face_split());
...
{
  // Check if the right end of overlap_cv is bounded. If so, compute
  // its right endpoint.
  const bool           cv_has_right_pt =
    m_geom_traits->is_bounded_2_object() (overlap_cv, ARR_MAX_END);
