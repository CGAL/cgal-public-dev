...
//    for (li = edges_to_resolve.begin(); li != edges_to_resolve.end(); ++li)
    unsigned int n_edges_to_resolve = edges_to_resolve.size();
    for(unsigned int li = 0; li < n_edges_to_resolve; ++li)
    {
      #ifdef CGAL_BENCH_ENVELOPE_DAC
        resolve_edge_timer.start();
      #endif
      //resolver->resolve(*li, result);
      resolver->resolve(edges_to_resolve.front(), result);
      edges_to_resolve.pop_front();
      #ifdef CGAL_BENCH_ENVELOPE_DAC
...
//        CGAL_assertion(trg_has_equal_1 == has_equal_aux_data(1, trg, h_face));
  ...
  void update_edge_face_flags(Halfedge_handle h)
  {
    ...
    if (decision == FIRST)
      ...
      {
        // both flags should be equal
        CGAL_assertion(has_equal_first == has_equal_second);
