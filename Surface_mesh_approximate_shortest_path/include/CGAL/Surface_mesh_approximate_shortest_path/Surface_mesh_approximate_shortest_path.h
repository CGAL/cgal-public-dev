 ...
 *
 * \details It consists of the 'Kernel::FT' objects 'sigma' and 'd', respectively representing
 * the geodesic distance from the source to the current virtual source and the overall current distance
 * from the source to the face target point (the face's barycenter unless otherwise specified, \sa 'add_target()).
 * The struct further contains a 'std::array<FT, 3>' storing the distance of the face's vertices
 * to the current virtual source.
 */
...
struct Face_values
{
  ...
  Face_values(const FT _sigma = 0.,
              const FT _d = std::numeric_limits<FT>::max(),
              const std::array<FT,3>& _d2verts = {-1., -1., -1.})
...
/*!
 * \brief The Unfolded_triangle_2 class is a struct containing two 'Point_2' objects,
 * i.e. the tangent space points B and P as in the paper.
 */
template<class Kernel>
...
public:
  ...
  {
    std::vector<int> cumulative_update_counts = get_cumulative_update_counts_after_iterations(m_iter_counter - 1); // @todo try that without the -1, too
 ...
 *
 * \brief Compute approximate shortest path distances between one or multiple source points and no,
 * one or multiple target points on a surface mesh.
 *
 * \details This package provides an implementation of the paper "Geodesic Distance Computation
 * via Virtual Source Propagation" by Trettner, Bommes and Kobbelt (2021).
 * It offers a simplified version of the algorithm behind the Surface_mesh_shortest_path package.
 * Instead of geodesic windows, we only propagte virtual sources. This leads to inexact reconstruction
 * of the previous geodesic information and hence only gives us an approximate geodesic distance.
 * In return, this impler algorithm is faster.
 *
...
template <typename Traits,
          typename Visibility_heuristic = typename Traits::Visibility_heuristic,
          typename Skip_condition = Surface_mesh_approximate_shortest_path_3::Never_skip_condition,
          typename Enqueue_policy = Surface_mesh_approximate_shortest_path_3::Static_speed_limiter<typename  Traits::Kernel>>
...
public:
  /*!
    * \brief Surface_mesh_approximate_shortest_path constructor that allows for custom contruction
    * of the 'Visibility_heuristic' and the 'Skip_condition' and 'Enqueue_policy'.
    */
  Surface_mesh_approximate_shortest_path(Surface_mesh& mesh,
    ...
    : m_mesh(mesh),
      m_A(), m_B(),
      m_sources_status(CGAL::NO_SOURCES_SET),
      m_targets_status(CGAL::NO_TARGETS_SET),
      m_propagation_status(CGAL::INVALID_RESULTS),
      m_Visibility_heuristic(VisibilityHeuristic),
      m_Skip_condition(SkipCondition),
      m_Enqueue_policy(EnqueuePolicy),
      m_build_shortest_path_tree(build_shortest_path_tree),
      m_with_observer(with_observer)
  {
  ...
  /*!
    * \brief Surface_mesh_approximate_shortest_path constructor that allows for custom contruction
    * of the 'Skip_condition' and 'Enqueue_policy'.
    *
  ...
  /*!
    * \brief Surface_mesh_approximate_shortest_path default constructor. The 'Visibility_condition'
    * and the 'Skip_condition' and 'Enqueue_policy' are default constructed.
    */
  Surface_mesh_approximate_shortest_path(Surface_mesh& mesh,
                                         bool build_shortest_path_tree = false,
                                         bool with_observer = false)
...
public:
  ...
  void init_propagation_property_maps()
  {
    ...
    std::tie(m_edge_lengths, created_edge_property_map) = m_mesh.template add_property_map<edge_descriptor, FT>();
    CGAL_postcondition(created_edge_property_map);

    std::tie(m_face_values, created_face_property_map) = m_mesh.template add_property_map<face_descriptor, Face_values>();
    CGAL_postcondition(created_face_property_map);
  }

  void init_shortest_path_tree()
  {
    bool created_shortest_path_tree;
    std::tie(m_shortest_path_tree, created_shortest_path_tree) = m_mesh.template add_property_map<face_descriptor, face_descriptor>("geodesic_path_tree");
    CGAL_postcondition(created_shortest_path_tree);
  }
  ...
  public:
    ...
    FT operator() (const Surface_mesh& tmesh, const halfedge_descriptor h)
    {
      const vertex_descriptor v1 = source(h, tmesh);
      const vertex_descriptor v2 = target(h, tmesh);
      return operator() (tmesh, v1, v2);
    }

    FT operator() (const Surface_mesh& tmesh, const vertex_descriptor v1, const vertex_descriptor v2)
    {
      const Point_3& v1_point = tmesh.point(v1);
      const Point_3& v2_point = tmesh.point(v2);
      FT length = squared_distance(v1_point, v2_point);
      return length;
    }
  };

  /*!
    * \brief This function looks up the squared length of an edge in the internal 'edge_property_map'
    * and only computes it if is has not been computed before.
    */
  ...
  public:
    ...
    result_type operator() (Surface_mesh& mesh,
                            const halfedge_descriptor h,
                            Edge_property_map& edge_lengths)
    {
      edge_descriptor e = edge(h, mesh);
  ...
  */
  ...
  public:
    ...
    result_type operator() (Surface_mesh& tmesh, halfedge_descriptor h)
    {
      ...
      // third edge length
      FT e2 = Compute_squared_edge_length()(tmesh, tmesh.target(nexth), tmesh.source(h));
  ...
  public:
                            ...
                            Face_values_map& face_values)
    {
      ...
      // there should never be negative numbers unless d2A == Sx and numerical errors hit
      if(-1e-10 < d2ASx && d2ASx < 0.)
      {
        //d2ASx = 0.;
        Sy = 0;
      }
  ...
  /*!
    ...
    */
  Point_2 construct_target_point(halfedge_descriptor opposite_halfedge,
                                 Face_location target_face_location,
                                 const Point_2& B,
                                 const Point_2& P)
  {
  ...
  /*!
    ...
    */
  void add_source(const int vertex_index)
  {
  ...
  /*!
    ...
    */
  void add_source(const vertex_descriptor vd)
  {
  ...
  /*!
    ...
    */
  void add_source(const Point_3& source_point)
  {
  ...
  {
    CGAL_precondition(i < m_source_face_locations.size());
    ...
    if(m_source_face_locations.empty())
      m_sources_status = CGAL::NO_SOURCES_SET;
  ...
  bool is_source_face(face_descriptor face)
  {
    ...
    {
      if(face == source_face_location.first)
        return true;
    }
  ...
  /*!
    ...
    */
  void add_target(const int vertex_index)
  {
    vertex_descriptor vd(vertex_index);
    add_target(vd);
  }
  ...
  /*!
    ...
    */
  void add_target(const vertex_descriptor vd)
  {
  ...
  /*!
    ...
    * \details Note that if the specfied point is not on the mesh, it will be projected onto the mesh.
    */
  void add_target(const Point_3& target_point)
  {
    // locate source_point on mesh
  ...
  /*!
    ...
    */
  void add_target(const Face_location& target_point)
  {
  ...
  /*!
    ...
    */
  void remove_target(const int i)
  {
    CGAL_precondition(i < m_target_face_locations.size());
    ...
    if(m_target_face_locations.empty())
      m_targets_status = CGAL::NO_TARGETS_SET;
  ...
  void print_target_information()
  {
    ...
    {
      const Face_location& target = m_target_face_locations[i];
      const Point_3& physical_target = m_target_physical_locations[i];
      std::cout << "Target with index " << std::to_string(i)
                ...
                << " and barycentric coordinates [" << std::to_string(target.second[0]) << ", "
                                                    << std::to_string(target.second[1]) << ", "
                                                    << std::to_string(target.second[2]) << "]\n"
                << "\t (Physical location: [" << std::to_string(physical_target[0]) << ", "
                                              << std::to_string(physical_target.second[1]) << ", "
                                              << std::to_string(physical_target.second[2]) << "])"
                << std::endl;
    }
  }
  ...
  /*!
    ...
    */
  std::pair<bool, int> belongs_to_target_face(const halfedge_descriptor h)
  {
  ...
  /*!
    * \brief Function to get the local indices of the vertices in a triangle face. Needed
    * for a consistent enumeration of vertices between adjacent cells.
    * \return 'std::array<int, 3>' of local vertex indices, i.e. permutations of {0, 1, 2}
    */
  std::array<int, 3> get_local_vertex_indices(const halfedge_descriptor h)
  {
    face_descriptor face = m_mesh.face(h);
    ...
    std::array<int, 3> local_vertex_indices = { local_idx_A_in_face,
                                                local_idx_B_in_face,
                                                local_idx_P_in_face };
  ...
  /*!
    ...
    */
  void set_squared_vertex_distances_intersection(const halfedge_descriptor oppo_h,
                                                 const Point_2& P_coords,
                                                 const Point_2& S_coords)
  {
    face_descriptor f = face(oppo_h, m_mesh);
    halfedge_descriptor h = opposite(oppo_h, m_mesh);
    face_descriptor prev_f = face(h, m_mesh);

    Face_values prev_face_values = m_face_values[prev_f];
    std::array<FT, 3> prev_face_vertex_distances = prev_face_values.d2verts;

    vertex_descriptor A = target(oppo_h, m_mesh);
    int local_idx_A_in_face = vertex_index_in_face(A, f, m_mesh);
    int local_idx_A_in_prev_face = vertex_index_in_face(A, prev_f, m_mesh);
    m_face_values[f].d2verts[local_idx_A_in_face] = prev_face_vertex_distances[local_idx_A_in_prev_face];

    vertex_descriptor B = source(oppo_h, m_mesh);
    int local_idx_B_in_face = vertex_index_in_face(B, f, m_mesh);
    int local_idx_B_in_prev_face = vertex_index_in_face(B, prev_f, m_mesh);
    m_face_values[f].d2verts[local_idx_B_in_face] = prev_face_vertex_distances[local_idx_B_in_prev_face];

    vertex_descriptor P = target(next(oppo_h, m_mesh), m_mesh);
    int local_idx_P_in_face = vertex_index_in_face(P, f, m_mesh);
    m_face_values[f].d2verts[local_idx_P_in_face] = squared_distance(P_coords, S_coords);
  }

  /*!
    * \brief Function to set the squared distances of the source to the vertices
    * of an 'Unfolded_triangle' in the case of a left turn.
    */
  void set_squared_vertex_distances_left_turn(halfedge_descriptor h)
  {
    face_descriptor f = face(h, m_mesh);
    std::array<int, 3> local_vertex_indices = get_local_vertex_indices(h);

    m_face_values[f].d2verts[local_vertex_indices[0]] = 0.;
    m_face_values[f].d2verts[local_vertex_indices[1]] = m_edge_lengths[edge(h, m_mesh)];
    m_face_values[f].d2verts[local_vertex_indices[2]] = m_edge_lengths[edge(prev(h, m_mesh), m_mesh)];
  }

  /*!
    * \brief Function to set the squared distances of the source to the vertices
    * of an 'Unfolded_triangle' in the case of a right turn.
    */
  void set_squared_vertex_distances_right_turn(halfedge_descriptor h)
  {
    ...
    m_face_values[f].d2verts[local_vertex_indices[0]] = m_edge_lengths[edge(h, m_mesh)];
    m_face_values[f].d2verts[local_vertex_indices[1]] = 0.;
    m_face_values[f].d2verts[local_vertex_indices[2]] = m_edge_lengths[edge(next(h, m_mesh), m_mesh)];
  }
  ...
  void set_squared_vertex_distances(IntersectionResult intersection,
                                    ...
                                    Point_2 S)
  {
    halfedge_descriptor oppo_h = opposite(h, m_mesh);
    face_descriptor next_face = face(oppo_h, m_mesh);
  ...
  /*!
    * \brief Function to update the face values based on the 'IntersectionResult'
    * and the unfolded geometry.
    * \return 'UpdateResult'
    */
                             ...
                             Point_3& overall_target_point)
  {
    ...
    EnqueueResult enqueue_result = m_Enqueue_policy(new_geodesic_dist,
  ...
  void propagate_over_halfedge(halfedge_descriptor halfedge, int iter)
  {
    ...
    if(m_mesh.is_border(opposite_halfedge))
      return;
    ...
    if(update_result.first == UPDATE)
    {
      ...
      if(m_build_shortest_path_tree)
        set_shortest_path_tree_entry(halfedge);

      int face_idx = m_mesh.face(opposite_halfedge);
      if(m_with_observer)
        m_observer.increment(face_idx);
    }
  }

  /*!
    * \brief Run the propagation algorithm. This function builds the 'Face_values_map'
    * and hence the geodesic distances.
    */
  void propagate_geodesic_source()
  {
    if(m_propagation_status == CGAL::VALID_RESULTS)
    {
      // the results are still valid from an earlier computation (neither sources nor targets were changed)
      return;
    }
  ...
  /*!
    * \brief Get the geodesic distances to all face target points (the barycenter
    * unless specified otherwise).
    * \details All entries corresponding to faces that have not been visited by the propagation
    * algorithm are set to -1.
    */
  ...
  {
    CGAL_precondition(m_propagation_status == CGAL::VALID_RESULTS);
  ...
  {
    CGAL_precondition("Shortest path tree was not build during propagation." && m_with_observer);
  ...
  /*!
    * \brief extracts a 'std::vector<bool>' indicating whether the face with a given index belonds
    * to the geodesic of the specified target.
    */
  ...
  /*!
    * \brief extracts a 'std::vector<bool>' indicating whether the face with a given index belonds
    * to the geodesic starting at a given face target.
    */
  ...
  std::vector<int> get_update_counts_per_iteration(int iteration)
  {
    CGAL_precondition(m_with_observer);
