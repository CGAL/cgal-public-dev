...
//
// $URL: svn+ssh://ybrise@scm.gforge.inria.fr/svn/cgal/trunk/QP_solver/include/CGAL/QP_solver/QP_solver_impl.h $
...
solution_numerator( ) const
{
  ...
  if (is_phaseII) z += et2 * ET(qp_c0) * denominator_ * denominator_;
  return z;
}
...
pivot_step( )
{
  ...
  << ", iteration " << m_pivots << ", INV ]" << std::endl;
...
// direction == +1 => x_O_v_i[index_entering] == (LOWER v ZERO)
// direction == -1 => x_O_v_i[index_entering] == (UPPER v ZERO) 
                             ...
                             int& i_min, ET& d_min, ET& q_min)
{
  if (q_k > et0) {                                // check for lower bound
    if (*(qp_fl+k)) {
      ET  diff = x_k - (denominator_ * ET(*(qp_l+k)));
      // BLAND rule: In case the ratios are the same, only update if the new index
      // is smaller. The special artificial variable is always made to leave first.
    ...
    if ((q_k < et0) && (*(qp_fu+k))) {
      ET  diff = (denominator_ * ET(*(qp_u+k))) - x_k;
      // BLAND rule: In case the ratios are the same, only update if the new index
      // is smaller. The special artificial variable is always made to leave first.
      if ((diff * q_min < -(d_min * q_k)) || ((k < i_min) && (i_min != art_s_i) && (diff * q_min == -(d_min * q_k))) ) {
        i_min = k;
...
replace_variable_original_original( )
{
  ...
  minus_c_B[ k] = 
  ( is_phaseI ? 
   ( index_entering < qp_n ? et0 : -aux_c[index_entering-qp_n-slack_A.size()]) : -ET( *(qp_c+ index_entering)));
...
remove_artificial_variable_and_constraint( )
{
  ...
  int old_row = art_A[index_leaving - qp_n - slack_A.size()].first;
...
enter_and_leave_variable( )
{
  ...
  if (ratio_test_bound_index == LOWER) {
    diff = x_j - ET(*(qp_l+index_entering));
  } else {
    diff = x_j - ET(*(qp_u+index_entering));
  }
...
enter_variable( )
{
  ...
  if (no_ineq || (index_entering < qp_n)) {              // original variable
    ...
    minus_c_B[B_O.size()] = -ET(*(qp_c+ index_entering)); // Note: B_O has always the
    // correct size.
    
    in_B[index_entering] = B_O.size();
    B_O.push_back(index_entering);
...
leave_variable( )
{
  ...
  if ( no_ineq || ( index_leaving < qp_n)) {                      // original variable
    ...
    in_B  [ index_leaving] = -1;     // i gets deleted
    ...
    A_Cj[ C.size()] = ( index_entering < qp_n ? ET( *((*(qp_A + index_entering))+ new_row)) : et0);
...
z_replace_variable_original_by_original( )
{
  ...
  minus_c_B[ k] = -ET( *(qp_c+ index_entering));
  ...
  // compute s_delta
  D_pairwise_accessor  d_accessor( qp_D, index_entering);
...
z_replace_variable_slack_by_original( )
{
  ...
  minus_c_B[ B_O.size()] = -ET( *(qp_c+ index_entering));
  ...
  // prepare kappa
  ET  kappa = denominator_ * ET(*((*(qp_A + index_entering))+new_row)) 
...
update_r_C_r_S_B__j(ET& x_j)
{
  ...
  A_by_index_accessor     a_accessor_j(*(qp_A + index_entering));
...
update_r_C_r_S_B__j_i(ET& x_j, ET& x_i)
{
  // update of vector r_{C}
  A_by_index_accessor     a_accessor_j(*(qp_A+index_entering));
  A_by_index_accessor     a_accessor_i(*(qp_A+index_leaving));
...
update_w_r_B_O__j(ET& x_j)
{
  ...
  if (!check_tag(Is_linear())) {
    D_pairwise_accessor     d_accessor_j(qp_D, index_entering);
...
update_w_r_B_O__j_i(ET& x_j, ET& x_i)
{
  ...
  // Note: we only do anything it we are dealing with a QP.
  if (!check_tag(Is_linear())) {
    D_pairwise_accessor     d_accessor_i(qp_D, index_leaving);
    D_pairwise_accessor     d_accessor_j(qp_D, index_entering);
...
check_basis_inverse( Tag_true)
{
  ...
  for ( col = 0; col < cols; ++col, ++i_it) {
    ratio_test_init__A_Cj( tmp_l.begin(), *i_it,
                          no_ineq);
    inv_M_B.solve_x( tmp_l.begin(), q_x_O.begin());
    ...
    if (q_k * ET(direction) > et0) {            // check for lower bound
      if (B_O[k] < qp_n) {                         // original variable
        if (*(qp_fl+B_O[k])) {                   // finite lower bound
          vout2.out() << "t_O_" << k << ": "
          << x_k - (denominator_ * ET(*(qp_l+B_O[k]))) << '/' << q_k
          << ( ( q_i != et0) && ( index_leaving == B_O[ k]) ? " *" : "")
          << std::endl;
      ...
      if (B_O[k] < qp_n) {                         // original variable
        if (*(qp_fu+B_O[k])) {                   // finite upper bound
          vout2.out() << "t_O_" << k << ": "
          << (denominator_ * ET(*(qp_l+B_O[k]))) - x_k << '/' << q_k
          << ( ( q_i != et0) && ( index_leaving == B_O[ k]) ? " *" : "")
          << std::endl;                    
