...
//  - Compute genus, if equals 0 give an empty return.
//  - Compute a shortest-path tree T rooted at an arbitrary vertex (with unit weight for each edge).
//  - On the dual graph of G\T compute a cotree T' (spanning tree using BFS). 
//  - Set X = edges that do not belong in T or T'.
//  - Compute for each edge the associated homology vector.
//     - Edges in T : 0.
//     - Edges in X : e_i (unit vector).
//     - Edges in T' : computed by a postorder traversal and a XOR with the vector of the darts on the same face.
//  - Compute the candidates list C.
//     - For each vertex y on the mesh : 
//       - Compute a shortest-path tree (with weight for each edge computed using the weight functor).
//       - For each edge e not in the tree, compute its length and homology vector (using XOR on the vector of the edges of the associated path).
//       - Keep in memory the length, the vector, the root vertex y and the generating edge e.
//  - Sort C, add iteratively the smallest element of C in the basis while checking independance (with Gauss elimination).
//  - For each candidate in the basis, compute the associated path generated by the edge using a shortest-path tree rooting from the vertex.
