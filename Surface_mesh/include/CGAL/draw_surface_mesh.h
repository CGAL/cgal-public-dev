...
#include <CGAL/Qt/Basic_viewer_three.h>
...
namespace CGAL
{
  ...
  public:
    ...
    SimpleSurfaceMeshViewerQt(QWidget *parent,
                              ...
                              bool anofaces = false,
                              const ColorFunctor &fcolor = ColorFunctor()) : // First draw: no vertex; edges, faces; mono-color; inverse normal
  ...
  //
  template <class K>
  void draw_three(const Surface_mesh<K> &amesh,
                  const char *title = "Surface_mesh Basic Viewer Three",
                  bool nofill = false)
  {
#if defined(CGAL_TEST_SUITE)
    bool cgal_test_suite = true;
