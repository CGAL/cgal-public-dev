...
template <class FT> class Myterminal_parser {
...
public:
  ...
  Myterminal_parser(const int num_parameters, const Parameters parameters,
  ...
  inline Input_parameters &get_input_parameters() { return m_lod_parameters; }
  ...
  void add_val_parameter(const std::string &parameter_name,
                         Scalar &variable_value) {

    if (!does_parameter_exist(parameter_name, m_lod_parameters))
      return;
  ...
  void add_str_parameter(const std::string &parameter_name,
  ...
  void add_bool_parameter(const std::string &parameter_name,
                          bool &variable_value) {
    ...
    variable_value = true;
    std::cout << parameter_name << " : " << (variable_value ? "true" : "false")
              << std::endl;
  }
...
private:
  ...
  bool is_asked_for_help(const int num_parameters,
  ...
  void print_help() {
    ...
    std::cout << "your terminal name $ ." << std::string(_SR_)
              << "lod -data path_to_data" << std::string(_SR_)
    ...
    std::cout << "parameter name: -data" << std::endl
              << "parameter value: path_to_data" << std::string(_SR_)
              << "data_name.ply" << std::endl
              << "description: path to the file with input data" << std::endl
              << std::endl;

    std::cout << std::endl << "OPTIONAL PARAMETERS:" << std::endl << std::endl;

    std::cout << "parameter name: -silent" << std::endl
              << "description: supress any intermediate output except for the "
                 "final result"
              << std::endl
              << std::endl;

    std::cout << "parameter name: -load_params" << std::endl
              << "parameter value: path_to" << std::string(_SR_)
              << "parameters.lod" << std::endl
              << "description: load parameters from the file" << std::endl
              << std::endl;
  }

  void set_input_parameters(const int num_parameters,
                            const Parameters parameters,
                            Input_parameters &input_parameters) {
  ...
  void set_lod_parameters(Input_parameters &input_parameters,
  ...
  bool are_required_parameters_set(const Input_parameters &input_parameters,
                                   const std::vector<std::string> &required) {
  ...
  bool is_required_parameter_set(const std::string &parameter_name,
                                 const Input_parameters &input_parameters) {

    const bool is_set =
        does_parameter_exist(parameter_name, input_parameters) &&
        !does_parameter_have_default_value(parameter_name, input_parameters);

    if (!is_set)
      std::cerr << std::endl
                << parameter_name << " parameter IS REQUIRED!" << std::endl;

    return is_set;
  }

  bool does_parameter_exist(const std::string &parameter_name,
                            const Input_parameters &input_parameters) {
  ...
  bool
  does_parameter_have_default_value(const std::string &parameter_name,
                                    const Input_parameters &input_parameters) {
  ...
  void load_parameters_from_file(Input_parameters &input_parameters) {
    ...
    if (!file) {
      ...
      std::cerr << std::endl
                << "ERROR: Error loading file with LOD parameters!" << std::endl
                << std::endl;
      exit(EXIT_FAILURE);
    }
  ...
  void save_parameters_to_file(const Input_parameters &input_parameters,
    ...
    std::cerr << std::endl
              << "ERROR: It is not possible to save parameters, because the "
                 "log path variable is not defined!"
              << std::endl
              << std::endl;
    exit(EXIT_FAILURE);
  }

  void save_input_parameters(const std::string &path,
                             const Input_parameters &input_parameters,
                             const std::vector<std::string> &exceptions) {
    ...
    std::cout << std::endl
              << "Parameters are saved in : " << file_name << std::endl
              << std::endl;
  }

  void save_parameters(const std::string &file_path,
                       const Input_parameters &input_parameters,
                       const std::vector<std::string> &exceptions) {
    ...
    if (!file) {
      std::cerr << std::endl
                << "ERROR: Error saving log file with the name " << file_path
                << std::endl
                << std::endl;
      exit(EXIT_FAILURE);
    }
  ...
  bool parameter_should_be_saved(const std::string &parameter_name,
                                 const std::vector<std::string> &exceptions) {
