#ifndef CGAL_LOD_TERMINAL_PARSER_H
#define CGAL_LOD_TERMINAL_PARSER_H
  ...
  namespace Levels_of_detail {
    ...
    private:
      ...
      void print_help() {
        ...
        std::cout << "your terminal name $ ." << std::string(_SR_) << "lod -data path_to_data" << std::string(_SR_) << "data_name.ply -other_param_name -other_param_value" << std::endl << std::endl;
      ...
      bool is_required_parameter_set(const std::string parameter_name, const Input_parameters &input_parameters) {
        ...
        if (!is_set) 
          std::cerr << std::endl << parameter_name << " parameter IS REQUIRED!" << std::endl;
      ...
      void save_parameters_to_file(const Input_parameters &input_parameters, const std::vector<std::string> &exceptions) {
        ...
        std::cerr << std::endl << "ERROR: It is not possible to save parameters, because the path variable is not defined!" << std::endl << std::endl;
        exit(EXIT_FAILURE);
      }

      void save_input_parameters(const std::string path_to_save, const Input_parameters &input_parameters, const std::vector<std::string> &exceptions) {

        const std::string file_path = path_to_save + "parameters.lod";
        save_parameters(file_path, input_parameters, exceptions);

        std::cout << std::endl << "Parameters are saved in : " << file_path << std::endl << std::endl;
      }
      ...
      void save_parameters(const std::string file_path, const Input_parameters &input_parameters, const std::vector<std::string> &exceptions) {
        ...
        if (!file) {
          ...
          std::cerr << std::endl << "ERROR: Error saving file with the name " << file_path << std::endl << std::endl;
          exit(EXIT_FAILURE);
        }
...
#endif // CGAL_LOD_TERMINAL_PARSER_H
