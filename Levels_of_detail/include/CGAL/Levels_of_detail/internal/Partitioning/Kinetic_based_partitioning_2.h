...
template<class GeomTraits, class PartitionFace>
...
public:
  ...
  void compute(const Segments& input,
               std::vector<Partition_face>& output) const {
    ...
    const FT average_spacing
      = average_spacing_estimator.compute_on_segments_2
      (segments) / m_aspacing_scale;
...
private:
  ...
  void compute_translation_and_bbox_size(const Segments &segments, Point_2 &translation, Bounding_box_size &bbox_size) const {
    ...
    Bounding_box bbox;
    internal::compute_bounding_box_2(segments, bbox);
  ...
  void create_partition_face(const Point_2 &translation, const Scale &scale, const Kinetic_face *kinetic_face, Output &output) const {
    ...
    output.push_back(Partition_face(face_vertices, CGAL::Identity_property_map<Point_2>()));
  }
};
