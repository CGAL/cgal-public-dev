...
public:
  using Self = Moebius_diagram_traits_2<Kernel_, Filter_>;
  using Base = Arr_circle_linear_traits_2<Kernel_, Filter_>;
  using Point_2 = typename Base::Point_2;
  using NT = typename Point_2::NT;
   ...
   * $d(x) = M * (x - p) - R$, where M, R are scalars.
   */
  class _Moebius_site {
  public:
  protected:
  ...
  public:
    ...
    const Kernel_point_2& point() const { return _point; }

    const FT& M() const { return _M; }

    const FT& R() const { return _R; }
    ...
    CoordNT distance(const Point_2& p) const {
      ...
      using Sqrt_ext = CGAL::Sqrt_extension<NT, NT>;
    ...
    friend std::ostream& operator<<(std::ostream& os, _Moebius_site& site) {
      os << "Point: " << site.point() << std::endl;
      os << "M: " << site.M() << "\tR: " << site.R() << std::endl;
      return os;
    }

    friend std::istream& operator>>(std::istream& is, _Moebius_site& site) {
      Kernel_point_2 p;
      FT M, R;
      is >> p >> M >> R;
  ...
  using Site_2 = _Moebius_site;

  /* construct the bisector between two points */
  class Construct_bisector_2 {
    using Make_x_monotone_2 = typename Base::Make_x_monotone_2;
  ...
  public:
    ...
    template <typename OutputIterator>
      ...
      using X_curve_list = std::list<Object>;
      Kernel ker;
  ...
  public:
    Comparison_result operator()(const Site_2& h1, const Site_2& h2,
      ...
      if (cv.orientation() == COLLINEAR) {
        Kernel ker;
        Comparison_result res = ker.compare_y_2_object()(h2.point(), h1.point());
        if (res == EQUAL)
        ...
        if (right == orient) return opposite(res);
        else return res;
      }
    }
  };
