...
namespace Envelope_voronoi_traits_2 {
   ...
   * The class represents a segment voronoi site. To make life easier in
   * computing the segment voronoi diagram we treat the end points of the
   * segment as sites on their own. This means that the class should support
   * both a segment site and a point site. The segment sites are open segment
   * actually.
   */
  ...
  public:
    ...
    using Self = Linear_object_site<NT>;
  ...
  public:
    ...
    Linear_object_site(const Ray_2& ray) : m_ray(ray), m_type(Ray_site) {}

    Linear_object_site(const Line_2& line) : m_line(line), m_type(Line_site) {}

    bool is_point() const { return m_type == Point_site; }

    bool is_segment() const { return m_type == Segment_site; }

    bool is_ray() const { return m_type == Ray_site; }

    bool is_line() const { return m_type == Line_site; }
    ...
    Line_2 supporting_line() const {
      ...
      switch (type()) {
       case Segment_site: return segment().supporting_line();
       case Ray_site: return ray().supporting_line();
       case Line_site: return line();
       default:
        CGAL_error();
        return Line_2();
      }
    ...
    Point_2 point_on(int i) const {
      ...
      switch (type()) {
       case Point_site: return point();
       case Segment_site: return segment().point(i);
       case Ray_site: return ray().point(i);
       case Line_site: return line().point(i);
       default:
        CGAL_error();
        return Point_2();
      }
    ...
    bool operator==(const Self& s) const {
      ...
      switch (type()) {
       case Point_site: return (m_point == s.m_point);
       case Segment_site: return (m_segment == s.m_segment);
       case Ray_site: return (m_ray == s.m_ray);
       case Line_site: return (m_line == s.m_line);
       default:
        CGAL_error();
        return false;
      }
  ...
  protected:
    Site_type type() const { return m_type; }
  };
}
...
<CurvedKernel_, Linear_objects_Voronoi_traits_2<CurvedKernel_>,
 ...
 CGAL::Field_tag>
{
  ...
  using Coefficient = typename Algebraic_kernel_2::Coefficient;
  using Rational = typename Algebraic_kernel_2::  Coordinate_1::Rational;
  using Get_arithmetic_kernel = CGAL::Get_arithmetic_kernel<Coefficient>;
  using Arithmetic_kernel= typename Get_arithmetic_kernel::Arithmetic_kernel;
  using Field_with_sqrt = typename Arithmetic_kernel::Field_with_sqrt;
  using Interval = boost::numeric::interval<Field_with_sqrt>;

public:
  using Site_2 = Envelope_voronoi_traits_2::Linear_object_site<Rational>;
  using Self = Linear_objects_Voronoi_traits_2<Curved_kernel_2>;
  using Base = Algebraic_apollonius_traits_base_2<Curved_kernel_2, Self, Site_2,
                                                  CGAL::Field_tag>;
...
protected:
   ...
   * we actually compute the SQUARED distance.
  ...
  static NT distance(const NT& px, const NT& py, const Site_2& site,
                     bool* out_excepted_at_endpoint = nullptr) {
    if (out_excepted_at_endpoint) *out_excepted_at_endpoint = false;
    ...
    if (! site.is_point()) {
      ...
      if (site.is_ray()) u = CGAL::max<NT>(u, 0);
      if (site.is_segment()) {
        u = CGAL::max<NT>(u, 0);
        u = CGAL::min<NT>(u, 1);
      }
...
public:
  ...
  public:
    ...
    OutputIterator operator()(Surface_3 S, bool is_lower,
      ...
      *oi++ = S;
      ...
      if (S.is_ray()) *oi++ = Xy_monotone_surface_3(S.ray().source());
      return oi;
    }
  };
  ...
  class Construct_projected_intersections_2 {
    using KPoint_2 = typename Site_2::Point_2;

  protected:
    const Self* m_traits;

  public:
    Construct_projected_intersections_2(const Self* traits) : m_traits(traits) {}
    ...
    OutputIterator operator()(const Xy_monotone_surface_3& s1,
      ...
      if (s1.is_point() == false && s2.is_point()) return operator()(s2, s1, o);
      ...
      if (s1.is_point() && s2.is_point()) {
        if (CGAL::compare_xy(s1.point(), s2.point()) == EQUAL) return o;
      ...
      if (s1.is_point() == false && s2.is_point() == false) {
        ...
        if ((line1 == line2) || (line1 == line2.opposite())) {
          ...
          if (CGAL::compare_xy(s1_point, s2_point) == SMALLER) {
            ...
            return bisector_of_collinear_point_and_linear_obj(p, s2, o);
          }
        }
        else {
      ...
      // with the line.
      CGAL_envelope_voronoi_assertion(s1.is_point() && s2.is_point() == false);

      if (s2.supporting_line().has_on(s1.point()))
        return bisector_of_collinear_point_and_linear_obj(s1.point(), s2, o);
      else {
  ...
  protected:
    ...
    */
    template <typename OutputIterator>
    OutputIterator construct_bisector_quadratic(const Rational& r,
                                                const Rational& s,
                                                const Rational& t,
                                                const Rational& u,
                                                const Rational& v,
                                                const Rational& w,
                                                OutputIterator o) const {
      using Xcurve_list = std::list<X_monotone_curve_2>;

      std::list<X_monotone_curve_2> x_conix;
      m_traits->construct_conic(r, s, t, u, v, w, std::back_inserter(x_conix));
    ...
    template <typename OutputIterator>
    OutputIterator
    bisector_of_collinear_point_and_linear_obj(const KPoint_2& point,
                                               const Site_2& site,
                                               OutputIterator o) const {
      using Xcurve_list = std::list<X_monotone_curve_2>;
      ...
      std::list<X_monotone_curve_2> x_conix;
      m_traits->
        bisector_of_collinear_point_and_linear_obj(point, site,
    ...
    OutputIterator convert_to_bisector(InputIterator begin, InputIterator end,
                                       OutputIterator o) const {
      while (begin != end)
        *o++ = make_object(std::make_pair(*begin++, Multiplicity(1)));
      return o;
    }
  };
  ...
  public:
    Construct_projected_boundary_2(const Self * traits) : m_traits(traits) {}
    ...
    */
    ...
    OutputIterator operator()(const Xy_monotone_surface_3& s,
      ...
      // the whole plane.
      if (s.is_point() || s.is_line()) return o;
      ...
      o = convert_to_boundary(x_conix.begin(), x_conix.end(), source_side, o);
  ...
  protected:
     ...
     */
    static Comparison_result compare_above(const typename Site_2::Point_2& p1,
    ...
    OutputIterator convert_to_boundary(InputIterator begin, InputIterator end,
                                       ...
                                       OutputIterator o) const {
      while (begin != end) *o++ = make_object(std::make_pair(*begin++, side));
      return o;
    }
  };
  ...
  public:
    Compare_z_at_xy_3(const Base* base) : m_base(base) {}
    ...
    Comparison_result operator()(const Point_2& p,
      ...
      try
      {
        ...
        if (res) return SMALLER;
        if (! res) return LARGER;
      }
      ...
      if (res == EQUAL && except_end1 != except_end2) {
        if (except_end1) return CGAL::LARGER;
  ...
  public:
    Comparison_result operator()(const X_monotone_curve_2& cv,
      ...
      bool except_end1, except_end2;
      Rational de1 =
        distance<Rational>(p_above.first, p_above.second, h1, &except_end1);
      Rational  de2 =
        distance<Rational>(p_above.first, p_above.second, h2, &except_end2);
      CGAL::Comparison_result res = CGAL::compare(de1, de2);
  ...
  public:
    ...
    Comparison_result operator()(const X_monotone_curve_2& cv,
      ...
      Comparison_result res =
        CGAL::opposite(m_self->compare_z_at_xy_above_3_object()(cv, h1, h2));
      ...
      if (res != EQUAL && res2 != EQUAL) assert(res == res2);
#endif
...
protected:
   ...
   */
  template <typename OutputIterator>
  OutputIterator construct_conic(const Rational& r, const Rational& s,
                                 const Rational& t, const Rational& u,
                                 const Rational& v, const Rational& w,
                                 OutputIterator o) const {
    using Polynomial_1 = CGAL::Polynomial< Coefficient >;
    using Polynomial_2 = CGAL::Polynomial< Polynomial_1 >;
    using Polynomial_traits_1 = CGAL::Polynomial_traits_d< Polynomial_1 >;
    using Polynomial_traits_2 = CGAL::Polynomial_traits_d< Polynomial_2 >;
    using Object_list = std::list<CGAL::Object>;
    using EV = CGAL::Exponent_vector;
    using Monomial = std::pair<EV, Coefficient>;
    ...
    typename Polynomial_traits_2::Construct_polynomial construct_polynomial;
    Polynomial_2 result =
      construct_polynomial(monomials.begin(), monomials.end());
    Object_list x_objs;
    Curve_2 cur = Base::instance().kernel().construct_curve_2_object()(result);
    this->make_x_monotone_2_object()(cur, std::back_inserter(x_objs));
    for (auto it = x_objs.begin(); it != x_objs.end(); ++it) {
      const X_monotone_curve_2 *xcurve;
      if ((xcurve = object_cast<X_monotone_curve_2>(&(*it))) != nullptr)
        *o++ = *xcurve;
      else CGAL_error_msg("This function supports only X_monotone_curve_2");
    }
    return o;
  }
  ...
  (const typename Site_2::Point_2& point,
   ...
   OutputIterator o) const {
    ...
    return construct_conic(0, 0, 0, line.a(), line.b(), line.c(), o);
  }
};
