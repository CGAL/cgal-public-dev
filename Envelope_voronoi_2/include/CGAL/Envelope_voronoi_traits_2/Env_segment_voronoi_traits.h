 ...
 */
template <typename Env_segment_voronoi_traits_2_>
class _Segment_voronoi_site {
 public:
  using Env_segment_voronoi_traits_2 = Env_segment_voronoi_traits_2_;
  using Self = _Segment_voronoi_site<Env_segment_voronoi_traits_2>;
  using Rat_kernel = typename Env_segment_voronoi_traits_2::Rat_kernel;
  using Alg_kernel = typename Env_segment_voronoi_traits_2::Alg_kernel;
  ...
  using Alg_segment_2 = typename Alg_kernel::Segment_2;
  using Point_2 = typename Env_segment_voronoi_traits_2::Point_2;
 ...
 public:
  ...
  const bool is_segment() const { return _is_segment; }

  const bool is_point() const { return !is_segment(); }
  ...
  const Algebraic squared_distance(const Point_2& p) const {
    if (is_segment()) return CGAL::squared_distance(p, _alg_segment);
    return CGAL::squared_distance(p, _alg_point);
  }

  bool operator== (const Self& s) const {
    if (is_segment() != s.is_segment()) return false;
    if (is_segment()) return (_rat_segment == s._rat_segment);
    return (_rat_point == s._rat_point);
  }
};

/*! \class A traits class for computing segment voronoi diagram using lower
 * envelope algorightm. The template parameters are needed because that the
 * class is based on the Arr_conic_traits_2.
 *
...
class Env_segment_voronoi_traits_2 : public CGAL::Arr_trace_traits_2<
...
public:
  ...
  using Self = Env_segment_voronoi_traits_2<Rat_kernel, Alg_kernel, Nt_traits>;
  using Multiplicity = unsigned int;
  using Rat_point_2 = typename Rat_kernel::Point_2;
  using Rat_line_2 = typename Rat_kernel::Line_2;
  using Rat_segment_2 = typename Rat_kernel::Segment_2;
  using Alg_point_2 = typename Alg_kernel::Point_2;
  using Alg_line_2 = typename Alg_kernel::Line_2;
  using Alg_segment_2 = typename Alg_kernel::Segment_2;
  ...
  using Surface_3 = _Segment_voronoi_site<Self>;
  ...
  public:
    Comparison_result operator()(const Point_2& p,
      ...
      if (h1.is_point() == h2.is_point()) return res;
      if (h1.is_point()) return SMALLER;
      else return LARGER;
    }
    ...
    Comparison_result operator()(const X_monotone_curve_2& cv,
      ...
      if (h1.is_segment() && h2.is_segment()) {
        ...
        if ((line1 == line2) || (line1 == line2.opposite())) {
          ...
          CGAL_envelope_voronoi_assertion (object_cast<Rat_segment_2>(&obj) ==
                                           nullptr);
        ...
        CGAL_envelope_voronoi_assertion
          ((CGAL::compare_xy(source1, source2) == EQUAL) ||
        ...
        Comparison_result res =
          CGAL::opposite(CGAL::orientation(cv.left(), cv.right(),
                                           segment1.source()));
        if (res == EQUAL)
          res = CGAL::opposite(CGAL::orientation(cv.left(), cv.right(),
                                                 segment1.target()));
        return res;
      }
      ...
      // two point sites.
      if (CGAL::collinear(h1.point(),
                          h2.segment().source(), h2.segment().target())) {
        // make sure that we don't take the same point in case that the point is
        // on a segment end point.
        Comparison_result res =
          CGAL::opposite(_compare_above(h1.point(), h2.segment().source()));
        if (res == EQUAL)
          res =
            CGAL::opposite(_compare_above(h1.point(), h2.segment().target()));
        return res;
      }

      // point isn't collinear with the segment. the bisector is a parabolic
      // arc.
      if (cv.is_facing_up()) return LARGER;
      else return SMALLER;
    }
  };
  ...
  public:
    ...
    OutputIterator operator()(const Xy_monotone_surface_3& s,
      ...
      using Boundary_curve = std::pair<X_monotone_curve_2, CGAL::Oriented_side>;
      if (s.is_point()) return o;
      ...
      {
        cv_above =
          _bisector_of_collinear_point_and_segment(s.segment().source(),
                                                   s.segment());
        cv_below =
          _bisector_of_collinear_point_and_segment(s.segment().target(),
                                                   s.segment());
      }
      else {
        cv_above =
  ...
  public:
    ...
    OutputIterator operator()(const Xy_monotone_surface_3& s1,
                              ...
                              OutputIterator o) const {
      Curve_2 bisector;
      if (s1.is_segment() && s2.is_point()) return operator() (s2, s1, o);
      ...
      if (s1.is_segment() && s2.is_segment()) {
        ...
        if ((line1 == line2) || (line1 == line2.opposite())) {
          ...
          if (CGAL::compare_xy(s1.segment().min(), s2.segment().max()) ==
              SMALLER) {
            bisector =
              _bisector_of_collinear_point_and_segment(s1.segment().max(),
                                                       s2.segment());
          }
          else {
            bisector =
              _bisector_of_collinear_point_and_segment(s1.segment().min(),
                                                       s2.segment());
          }
        }
        else {
          ...
          Rat_line_2 line2 = s2.segment().supporting_line();
          Rational a1 =
            line1.a(); Rational b1 = line1.b(); Rational c1 = line1.c();
          Rational a2 =
            line2.a(); Rational b2 = line2.b(); Rational c2 = line2.c();
          ...
          CGAL::Orientation o1 =
            CGAL::orientation(seg1.source(),seg1.target(), seg2.source());
          if (o1 == COLLINEAR)
            o1 = CGAL::orientation(seg1.source(),seg1.target(), seg2.target());

          CGAL::Orientation o2 =
            CGAL::orientation(seg2.source(), seg2.target(), seg1.source());
          if (o2 == COLLINEAR)
          ...
          if (o1 == o2) alg_seg2 = alg_seg2.opposite();

          Alg_line_2 l =
            CGAL::bisector(alg_seg1.supporting_line(), alg_seg2.supporting_line());
          Alg_point_2 p1 = l.point(0);
          Alg_point_2 p2 = l.point(1);

          // creating the conic
          bisector = Curve_2(r, s, t, u, v, w, COLLINEAR, p1, p2, true, true);
        }
      }
      ...
      if (s1.is_point() && s2.is_segment()) {
        if (CGAL::orientation(s1.point(), s2.segment().source(),
          ...
          bisector =
            _bisector_of_collinear_point_and_segment(s1.point(), s2.segment());
        }
        else {
      ...
      for (auto it = x_mono_list.begin(); it != x_mono_list.end(); ++it) {
        ...
        if (assign(x, *it)) *o++ = make_object(Intersection_curve(x, 1));
      }
      return o;
    }
  };
 ...
 protected:
   ...
   */
  static Curve_2
  _bisector_of_collinear_point_and_segment(const Rat_point_2& point,
                                           const Rat_segment_2& seg) {
    CGAL_envelope_voronoi_precondition
      (CGAL::orientation(point, seg.source(), seg.target()) == COLLINEAR);
    CGAL_envelope_voronoi_precondition
      ((point == seg.source()) || (point == seg.target()) ||
    ...
    if (CGAL::compare_xy(seg.min(), point) == SMALLER) p = &seg.max();
      ...
      CGAL_envelope_voronoi_assertion
        (CGAL::compare_xy(seg.max(), point) == LARGER);
      p = &seg.min();
    }
    ...
    else line = CGAL::bisector(*p, point);
...
{
  if (site.is_point()) os << "Point: " << site.point();
  else os << "Segment: " << site.segment();
