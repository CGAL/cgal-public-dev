...
class Triangle_area_distance_traits_2 : public Arr_linear_traits_2<Kernel_>
{
  ...
  typedef Triangle_area_distance_traits_2<Kernel>  Self;
...
public:
  ...
  typedef std::pair<Point_2, Point_2>              Site_2;
  
  Triangle_area_distance_traits_2() {}
  
  Triangle_area_distance_traits_2(const Kernel& ker)
    : Base(ker) {}

  class Construct_bisector_2
  {
  public:
    Construct_bisector_2(const Self * traits)
      : m_traits(traits) {}
    ...
    */
    template <class OutputIterator>
      OutputIterator operator()(const Site_2& pq,
                                const Site_2& rs,
                                OutputIterator o) const
    {
      ...
      {
        typename Kernel::Construct_ray_2 const_ray = 
          m_traits->construct_ray_2_object();
  ...
  public:
    ...
    /*! The function determines which of the sites dominates the area above
        ...
        and the other is even then the region is dominated by the second
        site.
    */
...

    {
          ...
          m_traits->collinear_2_object()(h1.first, h1.second, h2.second))
      {
        ...
        typename Kernel::Compute_squared_distance_2 comp_sq = 
      ...
      if (p_xcv_l1 == EQUAL)
      {
        return CGAL::opposite(p_xcv_l2 * p_l1 * p_l2);
      }
...
//       if (sup_line == l2 || sup_line == l2.opposite())
//         on_l1 = NEGATIVE;
...
//         if (l1.is_vertical())
//         {
//           l1_above_l2 = CGAL::opposite(CGAL::compare(
//                                          l1.x_at_y(0),
//                                          l2.x_at_y(0)));
//           if (p.x() < q.x())
//           {
...
//       }
  ...
  protected:
    ...
    CGAL::Comparison_result compare_point_and_line(const Point_2& p, 
                                                   const Line_2& l) const
      {
        if (l.is_vertical())
        {
