...
namespace Envelope_voronoi_traits_2 {
  ...
  public:
    ...
    const Rational& A() const { return _A; }

    const Rational& B() const { return _B; }

    const Rational& C() const { return _C; }

    const Rational& D() const { return _D; }
  };
} // Envelope_voronoi_2
...
<CurvedKernel_, Anisotropic_voronoi_traits_2<CurvedKernel_>,
...
public:
  using Site_2 = Envelope_voronoi_traits_2::_Anisotropic_site<Rational>;
  using Self = Anisotropic_voronoi_traits_2<Curved_kernel_2>;
  using Base = Algebraic_apollonius_traits_base_2<Curved_kernel_2, Self,
                                                  Site_2, CGAL::Field_tag>;
...
public:
   ...
   * the segment. (A ray is treated as 2~different sites.)  The handling of all
   * predicates, specifically predicates related to bisector construction, are
   * made much simpler with the cost of an enlarged set of sites.
   ...
   * namely, the bisector between two points, the bisector between two lines
   * (where a line can also relate to the interior of a segment or a ray) that
   * is a two-line bisector, and the bisector between a point and a line that is
   * a full parabola.
   */
  ...
  public:
    Construct_projected_intersections_2(Self* traits) : m_traits(traits) {}
  ...
  protected:
     ...
     * which contain pair<X_monotone_curve_2,Multiplicity>.  The multiplicity
     * used is always 1 (this is the multiplicity of the bisector in this
     * function).
     *
    ...
    OutputIterator convert_to_bisector(InputIterator begin, InputIterator end,
                                       OutputIterator o) const {
      while (begin != end)
        *o++ = make_object(std::make_pair(*begin++, Multiplicity(0)));
      return o;
    }
  };
  ...
  public:
    Construct_projected_boundary_2(Self* traits) : m_traits(traits) {}
...
protected:
  ...
  */
  template <typename OutputIterator>
  OutputIterator construct_conic(const Rational& r,
                                 const Rational& s,
                                 const Rational& t,
                                 const Rational& u,
                                 const Rational& v,
