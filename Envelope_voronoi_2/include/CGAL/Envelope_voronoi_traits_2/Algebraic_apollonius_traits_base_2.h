...
public:
  using Rat_point_2 = std::pair<Rational, Rational>;
  ...
  friend bool operator==(const _Apollonius_disk& s1,
                         const _Apollonius_disk& s2)
  { return (s1.center() == s2.center() && s1.r() == s2.r()); }
  ...
  friend std::istream & operator>>(std::istream & is, _Apollonius_disk& site) {
    Rational x, y;
    is >> x >> y >> site._r;
 ...
 */
template <typename CurvedKernel_, typename Derived_,
          typename Site_2_, typename Distance_ret_req_>
class Algebraic_apollonius_traits_base_2 : public CurvedKernel_ {
public:
  using Curved_kernel_2 = CurvedKernel_;
  using Derived = Derived_;
  using Site_2 = Site_2_;
  using Distance_ret_req = Distance_ret_req_;

  using Self = Algebraic_apollonius_traits_base_2<Curved_kernel_2, Derived,
                                                  Site_2, Distance_ret_req>;
  using Base = Curved_kernel_2;
  ...
  using Field_with_sqrt = typename CGAL::Get_arithmetic_kernel<Coefficient>::
    Arithmetic_kernel::Field_with_sqrt;
  using Coordinate_1 = typename Algebraic_kernel_2::Coordinate_1;

  using Rational = typename Coordinate_1::Rational;
  using Solve_1 = typename Algebraic_kernel_2::Algebraic_kernel_1::Solve_1;
  ...
  using Distance_nt =
    typename boost::mpl::if_<boost::is_base_of<Field_with_sqrt_tag,
                                               Distance_ret_req>,
                             Field_with_sqrt, Rational>::type;

  // Field_with_sqrt is CORE::Expr !!!!
  static inline void convert_to_CORE(const Point_2& p,
                                     Field_with_sqrt& x, Field_with_sqrt& y) {
    Coordinate_1 point_x = p.x();

    using Root_of =
      typename CGAL::Algebraic_structure_traits<Field_with_sqrt>::Root_of;
    Solve_1 solve;
    ...
    int kid = std::distance(real_roots.begin(),
                            std::find(real_roots.begin(), real_roots.end(),
                                      point_x));
    ...
    using Poly_sqrt_1 = CGAL::Polynomial< Field_with_sqrt>;
    using PT_sqrt_1 = CGAL::Polynomial_traits_d<Poly_sqrt_1>;
...
protected:
  ...
  template <typename Field_, typename FieldWithSqrt, typename NT_,
    class OutputIterator >
    static inline int solve_quadratic(Polynomial< NT_ > p, OutputIterator it) {

    using Field = Field_;
    using Field_with_sqrt = FieldWithSqrt;
    using NT = NT_;
...
public:
  using Xy_monotone_surface_3 = Site_2;
  using Surface_3 = Site_2;
  ...
  public:
    ...
    Comparison_result operator()(const Xy_monotone_surface_3& h1,
                                 const Xy_monotone_surface_3& h2) {
      Distance_nt de1 = Derived::distance(Distance_nt(0), Distance_nt(0), h1);
      Distance_nt de2 = Derived::distance(Distance_nt(0), Distance_nt(0), h2);
  ...
  public:
    Comparison_result operator()(const X_monotone_curve_2& cv,
      ...
      Distance_nt de1 = Derived::template
        distance<Distance_nt>(p_above.first, p_above.second, h1);
      Distance_nt  de2 = Derived::template
        distance<Distance_nt>(p_above.first, p_above.second, h2);
  ...
  public:
    ...
    Comparison_result operator()(const X_monotone_curve_2& cv,
      ...
      Distance_nt de1 = Derived::template
        distance<Distance_nt>(p_below.first, p_below.second, h1);
      Distance_nt de2 = Derived::template
        distance<Distance_nt>(p_below.first, p_below.second, h2);
  ...
  template <typename OutputIterator>
    ...
    if (r_res == EQUAL) {
      ...
      traits.make_x_monotone_2_object()(cur, std::back_inserter(x_objs));
    ...
    */
    ...
    for (auto it = x_objs.begin(); it != x_objs.end(); ++it) {
      ...
      if ((xcurve = object_cast<X_monotone_curve_2> (&(*it))) != nullptr) {
        ...
        CGAL_error_msg("The bisector at this point should be a hyperbola");
    }
    ...
    if (x_conix.size() == 2) {
      ...
      if (x_conix.front().arcno() == x_conix.back().arcno()) {
        ...
        Comparison_result res;
        // ri < rj
        if (r_res == SMALLER) res = CGAL::compare(xi, xj);
        else res = CGAL::compare(xj, xi);
        CGAL_envelope_voronoi_assertion(res != EQUAL);

        if (res == SMALLER)
          *o++ = make_object(std::make_pair(x_conix.front(), Multiplicity(1)));
        else
          *o++ = make_object(std::make_pair(x_conix.back(), Multiplicity(1)));
      }
      else {
        // ri < rj
        if (r_res == SMALLER) res = CGAL::compare(yi, yj);
        else res = CGAL::compare(yj, yi);
        CGAL_envelope_voronoi_assertion(res != EQUAL);

        // we don't have vert asymptot
        if (res == SMALLER && x_conix.front().arcno() == 0)
          *o++ = make_object(std::make_pair(x_conix.front(), ultiplicity(1)));
        else
          *o++ = make_object(std::make_pair(x_conix.back(), Multiplicity(1)));
      }
    }
      ...
      // is lexicographic
      typename std::list<X_monotone_curve_2>::iterator it = x_conix.begin();
      if (res == SMALLER) {
