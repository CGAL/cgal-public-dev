...
Qt_widget&
operator<<(Qt_widget& ws, const _Conic_x_monotone_arc_2<ConicArc>& cv) {
  ...
  if (cv.orientation() == COLLINEAR) n = 1;
  if (n <= 0) return (ws);
  using App_point_2 = std::pair<double, double>;
  int i;
  App_point_2* pts = new App_point_2[n + 1];
  cv.polyline_approximation (n, bbox, pts);
  ws.get_painter().moveTo(ws.x_pixel(pts[0].first), ws.y_pixel(pts[0].second));
  for (i = 1; i <= n; i++) {
...
operator<<(Qt_widget& ws,
           const typename Arr_conic_traits_2
           <Rat_kernel, Alg_kernel, Nt_traits>::Curve_2& cv) {
  using Conic_traits_2 = Arr_conic_traits_2<Rat_kernel, Alg_kernel, Nt_traits>;
  using X_monotone_conic_arc_2 = typename Conic_traits_2::X_monotone_curve_2;
  ...
  traits.curve_make_x_monotone(cv, std::back_inserter (x_arcs));
  for (x_iter = x_arcs.begin(); x_iter != x_arcs.end(); ++x_iter) ws << *x_iter;
  return ws;
}
