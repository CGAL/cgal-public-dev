 ...
 */
template <typename Conic_traits>
...
{
  using Rat_kernel = typename Conic_traits::Rat_kernel;
  using Segment_2 = typename Rat_kernel::Segment_2;
  using Point_2 = typename Rat_kernel::Point_2;
 ...
 */
template <typename Conic_traits>
void write_conic_arc(CGAL::Fig_stream<typename Conic_traits::Rat_kernel>& fs,
                     const typename Conic_traits::Curve_2& cv) {
  using Conic_arc_2 = typename Conic_traits::X_monotone_curve_2;

  // Subdivide the arc into x-monotone sub-arcs.
  Conic_traits           traits;
  std::list<Object> xcvs;
  traits.make_x_monotone_2_object()(cv, std::back_inserter(xcvs));
  // Write the x-monotone sub-arcs.
  for (auto xit = xcvs.begin(); xit != xcvs.end(); ++xit) {
    Conic_arc_2 arc;
    if (assign(arc, *xit)) write_x_monotone_conic_arc<Conic_traits> (fs, arc);
  }
}
