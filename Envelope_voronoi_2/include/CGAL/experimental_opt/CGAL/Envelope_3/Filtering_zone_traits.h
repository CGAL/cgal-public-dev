...
public:
  using EnvelopeTraits = EnvelopeTraits_;
  using X_monotone_curve_2 = typename    EnvelopeTraits_::X_monotone_curve_2;
  using Point_2 = typename EnvelopeTraits_::Point_2;
  using Traits_adaptor_2 = Arr_traits_adaptor_2<EnvelopeTraits>;
...
template <typename EnvelopeTraits_>
...
public:
  ...
  public:
    ...
    Is_between_endpoints(const Base *traits) : m_traits(traits) {}
...
public:
  ...
  using Self = Filtering_zone_traits<Traits_, Arrangement_>;
  using Base = Arr_basic_insertion_traits_2<Traits_, Arrangement_>;
  using Base_Split_2 = typename Traits_2::Split_2;
  using Base_Intersect_2 = typename Traits_2::Intersect_2;
  using Base_point_2 = typename Base::Point_2;
  using Base_x_monotone_curve_2 = typename Base::X_monotone_curve_2;
  using Base_compare_xy_2 = typename Traits_2::Compare_xy_2;
  using Base_equal_2 = typename Base::Equal_2;
  using Base_compare_x_2 = typename Base::Compare_x_2;
  using Base_compare_x_near_boundary_2 =
    typename Base::Compare_x_near_boundary_2;
  using Base_construct_min_vertex_2 = typename Base::Construct_min_vertex_2;
  using Base_construct_max_vertex_2 = typename Base::Construct_max_vertex_2;
  using Base_compare_y_at_x_2 = typename Base::Compare_y_at_x_2;
  using Compare_y_near_boundary_2 = typename Base::Compare_y_near_boundary_2;
  using Base_compare_y_at_x_left_2 = typename Traits_2::Compare_y_at_x_left_2;
  using Base_compare_y_on_identification_2 =
    typename Base::Compare_y_on_identification_2;
  using Curve_2 = typename Traits_2::Curve_2;
  using Make_x_monotone_2 = typename Traits_2::Make_x_monotone_2;
  using Multiplicity = typename Traits_2::Multiplicity;
  using Has_boundary_category = typename Base::Boundary_category;
  using Has_left_category = typename Base::Has_left_category;
  using Has_merge_category = typename Traits_2::Has_merge_category;
  ...
  // for filtering zone.
  using Xy_monotone_surface_3 = typename Traits_2::Xy_monotone_surface_3;
  using Surface_3 = typename Traits_2::Surface_3;
  using Surface_id = typename Traits_2::Surface_id;
  using Is_between_endpoints = typename Traits_2::Is_between_endpoints;
  ...
  public:
    ...
    Point_2(const Base_point_2& p) : Base_point_2(p), is_current_point(false) {}
  ...
  friend std::ostream& operator<<(std::ostream &out, const Point_2& point)
...
public:
  ...
  public:
    ...
    { return (m_base_cmp_y_at_x_left(cv1.base(), cv2.base(), p.base())); }
  };
  ...
  public:
    Compare_y_on_identification_2
    (const Base_compare_y_on_identification_2& base) :
...
//      std::clog << "intersecting " << cv1 << " and " << cv2 << std::endl;
      ...
      using Intersection_point = std::pair<typename Traits_2::Point_2, Multiplicity>;
      using Intersection_curve = typename Traits_2::X_monotone_curve_2;
      ...
      for(it = obj_list.begin(); it != obj_list.end(); ++it)
      {
        ...
        if (base_overlap_cv != nullptr) {
          ...
          *oi++ = make_object (overlap_cv);
          ...
          intersect_p = object_cast<Intersection_point> (&(*it));

          CGAL_envelope_voronoi_assertion (intersect_p != nullptr);
  ...
  public:
    Equal_2(const Base_equal_2& base) : Base_equal_2(base) {}
  ...
  public:
    Compare_x_2(const Self* traits) : _traits(traits) {}

    Comparison_result operator()(const Point_2& p1, const Point_2& p2) const {
      if (p1.vertex_handle() == p2.vertex_handle() &&
          p1.vertex_handle() != Vertex_handle()) {
        CGAL_envelope_voronoi_assertion
          (_traits->Base::compare_x_2_object()(p1, p2) == EQUAL);
        return EQUAL;
      }
  ...
  public:
    Compare_x_near_boundary_2(const Self* traits) : _traits(traits) {}
      ...
      if (vh != Vertex_handle()) {
        if (cv.is_current_curve) {
          if (vh->is_on_surfaces(_traits->_surf1.id, _traits->_surf2.id)) {
            CGAL_envelope_voronoi_assertion
              (_traits->Base::compare_x_near_boundary_2_object()(p, cv, ce) ==
               EQUAL);
            return EQUAL;
          }
        }
    ...
    Comparison_result operator()(const X_monotone_curve_2& cv1,
      ...
      if (cv1.is_current_curve && cv2.is_current_curve) {
        CGAL_envelope_voronoi_assertion
          (_traits->Base::compare_x_near_boundary_2_object()(cv1, ind1,
                                                             cv2, ind2) ==
           EQUAL);
        return EQUAL;
      }
      ...
      return
        _traits->Base::compare_x_near_boundary_2_object()(cv1, ind1, cv2, ind2);
    }
  };
  ...
  private:
    const Self* _traits;
  public:
  Compare_y_at_x_2(const Self* traits) : _traits(traits) {}
    ...
    Comparison_result operator()(const Point_2& p,
      ...
      if (vh != Vertex_handle()) {
        if (cv.is_current_curve) {
          ...
          {
            CGAL_envelope_voronoi_assertion
              (_traits->Base::compare_y_at_x_2_object() (p, cv) == EQUAL);
            return EQUAL;
          }
        }
        else {
