...
//
// $URL: svn+ssh://scm.gforge.inria.fr/svn/cgal/branches/CGAL-3.3-branch/Envelope_3/include/CGAL/Envelope_3/Eos_element_visitor_3.h $
// $Id: Eos_element_visitor_3.h 5450 2007-11-28 14:19:24Z ophirset $
//
// Author(s)     : Michal Meyerovitch     <gorgymic@post.tau.ac.il>
//                 Baruch Zukerman        <baruchzu@post.tau.ac.il>

#ifndef CGAL_EOS_ELEMENT_VISITOR_3_H
#define CGAL_EOS_ELEMENT_VISITOR_3_H
...
#include <CGAL/Envelope_3/Filtered_envelope_observer.h>
#include <CGAL/Envelope_3/Filtering_zone_traits.h>
...
CGAL::Timer timer_bisector;
CGAL::Timer timer_copy_face;
CGAL::Timer timer_insert_curve;
CGAL::Timer timer_decide_over_face;
CGAL::Timer timer_zone;
#endif

#if defined(CGAL_MEASURE_FACES)
static long intersect_faces = 0;
static long non_intersect_faces = 0;
#endif
...
public:
                             ...
                             Copied_original_vertices_map &map_copied_orig_vert)
  {
    ...
    do
    {
                                                   ...
                                                   xcurve) == EQUAL)
      {
        ...
        {
          CGAL_assertion(inter_vertices.size() == 1);
          if ((inter_vertices.front() == it->target()) ||
              (inter_vertices.front() == it->twin()->next()->target()))
          {
            return false;
          }
        }
        inter_vertices.push_back(it->source());
      }
    } while (++it != end_it);

    // if we didn't find any vertex we'll use the regular alg.
    if (inter_vertices.size() == 0)
      return false;

    // if we found only one vertex then we need to find the intersection.
    std::vector<Copied_point_2> inter_points;
    std::vector<Copied_halfedge_handle> inter_halfedges;
    if (inter_vertices.size() < 2)
    {
      CGAL_assertion(inter_vertices.size() == 1);
      it = end_it;
      do
      {
        ...
        if ((it->target() == inter_vertices.front()) ||
            (it->source() == inter_vertices.front()))
          continue;
    ...
    Copied_x_monotone_curve_2 temp_curve, left_curve, right_curve, result_curve;

    Arr_parameter_space bx = copied_traits->parameter_space_in_x_2_object()
      (xcurve, ARR_MIN_END);
  ...
  void resolve(Face_handle face, Minimization_diagram_2& result)
  {
...
#if defined(FILTERED_ZONE)
    Copied_face_traits copied_traits(*traits, surf1, surf2);
#else
    Copied_face_traits copied_traits(*traits);
#endif
    Copied_minimization_diagram copied_face_arr(&copied_traits);

    // here we maintain a mapping between edges in the copied arrangement and
    // their original generating edge from result
    Copied_original_halfedges_map map_copied_to_orig_halfedges;
    ...
    Copied_original_faces_map     map_copied_to_orig_faces;
    ...
    Copied_face_handle copied_face = copy_face(face, result, copied_face_arr,
    ...
    Copied_face_zone_visitor zone_visitor(result,
                                          ...
                                          this);
#if defined(CGAL_MEASURE_FACES)
    bool bisector_intersects = false;
#endif
    ...
    for(; inter_objs_it != inter_objs.end(); ++inter_objs_it)
    {
      ...
      else if (assign(curve, cur_obj))
      {
...
#else
        Copied_x_monotone_curve_2& xcurve(curve.first);
#endif
        ...
        zone_visitor.init (&copied_face_arr);
#if defined(CONVEX_ZONE) && defined(FILTERED_ZONE)
...
#endif
        {
          insert (copied_face_arr, xcurve, pl, zone_visitor);
        }
#if defined(CGAL_MEASURE_TIME)
        timer_zone.stop();
#endif
        ...
        special_edge_it != result_special_edges.end(); ++special_edge_it)
    {
          ...
          can_copy_decision_from_face_to_edge(special_he))
      {
        ...
        // {
        //   // this can only happen when the edge is fake, since the
        //   // edge is on the face's boundary
          ...
          else
          {
            ...
            if (tr_adaptor.parameter_space_in_y_2_object()(*icv, ARR_MIN_END) ==
              ...
              split_points.push_back
                (Point_2_with_info(traits->construct_min_vertex_2_object()(*icv),
                                   true, false));
            else
            ...
            if (tr_adaptor.parameter_space_in_y_2_object()(*icv, ARR_MAX_END) ==
                ...
                ARR_INTERIOR)
              split_points.push_back
                (Point_2_with_info(traits->construct_max_vertex_2_object()(*icv),
                                   false, true));
            else
...
protected:
                                     ...
                                     Copied_face_zone_visitor& visitor)
  {
    ...
    const Copied_halfedge_const_handle  *hh;
    const Copied_vertex_const_handle    *vh;
...
*/
                                            ...
                                            Arr_all_sides_oblivious_tag)
  {
    ...
    Copied_point_2 p;
    if (source)
      p = Copied_point_2(hh->source()->point(), hh->source());
    else
      p = Copied_point_2(hh->target()->point(), hh->target());
#else
    Copied_point_2* p_p;
    if (source)
      p_p = &(hh->source()->point());
  ...
  void copy_ccb(Ccb_halfedge_circulator hec, // the circulator to insert
                ...
                Copied_face_handle inside_face,     // the face in which we insert it
                Copied_minimization_diagram &to,  // the arrangement to which we insert
    ...
    do 
    {
      ...
      else
      {
...
#if defined(FILTERED_ZONE)
        Halfedge_handle hh_right_to_left = hh;
        if (HE_COMP_RES(hh) == SMALLER)
          hh_right_to_left = hh_right_to_left->twin();
        Copied_x_monotone_curve_2 copied_cv(current_cv, hh_right_to_left);
#else
        const Copied_x_monotone_curve_2& copied_cv(current_cv);
#endif
        ...
        else
        {
          ...
          Copied_vertex_handle copied_v2;
          ...
          Copied_halfedge_handle copied_new_he;
          if (!use_2_vertices)
          {
          ...
          else
          {
            ...
            if (is_outer_ccb)
            {
              ...
              else
              {
                ...
                copied_new_he = to.insert_at_vertices(copied_cv, copied_prev_he, copied_prev_v2);
              }
            ...
            else // copy inner boundary
            {
              ...
              copied_new_he = to_accessor.insert_at_vertices_ex(copied_cv,
                                                                copied_prev_v2,
                                                                copied_prev_he,
                                                                HE_COMP_RES(hh->twin()),
                                                                new_face);
                          ...
                          Original_copied_vertices_map&  map_orig_to_copied_vertices)
  {
    ...
    if (non_fict == Halfedge_handle())
    {
      ...
      Copied_face_handle              to_uf = to.faces_begin();
      Copied_ccb_halfedge_circulator  to_uf_hec = to_uf->outer_ccb();
    ...
    else
    {
...
#if defined(FILTERED_ZONE)
      Halfedge_handle temp = he_from;
      if (HE_COMP_RES(temp) == SMALLER)
        temp = temp->twin();
      Copied_x_monotone_curve_2 curve(he_from->curve(), temp);
#else
      Copied_x_monotone_curve_2 curve(he_from->curve());
#endif
      he_to = insert_non_intersecting_curve (to, curve);
      ...
      Halfedge_handle                 he;
      std::list<Copied_x_monotone_curve_2>   xcvs;
      ...
      for (he = he_from->next(); he != he_from; he = he->next())
      {
        ...
        {
#if defined(FILTERED_ZONE)
          Halfedge_handle temp = he;
          if (HE_COMP_RES(temp) == SMALLER)
            temp = temp->twin();
          xcvs.push_back (Copied_x_monotone_curve_2(he->curve(), temp));
#else
    ...
    const Copied_halfedge_handle   he_to_end = he_to;

    do
    {
#if defined(FILTERED_ZONE)
      if (he_to->source()->is_at_infinity() == false)
      {
        he_to->source()->point().set_vertex_handle(he_from->source());
      }
#endif
  ...
  Copied_face_handle copy_face(Face_handle face, Minimization_diagram_2& from,
                               Copied_minimization_diagram& to,
                               Copied_original_halfedges_map& map_copied_to_orig_halfedges,
                               Copied_original_vertices_map&  map_copied_to_orig_vertices,
                               Original_copied_halfedges_map& map_orig_to_copied_halfedges,
                               Original_copied_vertices_map&  map_orig_to_copied_vertices)
  {   
    ...
    if (face->number_of_outer_ccbs() > 0)
    {
      ...
      Copied_halfedge_handle  hec_to = map_orig_to_copied_halfedges[hec];
  ...
  public:
    ...
    void set_elements_collections(Copied_halfedges_hash& boundary,
                                  Copied_halfedges_hash& specialh,
                                  Copied_halfedges_hash_w_type& newh,
                                  Copied_faces_hash& parts,
                                  Copied_vertices_hash& boundaryv,
                                  Copied_vertices_hash& specialv,
                                  Copied_vertices_to_edges_map& v_to_h)
    {
  ...
  protected:
    Copied_halfedges_hash *boundary_halfedges;
    Copied_halfedges_hash *special_edges;
    Copied_halfedges_hash_w_type *new_edges;
    Copied_faces_hash     *face_parts;
    Copied_vertices_hash  *boundary_vertices;
    Copied_vertices_hash  *special_vertices;
    Copied_vertices_to_edges_map *vertices_to_halfedges;
  ...
  public:
    Copy_observer(Copied_minimization_diagram& small_,
                  ...
                  Copied_original_halfedges_map& map_h,
                  Copied_original_vertices_map&  map_v,
                  Copied_original_faces_map&     map_f)
    ...
    bool is_bounded_impl(Are_all_sides_oblivious_tag)
    {
    ...
    {
      // TEMPORARY IMPLEMENTATION - ASK EFI WHAT TO DO - No, ask yourself!!!
      Copied_topology_traits *tr = small_arr.topology_traits();
    ...
    void before_split_fictitious_edge (Copied_halfedge_handle e,
    ...
    void after_split_fictitious_edge (Copied_halfedge_handle e1,
                                      Copied_halfedge_handle e2)
    {
    ...
    virtual void before_create_edge (const Copied_x_monotone_curve_2& /* c */,
                                     Copied_vertex_handle v1,
                                     Copied_vertex_handle v2)
    {
    ...
    virtual void after_create_edge (Copied_halfedge_handle e)

    {
      ...
      else if (!v1_is_new && !v2_is_new)
      {
        ...
        Copied_halfedge_handle prev1 = he->prev();
        Copied_halfedge_handle prev2 = he->twin()->prev();
      ...
      else
      {
        ...
        Copied_halfedge_handle prev = he->prev();
    ...
    virtual void before_split_edge (Copied_halfedge_handle e,
                                    Copied_vertex_handle v,
                                    const Copied_x_monotone_curve_2& /* c1 */,
                                    const Copied_x_monotone_curve_2& /* c2 */)
    {
      // save state info for using _split_edge in after event
      split_v = v;
      split_e = e;
    }
    virtual void after_split_edge (Copied_halfedge_handle e1,
                                   Copied_halfedge_handle e2)
    {
      // find the corresponding split vertex in big_arr
    ...
    virtual void before_add_isolated_vertex (Copied_face_handle f,
                                             Copied_vertex_handle /* v */)
    {
      saved_face = f;
    }
    virtual void after_add_isolated_vertex (Copied_vertex_handle v)
    {
      // make sure it is the only new vertex right now
      CGAL_assertion(new_vertices.size() == 1 &&
                     new_vertices.back() == v);
    ...
    virtual void before_move_inner_ccb (Copied_face_handle from_f,
                                   Copied_face_handle to_f,
                                   Copied_ccb_halfedge_circulator )
    {
    ...
    virtual void after_move_inner_ccb (Copied_ccb_halfedge_circulator h)
    {
    ...
    virtual void before_move_isolated_vertex (Copied_face_handle from_f,
                                              Copied_face_handle to_f,
                                              Copied_vertex_handle )
    {
      // should be used after insert_at_vertices which creates a new face
    ...
    virtual void after_move_isolated_vertex (Copied_vertex_handle v)
    {
      CGAL_assertion(map_faces.is_defined(move_from));
      CGAL_assertion(map_faces.is_defined(move_to));
      CGAL_assertion(map_vertices.is_defined(v));
  ...
  public:
     ...
     */
    Result found_subcurve (const Copied_x_monotone_curve_2& cv,
                           Copied_face_handle face,
                           Copied_vertex_handle left_v, Copied_halfedge_handle left_he,
                           Copied_vertex_handle right_v, Copied_halfedge_handle right_he)
    {
#if defined(CGAL_MEASURE_TIME)
      timer_zone.stop();
#endif
...
*/      
                          ...
                          Copied_vertex_handle left_v, Copied_vertex_handle right_v)
    {
      ...
      Copied_halfedge_handle overlap_he = base_res.first;
    ...
    Copied_vertex_handle found_point_on_edge(const Copied_point_2& p, Copied_halfedge_handle he)
    {
      ...
      Copied_halfedge_handle     split_he;
