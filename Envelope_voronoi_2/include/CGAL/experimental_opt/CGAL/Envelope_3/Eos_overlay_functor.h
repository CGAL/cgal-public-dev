...
// \todo erase FILTERED_PLANES

#ifndef CGAL_EOS_OVERLAY_FUNCTOR_H
#define CGAL_EOS_OVERLAY_FUNCTOR_H
...
class Eos_overlay_functor {
public:
  using Minimization_diagram_2 = MinimizationDiagram_2;
  using Face_const_handle1 = typename Minimization_diagram_2::Face_const_handle;
  using Face_const_handle2 = typename Minimization_diagram_2::Face_const_handle;
  using Vertex_const_handle1 =
    typename Minimization_diagram_2::Vertex_const_handle;
  using Vertex_const_handle2 =
    typename Minimization_diagram_2::Vertex_const_handle;
  using Halfedge_const_handle1 =
    typename Minimization_diagram_2::Halfedge_const_handle;
  using Halfedge_const_handle2 =
    typename Minimization_diagram_2::Halfedge_const_handle;
  ...
  using Res_face_handle = typename Minimization_diagram_2::Face_handle;
  using Res_halfedge_handle = typename Minimization_diagram_2::Halfedge_handle;
  using Res_vertex_handle = typename Minimization_diagram_2::Vertex_handle;

protected:
  using Envelope_data_iterator =
    typename Minimization_diagram_2::Dcel::Dcel_data_iterator;
...
public:
  Eos_overlay_functor(Minimization_diagram_2& md1,
                      Minimization_diagram_2& md2,
                      Minimization_diagram_2& result) :
    m_1(md1), m_2(md2), m_result(result)
  {}

  ~Eos_overlay_functor() { traversed_vertices.clear(); }
  ...
  void create_edge(Halfedge_const_handle1 h1, Halfedge_const_handle2 h2,
    ...
    res_h->surfaces() = h1->surfaces();
    res_h->surfaces().insert(h2->surfaces().begin(), h2->surfaces().end());
    res_h->twin()->surfaces() = res_h->surfaces();
#endif
  }
...
#if defined(FILTERED_ZONE)
    res_h->surfaces() = h1->surfaces();
    res_h->twin()->surfaces() = h1->surfaces();
#endif
  }
...
#if defined(FILTERED_ZONE)
    res_h->surfaces() = h2->surfaces();
    res_h->twin()->surfaces() = h2->surfaces();
#endif
  }
