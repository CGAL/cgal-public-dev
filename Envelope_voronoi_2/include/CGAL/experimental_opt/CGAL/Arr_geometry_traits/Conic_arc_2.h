...
public:
  ...
  typedef typename Alg_kernel::Direction_2                 Direction_2;
  typedef typename Alg_kernel::Line_2                      Line_2;
  typedef typename Alg_kernel::Vector_2                    Vector_2;
 ...
 public:
   ...
   * \param source_dir_unbounded If true, the side of the source point is not bounded by the source point.
   * \param target_dir_unbounded If true, the side of the target point is not bounded by the target point.
  ...
  _Conic_arc_2 (const Rational& r, const Rational& s, const Rational& t,
    ...
    _extra_data_P (NULL)
    {
      ...
      // if this is an elipse, then either target_dir_unbounded & source_dir_unbounded
      // make it full conic.
      ...
      if (target_dir_unbounded)
        _info |= IS_TARGET_DIR_UNBOUNDED;
      if (source_dir_unbounded)
        _info |= IS_SOURCE_DIR_UNBOUNDED;
    }
   ...
   * \pre The arc does not represent a full conic curve and the source is not bounded.
   */
   ...
   * \pre The arc does not represent a full conic curve and the target is not bounded.
   */
  ...
  bool has_vertical_asymptote () const
    {
      ...
      if (this->_orient != COLLINEAR && CGAL::sign(this->_s) == ZERO)
        return true;
      return false;
    }
  ...
  Line_2 get_algebraic_vertical_asymptote () const
    {
      ...
      return Line_2(nt_traits.convert(rat_line.a()), nt_traits.convert(rat_line.b()), 
 ...
 protected:
   ...
   * If this is hyperbolic arc, then the data contains the characterization of the
   * hyperbolic branch the arc is placed on (We save the minor axis).
  ...
  bool _is_strictly_between_endpoints (const Point_2& p) const
    {
      ...
      if (_extra_data_P != NULL && is_parabola==false)
        {
          ...
          if (_orient == COLLINEAR)
            {
              ...
              if (ker.compare_x_2_object() (_source, _target) == EQUAL)
              ...
              else
                {
                  ...
                  // x coordinate of p is on the same side of bounded_point as the unbounded_point.
          ...
          else
            {
              ...
              // This is the case of parabola/hyperbola arc which has one unbounded side.
              // We divide the unit sphere into to arcs on the directions perpendicular
              // to extra_data line.
              // We check that the direction (bounded_point, p) is on the same side of the
              // unit sphere as (bounded_point, unbounded_point).
              CGAL_assertion(_extra_data_P != NULL);
              Line_2 line(_extra_data_P->a, _extra_data_P->b, _extra_data_P->c);
              Vector_2 vec_data = ker.construct_vector_2_object() (line);
              Vector_2 perpen_vec = ker.construct_perpendicular_vector_2_object() (vec_data, CLOCKWISE);

            
              Vector_2 vec1 = ker.construct_vector_2_object() (bounded_point, p);
              Vector_2 vec2 = ker.construct_vector_2_object() (bounded_point, unbounded_point);
