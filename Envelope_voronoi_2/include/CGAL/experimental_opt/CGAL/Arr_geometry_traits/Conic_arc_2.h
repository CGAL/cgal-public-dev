...
public:
  ...
  using Self = _Conic_arc_2<Rat_kernel, Alg_kernel, Nt_traits>;
  ...
  using Point_2 = typename Alg_kernel::Point_2;
  using Direction_2 = typename Alg_kernel::Direction_2;
  using Line_2 = typename Alg_kernel::Line_2;
  using Vector_2 = typename Alg_kernel::Vector_2;
  using Conic_point_2 = _Conic_point_2<Alg_kernel>;
...
protected:
  ...
  // TODO: make enum normal. Add type of conic.

  // Bit masks for the _info field.
  // If the source is unbounded and the target is unbounded it doesn't mean
  // that this is a full curve (hyperbola, two lines etc.)
  enum {
    IS_VALID = 1,
    IS_FULL_CONIC = 2,
  ...
  unsigned int _info;    // Does the arc represent a full conic curve.
  ...
  // TODO: change the extra data to hold a Line_2<Alg_kernel>.
  /*! \struct
   ...
   * In case of an arc whose base is a parabola we hold a coincident line
   * to its directrix.
   */
  struct Extra_data {
    Algebraic a;
...
public:
   ...
   * \param source_dir_unbounded If true, the side of the source point is not
   * bounded by the source point.
   * \param target_dir_unbounded If true, the side of the target point is not
   * bounded by the target point.
  ...
  _Conic_arc_2(const Rational& r, const Rational& s, const Rational& t,
               ...
               bool source_dir_unbounded = false,
               bool target_dir_unbounded = false) :
    _orient(orient),
    _source(source),
    _target(target),
    _extra_data_P(nullptr) {
    ...
    // if this is an elipse, then either target_dir_unbounded & source_dir_
    // unbounded make it full conic.
    if (sign_conic == POSITIVE) _set_full(rat_coeffs, true);
    else _set(rat_coeffs);

    if (target_dir_unbounded) _info |= IS_TARGET_DIR_UNBOUNDED;
    if (source_dir_unbounded) _info |= IS_SOURCE_DIR_UNBOUNDED;
  }
  ...
  _Conic_arc_2(const Rat_circle_2& circ) :
    _orient(CLOCKWISE), _extra_data_P(nullptr) {
   ...
   *      not be the same.
   */
  _Conic_arc_2(const Rat_circle_2& circ, const Orientation& orient,
               const Point_2& source, const Point_2& target) :
    _orient(orient),
    _source(source),
    _target(target),
  ...
  _Conic_arc_2(const Rat_point_2& p1, const Rat_point_2& p2,
    ...
    Rat_kernel ker;
    auto orient_f = ker.orientation_2_object();
  ...
  _Conic_arc_2(const Rational& r, const Rational& s, const Rational& t,
      ...
      for (i = 0; i < n_xs; ++i) {
        for (j = 0; j < n_ys; ++j) {
          ...
          if (CGAL::sign(val) == ZERO) {
            ...
            // Update the endpoint if (xs[i], ys[j]) is the nearest pair so far.
            if (! found || curr_dist < min_dist) {
              if (k == 1) _source = Point_2(xs[i], ys[j]);
              else _target = Point_2(xs[i], ys[j]);
    ...
    if (Alg_kernel().compare_xy_2_object()(_source, _target) == EQUAL) {
      _info = 0;      // Invalid arc.
      return;
    }
   ...
   */
  bool is_valid() const { return ((_info & IS_VALID) != 0); }
  ...
  /*! Check whether the arc source is unbounded.
   */
  bool is_source_unbounded() const
  { return ((_info & IS_SOURCE_DIR_UNBOUNDED) != 0); }

  /*! Check whether the arc target is unbounded.
   */
  bool is_target_unbounded() const
  { return ((_info & IS_TARGET_DIR_UNBOUNDED) != 0); }

  bool is_hyperbola() const
  { return (CGAL::sign(4*_r*_s - _t*_t) == NEGATIVE); }

  /*! Get the arc's source.
   * \return The source point.
   * \pre The arc does not represent a full conic curve and the source is not
   *      bounded.
   */
   ...
   * \pre The arc does not represent a full conic curve and the target is not
   * bounded.
   */
   ...
   * \pre The source and target are bounded.
   * \return The bounding box.
   */
  Bbox_2 bbox() const {
      CGAL_precondition(is_valid());
      CGAL_precondition(! is_source_unbounded());
      CGAL_precondition(! is_target_unbounded());
   ...
   */
  int get_points_at_x(const Point_2& p, Point_2* ps) const {
   ...
   * \return The number of points found.
   */
  int get_points_at_y(const Point_2& p,Point_2* ps) const {
    // Get the y coordinates of the points on the conic.
    Algebraic xs[2];
  ...
  bool has_vertical_asymptote() const {
    CGAL_precondition(this->is_hyperbola());
    if (this->_orient != COLLINEAR && CGAL::sign(this->_s) == ZERO) return true;
    return false;
  }

  /*! Gets the asymptote of this hyperbola.
   */
  Rat_line_2 get_vertical_asymptote() const {
    CGAL_precondition(this->is_hyperbola());
    CGAL_precondition(has_vertical_asymptote());
    CGAL_assertion(CGAL::sign(this->_t) != ZERO);

    // The vertical asymptote is at x = -v/t.
    return Rat_line_2(1, 0, this->_v / this->_t);
  }

  /*! Gets the vertical asymptote of this hyperbola.
   */
  Line_2 get_algebraic_vertical_asymptote() const {
    Nt_traits nt_traits;
    Rat_line_2 rat_line = this->get_vertical_asymptote();

    return Line_2(nt_traits.convert(rat_line.a()),
                  nt_traits.convert(rat_line.b()),
                  nt_traits.convert(rat_line.c()));
  }
...
private:
  ...
  void _set(const Rational* rat_coeffs) {
    ...
    if ((CGAL::sign(_r) != ZERO || CGAL::sign(_s) != ZERO ||
         CGAL::sign(_t) != ZERO)) {
      if (_orient == COLLINEAR) {
        ...
        Alg_kernel ker;
        Point_2 p_mid = ker.construct_midpoint_2_object()(_source, _target);
        ...
        // TODO: verify what this does with Ron.
   ...
   * (could be any conic curve, not just an ellipse).
   * \param rat_coeffs A vector of size 6, storing the rational coefficients
   *                   of x^2, y^2, xy, x, y and the free coefficient resp.
  ...
  void _set_full(const Rational* rat_coeffs, const bool& comp_orient) {
    ...
    // if this is an imaginary conic then it is invalid.
    if (temp_conic.is_empty() == true) {
      _info = 0;
      return;
    }

    if (comp_orient) _orient = temp_orient;
...
protected:
  /*! Build extra data to be saved.
   * If this is hyperbolic arc, then the data contains the characterization of
   * the hyperbolic branch the arc is placed on (We save the minor axis).
   * If this is parabola, we save a line that is parallel to its directrix.
   */
  void _build_extra_data() {
    if (_extra_data_P != nullptr) return;
    ...
    //
    Nt_traits nt_traits;
    const int r_fact = (_orient == CLOCKWISE) ? -1 : 1;
    ...
    // in case of an elipse we don't do anything.
    if (CGAL::sign(det) == POSITIVE) return;
    ...
    // If this is hyperbola make sure that the two endpoints are located
    // on the same branch.
  ...
  Sign _sign_of_extra_data(const Algebraic& px, const Algebraic& py) const {
    ...
    if (_extra_data_P == nullptr) return (ZERO);
    Algebraic val =
      (_extra_data_P->a*px + _extra_data_P->b*py +_extra_data_P->c);
   ...
   * If one of them is unbounded then its whole branch is considered.
  ...
  bool _is_between_endpoints(const Point_2& p) const {
    ...
    Alg_kernel ker;
    if (ker.equal_2_object()(p, _source) || ker.equal_2_object()(p, _target))
      return true;
  ...
  bool _is_strictly_between_endpoints(const Point_2& p) const {
    ...
    bool is_parabola = (CGAL::sign(4*_r*_s-_t*_t)==ZERO);

    // Check if we have extra data available.
    if (_extra_data_P != nullptr && is_parabola==false) {
      if (_extra_data_P->side != ZERO) {
        ...
        if (_sign_of_extra_data(p.x(), p.y()) != ZERO) return false;
      }
    }

    // If both target and source are unbounded then the point is between
    // its end points because we checked the extra data (that it is on the
    // same branch / satisfies the line equation).
    const bool source_unbounded = is_source_unbounded();
    const bool target_unbounded = is_target_unbounded();
    if (source_unbounded && target_unbounded) return (true);

    // Act according to the conic degree.
    Alg_kernel ker;

    // if one of the end points is unbounded
    if (source_unbounded || target_unbounded) {
      const Point_2& unbounded_point = (source_unbounded) ? _source : _target;
      const Point_2& bounded_point = (source_unbounded) ? _target : _source;
      ...
      if (_orient == COLLINEAR) {
        ...
        if (ker.compare_x_2_object()(_source, _target) == EQUAL) {
          ...
          // of p is on the same side of bounded_point as the unbounded_point.
          res1 = ker.compare_y_2_object()(bounded_point, p);
          res2 = ker.compare_y_2_object()(bounded_point, unbounded_point);
        }
        else {
          // Otherwise, since the segment is x-monotone, just check whether the
          // x coordinate of p is on the same side of bounded_point as the
          // unbounded_point.
          res1 = ker.compare_x_2_object()(bounded_point, p);
          res2 = ker.compare_x_2_object()(bounded_point, unbounded_point);
        }
        // the bounded and the unbounded points shouldn't be equal
        CGAL_assertion(res2!=EQUAL);

        if (res1 == EQUAL || res1 != res2) return false;
        return true;
      }
      else {
        // TODO: change this to a kernel line. Change to not include target.
        // This is the case of parabola/hyperbola arc which has one unbounded
        // side.
        // We divide the unit sphere into to arcs on the directions
        // perpendicular to extra_data line.
        // We check that the direction (bounded_point, p) is on the same side
        // of the unit sphere as (bounded_point, unbounded_point).
        CGAL_assertion(_extra_data_P != nullptr);
        Line_2 line(_extra_data_P->a, _extra_data_P->b, _extra_data_P->c);
        Vector_2 vec_data = ker.construct_vector_2_object()(line);
        Vector_2 perpen_vec =
          ker.construct_perpendicular_vector_2_object()(vec_data, CLOCKWISE);

        Vector_2 vec1 = ker.construct_vector_2_object()(bounded_point, p);
        Vector_2 vec2 =
          ker.construct_vector_2_object()(bounded_point, unbounded_point);
        return (ker.orientation_2_object()(perpen_vec, vec1)==
                ker.orientation_2_object()(perpen_vec, vec2) );
      }
    }
    else {
      if (_orient == COLLINEAR) {
   ...
   */
  int _solve_quadratic_equation(const Algebraic& A, const Algebraic& B,
                                const Algebraic& C,
