...
public:
  using Conic_arc_2 = Conic_arc_;
  using Self = _Conic_x_monotone_arc_2<Conic_arc_2>;
  using Alg_kernel = typename Conic_arc_2::Alg_kernel;
  using Algebraic = typename Conic_arc_2::Algebraic;
  using Rational = typename Conic_arc_2::Rational;
  using Point_2 = typename Conic_arc_2::Point_2;
  using Conic_point_2 = typename Conic_arc_2::Conic_point_2;

  // Type definition for the intersection points mapping.
  using Conic_id = typename Conic_point_2::Conic_id;
  using Conic_pair = std::pair<Conic_id, Conic_id>;
  using Intersection_point_2 = std::pair<Conic_point_2, unsigned int>;
  using Intersection_list = std::list<Intersection_point_2>;
  ...
  using Intersection_map =
    std::map<Conic_pair, Intersection_list,Less_conic_pair>;
...
protected:
  using Base = Conic_arc_2;
  using Integer = typename Conic_arc_2::Integer;
  using Nt_traits = typename Conic_arc_2::Nt_traits;
  using Rat_kernel = typename Conic_arc_2::Rat_kernel;
...
public:
  ...
  /*! Default constructor.
   */
  _Conic_x_monotone_arc_2 () : Base(), _id() {}
  ...
  _Conic_x_monotone_arc_2(const Base& arc,
    ...
    if (target_dir_unbounded) this->_info |= this->IS_TARGET_DIR_UNBOUNDED;
    else this->_info &= ~this->IS_TARGET_DIR_UNBOUNDED;

    if (source_dir_unbounded) this->_info |= this->IS_SOURCE_DIR_UNBOUNDED;
    else this->_info &= ~this->IS_SOURCE_DIR_UNBOUNDED;
  ...
  _Conic_x_monotone_arc_2(const Point_2& source, const Point_2& target,
    ...
    if (dir_res == SMALLER) this->_info = (this->_info | IS_DIRECTED_RIGHT);
   ...
   * \param target The target point.
   */
  _Conic_x_monotone_arc_2(const Algebraic& a, const Algebraic& b,
                          const Algebraic& c,
                           const Point_2& source, const Point_2& target) :
    Base() {
    ...
    Alg_kernel ker;
    Comparison_result
      res = ker.compare_x_2_object()(this->_source, this->_target);
    ...
    if (res == SMALLER) this->_info = (this->_info | IS_DIRECTED_RIGHT);
  ...
  const Conic_point_2& left() const {
    if (is_directed_right()) return (this->_source);
  ...
  const Conic_point_2& right() const {
    if (is_directed_right()) return (this->_target);
  ...
  bool is_left_unbounded() const {
    if (is_directed_right()) return (this->is_source_unbounded());
    else return (this->is_target_unbounded());
  }
  ...
  bool is_right_unbounded() const {
    if (is_directed_right()) return (this->is_target_unbounded());
    else return (this->is_source_unbounded());
  }
...
public:
  ...
  Sign infinite_in_y (Arr_curve_end ce) const {
    ...
    if (this->is_hyperbola() && this->has_vertical_asymptote()) {
      if (_is_in_asymptote_direction(ce)) {
        ...
        if (ce == ARR_MIN_END) return (res==LARGER) ? POSITIVE : NEGATIVE;
        else return (res==LARGER) ? NEGATIVE : POSITIVE;
      }
    }

    if (this->_orient == COLLINEAR) {
      CGAL_assertion(this->_extra_data_P != nullpnt);

      Sign sign_a = CGAL::sign(this->_extra_data_P->a);
      Sign sign_b = CGAL::sign(this->_extra_data_P->b);

      if (sign_a == ZERO) return ZERO;
      if (sign_a == sign_b) {
        if (ce == ARR_MIN_END) return POSITIVE;
        else return NEGATIVE;
      }
      else {
        if (ce == ARR_MIN_END) return NEGATIVE;
      ...
      if (sign_s != ZERO)
      {
        ...
        Sign sign_to_check =
          CGAL::sign(equivalent_numerator) * sign_s * sign_inf;
        ...
        if (CGAL::sign(this->_t) != ZERO) {
          switch (CGAL::opposite(sign_inf) * CGAL::sign(this->_r) *
                  CGAL::sign(this->_t)) {
           case POSITIVE: return POSITIVE;
           case NEGATIVE:  return NEGATIVE;
           default: return ZERO;
   ...
   * \param inter_map Intersection map to the case we need to intersect the
   *        curves.
   * \return NEGATIVE if the curve end is at y = -oo;
   *         POSITIVE if the curve end is at y = +oo;
   *         ZERO if the curve end y-coordinate is finite.
                                          ...
                                          bool no_intersections = true) const
  {
    ...
    if ((this->orientation() == COLLINEAR) && (xc.orientation() == COLLINEAR)) {
      CGAL_assertion (this->_extra_data_P != nullpnt);
      CGAL_assertion (xc._extra_data_P != nullpnt);
    ...
    if (b1 > b2) return LARGER;
    else if (b1 < b2) return SMALLER;
    ...
    if (this->has_same_supporting_conic(xc)) {
      ...
      if ((this->_info & PLUS_SQRT_DISC_ROOT) ==
        ...
        return EQUAL;
      else if ((this->_info & PLUS_SQRT_DISC_ROOT) != 0) return LARGER * sign;
      else return SMALLER * sign;
    }
    ...
    if (no_intersections == true) {
      if (CGAL::sign(this->s()) != ZERO &&
        ...
        const Algebraic exp1 =
          xc.alg_s * (op_sign_ce * alg_t + sign_first_curve * det1);
        const Algebraic exp2 =
          alg_s * (op_sign_ce * xc.alg_t + sign_second_curve * det2);
        ...
        if (comp != EQUAL) {
          ...
          if (s == NEGATIVE) return CGAL::opposite(comp);
          return comp;
        }
      }
    }
  ...
  bool contains_point(const Conic_point_2& p) const {
    ...
    if (p.is_generating_conic(_id)) p_on_conic = true;
    else {
  ...
  Point_2 get_point_at_x(const Point_2& p) const {
    ...
    CGAL_precondition(is_left_unbounded() ||
                      ker.compare_x_2_object()(p, left()) != SMALLER);
    CGAL_precondition (is_right_unbounded() ||
    ...
    if (_is_special_segment()) {
      ...
      // simply have:
      Algebraic _y = -(this->_extra_data_P->a*p.x() + this->_extra_data_P->c) /
    ...
    if ((this->_info & DEGREE_MASK) == DEGREE_1) {
      ...
      if (num_roots == 1)
        ...
        if ((this->_info & PLUS_SQRT_DISC_ROOT) != 0) y = larger;
        else y = smaller;
      }
    }
   ...
   * \param bbox In case of infinite curves the given bounding box will be the
   *        limit of the painting.
  ...
  void split(const Conic_point_2& p, Self& c1, Self& c2) const {
    ...
    CGAL_precondition(this->is_target_unbounded() ||
                      ! ker.equal_2_object()(p, this->_target));

    // This code deals with the case where p is exactly at the source or the
    // target and they are unbounded. It solves the problem by moving the u
    // nbounded point a little bit in the direction of the "unboundness".
         ...
         (!dup.is_directed_right() && comp_source != LARGER)))
    {
      ...
      if (dup.is_vertical()) {
        ...
        if (! dup.is_directed_right()) new_y = p.y() + 1;
  ...
  Self flip() const {
    ...
    if (source_unbounded) arc._info |= Base::IS_TARGET_DIR_UNBOUNDED;
    else arc._info &= ~Base::IS_TARGET_DIR_UNBOUNDED;

    if (target_unbounded) arc._info |= Base::IS_SOURCE_DIR_UNBOUNDED;
    else arc._info &= ~Base::IS_SOURCE_DIR_UNBOUNDED;
  ...
  bool can_merge_with(const Self& arc) const {
    ...
    // the other conic.
    if (this->is_source_unbounded() && this->is_target_unbounded()) return false;
    if (arc.is_source_unbounded() && arc.is_target_unbounded()) return false;

    // Same goes for the same side of the two arcs.
    if (this->is_left_unbounded() && arc.is_left_unbounded()) return false;
    if (this->is_right_unbounded() && arc.is_right_unbounded()) return false;
  ...
  bool is_facing_up() const { return ((this->_info & FACING_UP) != 0); }

  bool is_facing_down() const { return ((this->_info & FACING_DOWN) != 0); }
  ...
  bool has_same_supporting_conic(const Self& arc) const {
    ...
    if (_id == arc._id && _id.is_valid() && arc._id.is_valid()) return true;
    ...
    if (this->_orient == COLLINEAR && arc._orient == COLLINEAR) {
      ...
      typename Alg_kernel::Line_2 l1 =
        construct_line(this->_source, this->_target);
      typename Alg_kernel::Line_2 l2 = construct_line(arc._source, arc._target);
    ...
    if (CGAL::sign (this->_r) != ZERO) factor1 = this->_r;
    else if (CGAL::sign (this->_s) != ZERO) factor1 = this->_s;
    else if (CGAL::sign (this->_t) != ZERO) factor1 = this->_t;
    else if (CGAL::sign (this->_u) != ZERO) factor1 = this->_u;
    else if (CGAL::sign (this->_v) != ZERO) factor1 = this->_v;
    else if (CGAL::sign (this->_w) != ZERO) factor1 = this->_w;

    if (CGAL::sign (arc._r) != ZERO) factor2 = arc._r;
    else if (CGAL::sign (arc._s) != ZERO) factor2 = arc._s;
    else if (CGAL::sign (arc._t) != ZERO) factor2 = arc._t;
    else if (CGAL::sign (arc._u) != ZERO) factor2 = arc._u;
    else if (CGAL::sign (arc._v) != ZERO) factor2 = arc._v;
    else if (CGAL::sign (arc._w) != ZERO) factor2 = arc._w;
...
private:
   ...
   */
  bool _is_on_supporting_conic(const Algebraic& px, const Algebraic& py) const {
    CGAL::Sign       _sign;
  ...
  void _derive_by_x_at(const Point_2& p, const unsigned int& i,
    ...
    //
    const Algebraic
      sl3_numer = (_two * alg_s * sl_numer - alg_t * sl_denom) * sl2_numer;
  ...
  bool _is_in_asymptote_direction (Arr_curve_end ce) const {
    ...
    if ((res == LARGER) && (ce == ARR_MIN_END)) return true;
    if ((res == SMALLER) && (ce == ARR_MAX_END)) return true;
  ...
  bool _compute_overlap(const Self& arc, Self& overlap) const {
    ...
    if (_is_strictly_between_endpoints(arc.left()) &&
        arc.is_left_unbounded() == false) {
      if (_is_strictly_between_endpoints(arc.right()) &&
          arc.is_right_unbounded() == false) {
        // Case 1 - *this:     +----------->
        //            arc:       +=====>
        overlap = arc;
      }
