...
public:
   ...
   * \param z An integer.
   * \return A rational number equivalent to z.
   */
  Rational convert_to_rational (const Integer& z) const
  {
    return (Rational (z, 1));
  }
                                 ...
                                 const Algebraic& x2) const
  {
    ...
    while (true)
    {
      // Check if x1*2^k and x2*2^k are separated by more than 2.
      ix1 = scaled_x1.BigIntValue();
      ix2 = scaled_x2.BigIntValue();

      if (CORE::abs (ix2 - ix1) > two)
        break;
