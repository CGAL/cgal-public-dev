...
//
// Library       : QdX
// File          : demos/xtriangulate/ogl_exts.C
// QdX_release   : $Name:  $
...
GLhandleARB OGL_view::load_shader(const char* vs_filename,
...
bool OGL_view::is_extension_supported(const char *name) {
    
    if(!name || *name == '\0') 
        return false;
 
    char *where;   
    char *start = (char *)glGetString(GL_EXTENSIONS);
