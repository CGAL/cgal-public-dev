...
/*! \file server.h
 *  \brief Defines class \c XAlci_server 
 *  
 *  Main server class: handling IPC requests from clients, sending commands to
 *  \c Rasterizer. \file rasterizer.h must be included *before* this header
 */

#ifndef XALCI_SERVER_H
#define XALCI_SERVER_H
...
//! timeout for rasterization phase (in seconds)
#define RASTERIZE_TIMEOUT   110
...
public:
    ...
    // TODO: replace boost::multi_index by STL map (long compilation times)
    //! curve analysis cache
    typedef boost::multi_index::multi_index_container<
        Analysis_entry,
        boost::multi_index::indexed_by<
            boost::multi_index::sequenced<>,
            boost::multi_index::hashed_unique<
                BOOST_MULTI_INDEX_MEMBER(Analysis_entry,MD5_digest,curve_ID),
                MD5_hash_func> > >
    Analysis_cache;

//     typedef std::map< MD5_digest, Analysis_entry, MD5_compare > Analysis_cache;
...
public:
    ...
    XAlci_server(int _id)  : server_id(_id)
    { }
    ...
    Error_code handle_analyse_request(Analysis_entry& analysis,
        char *ascii_poly, Thread_cleanup_info *info);
    
    //! rasterization request handling
    uint handle_rasterize_request(const Analysis_entry& analysis,
...
private:
    ...
    int width, height; //! dimensions of the graphic plot
    ...
    Analysis_cache analysis_cache; //! curve analysis cache
    //! stores a set of host IPs whose requests are currently being processed
    //! by the server
