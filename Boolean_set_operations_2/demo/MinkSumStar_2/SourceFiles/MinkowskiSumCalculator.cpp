...
int MinkowskiSumCalculator::getKValue()
{
  return kValue_;
}
...
std::shared_ptr<const MinkowskiSumResult> MinkowskiSumCalculator::getSum(int k)
{
  ...
  for(int i = initK + 1; i <= k; ++i)
  {
    ...
    {
      scaledSum.push_back(removeCollinearPoints(scalePolygon(*it, CGAL::Gmpq(1, i))));
    }
...
void MinkowskiSumCalculator::nextSum()
{
  toSum(kValue_ + 1);
}
void MinkowskiSumCalculator::toSum(int k)
{
  if (kValue_ < k) {
    kValue_ = k;
  }
  const MinkowskiSumResult* result;
  result = getSum(k).get();
  emit(polygonReady(new StdPolygonList(result->getSum()), k));
}

void MinkowskiSumCalculator::slotSetInput(const StdPolygonList& input)
{
  setInput(input);
}
...
MinkowskiSumResult::MinkowskiSumResult(const StdPolygonList& sum)
  : sum_(sum)
{
...
MinkowSkiSumStarController::MinkowSkiSumStarController()
{
  m_minkSum = new MinkowskiSumCalculator();
  m_minkSum->moveToThread(&m_workerThread);
  connect(this, SIGNAL(signalSetInput(const StdPolygonList&)),
    m_minkSum, SLOT(slotSetInput(const StdPolygonList&)));
  connect(&m_workerThread, &QThread::finished, m_minkSum, &QObject::deleteLater);
  connect(this, SIGNAL(operate()), m_minkSum, SLOT(nextSum()));
  connect(m_minkSum, SIGNAL(polygonReady(StdPolygonList*, int)),
    this, SLOT(handleResults(StdPolygonList*, int)));
...
void MinkowSkiSumStarController::setInput(const StdPolygonList& input)
{
  m_inputIsSet = true;
  emit(signalSetInput(input));
}

void MinkowSkiSumStarController::runCalculation(int maxK)
{
  if (m_inputIsSet) {
    m_maxK = maxK;
    if (m_maxK > 1 || m_maxK == -1) {
      m_running = true;
      emit(operate());
    }
  }
...
void MinkowSkiSumStarController::handleResults(StdPolygonList* polygon, int k)
{
  for (StdPolygonList::iterator pwhi = polygon->begin(); pwhi != polygon->end(); ++pwhi) {
    emit(requestNewPolygonCreation(&*pwhi, "MinkowskiSumStar", k));
  }
  // This here might be *dengerous*
  if (m_running && (m_maxK > m_minkSum->getKValue() || m_maxK == -1)) {
    emit(operate());
  }
}
