...
{ states_stack.back().ToggleView(RESULT_GROUP, a_check);}
...
bool MainWindow::read_circular ( QString aFileName, Circular_polygon_set& rSet, Circular_region_source_container& rSources )
{
  bool rOK = false ;
...
bool save_linear ( QString aFileName, Linear_polygon_set& rSet )
{
  ...
  if ( out_file )
  {
      ...
      rit != lpwh_container.end() ; ++ rit )
    {
      ...
      for ( Linear_polygon::Curve_const_iterator cit = lpwh.outer_boundary().curves_begin(); cit != lpwh.outer_boundary().curves_end(); ++ cit,++ i)
      {
        auto pt = cit->source();
      ...
      for ( Linear_polygon_with_holes::Hole_const_iterator hit = lpwh.holes_begin() ; hit != lpwh.holes_end() ; ++ hit )
      {
        ...
        for ( Linear_polygon::Curve_const_iterator cit = hit->curves_begin(); cit != hit->curves_end(); ++ cit,++ i)
        {
          auto pt = cit->source();
          out_file << "   " << CGAL::to_double(pt.x()) << " " << CGAL::to_double(pt.y()) << std::endl ;
        }
...
bool save_circular ( QString aFileName, Circular_polygon_set& rSet )
{
  ...
  if ( out_file )
  {
    ...
    for(std::vector<Circular_polygon_with_holes>::const_iterator rit = cpwh_container.begin(); rit != cpwh_container.end(); ++ rit)
    {
      ...
      for (Circular_polygon::Curve_const_iterator cit = cpwh.outer_boundary().curves_begin(); cit != cpwh.outer_boundary().curves_end() ; ++ cit,++ i)
      {
        auto pt = cit->source();
        if(cit->is_circular())
        {
      ...
      for ( Circular_polygon_with_holes::Hole_const_iterator hit = cpwh.holes_begin() ; hit != cpwh.holes_end() ; ++ hit )
      {
        ...
        for ( Circular_polygon::Curve_const_iterator cit = hit->curves_begin(); cit != hit->curves_end(); ++ cit,++ i)
        {
          auto pt = cit->source();
          if(cit->is_circular())
          {
...
void MainWindow::on_actionIntersectionH_toggled(bool aChecked)
{
  if(actionIntersectionH->isChecked())
  {
	...
	{
	  case 0:
	      if (!states_stack.back().blue_set().is_empty() && m_blue_int) states_stack.back().result_set().assign(states_stack.back().blue_set());
...
void MainWindow::on_actionSymmetric_DifferenceH_toggled(bool aChecked)
{
  if(actionSymmetric_DifferenceH->isChecked())
  {
	...
	{
        case 0:
            if (!states_stack.back().blue_set().is_empty() && m_blue_sym_diff) states_stack.back().result_set().assign(states_stack.back().blue_set());
...
void MainWindow::on_actionUnionH_toggled(bool aChecked)
{
  if(actionUnionH->isChecked())
  {
   ...
   /*operation_name
	...
	{
	    case 0:
	        if(m_red_union) states_stack.back().result_set().assign(states_stack.back().red_set());
