        ...
        try {
            ...
            for (unsigned int r = 0; r < n_regions; ++r) {
                ...
                for (unsigned int r = 0; r < n_boundaries; ++r) {
                    ...
                    Gps_traits traits;
                    typename Gps_traits::Make_x_monotone_2 make_x_monotone = traits.make_x_monotone_2_object();
                    typedef boost::variant <Circular_X_monotone_curve, Arc_point> Make_x_monotone_result;
...
bool save_linear(QString aFileName, Linear_polygon_set &rSet) {
    ...
    if (out_file)
    {
             ...
             rit != lpwh_container.end(); ++rit)
        {
            ...
            for (Linear_polygon_with_holes::Hole_const_iterator hit = lpwh.holes_begin(); hit != lpwh.holes_end();
                 ++hit) {
                int cc = hit->size();
                int lc = cc - 1;
                out_file << "  " << cc << std::endl;
                ...
                for (Linear_polygon::Curve_const_iterator cit = hit->curves_begin(); cit != hit->curves_end();
                     ++cit, ++i) {
                    auto pt = cit->source();
                    out_file << "   " << CGAL::to_double(pt.x()) << " " << CGAL::to_double(pt.y()) << std::endl;
                }
...
bool save_circular(QString aFileName, Circular_polygon_set &rSet)
{
    ...
    if (out_file) {
        ...
        for (std::vector<Circular_polygon_with_holes>::const_iterator rit = cpwh_container.begin();
            ...
            for (Circular_polygon_with_holes::Hole_const_iterator hit = cpwh.holes_begin(); hit != cpwh.holes_end();
                 ++hit) {
                int cc = hit->size();
                int lc = cc - 1;
                out_file << "  " << cc << std::endl;
                ...
                for (Circular_polygon::Curve_const_iterator cit = hit->curves_begin(); cit != hit->curves_end();
                     ++cit, ++i) {
                    auto pt = cit->source();
                    if (cit->is_circular()) {
                        out_file << "   1" << std::endl;
...
bool save_polyline(QString aFileName, Polyline_polygon_set &rSet) {
    ...
    if (out_file)
    {
             ...
             rit != lpwh_container.end(); ++rit)
        {
            ...
            for (Polyline_polygon::Curve_const_iterator cit = lpwh.outer_boundary().curves_begin();
                 cit != lpwh.outer_boundary().curves_end(); ++cit, ++i)
            {
                auto it = cit->points_begin();
                out_file<<" "<<CGAL::to_double((*it).x())<<" "<<CGAL::to_double((*it).y())<<std::endl;
            ...
            for (Polyline_polygon_with_holes::Hole_const_iterator hit = lpwh.holes_begin(); hit != lpwh.holes_end();
                 ++hit)
            {
                int cc = hit->size();
                int lc = cc - 1;
...
bool save_bezier_result(QString aFileName, Bezier_polygon_set const &aSet) {
    ...
    if (out_file) {
        ...
        for (std::vector<Bezier_polygon_with_holes>::const_iterator rit = bpwh_container.begin();
            ...
            for (Bezier_polygon_with_holes::Hole_const_iterator hit = bpwh.holes_begin(); hit != bpwh.holes_end();
                 ++hit)
