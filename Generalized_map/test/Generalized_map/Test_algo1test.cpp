...
public:
  ...
  void test(char* name,int geometric,int boundary) {
    ...
    Graph g2;
    std::vector<Vertex> verticesdual; // DOES NOT COMPILE  =gma0.create_CGAL_dual_Graph2(g2,0,Wdual);
    dh=gma0.map.darts().begin();
    // DOES NOT COMPILE gma0.CGAL_dual_Dijkstra(dh,gma0.mark0,g2,verticesdual);
    // DOES NOT COMPILE  gma0.remove_edges();
    gma0.map.display_characteristics(std::cout);
