...
#include <CGAL/My_cell_attribute_with_point.h>
...
public :
			...
			typedef typename K::template Attribute_handle<2>::type Attrib_h2;
				...
				for (typename K::template One_dart_per_cell_range<0>::
					           ...
					           it!=map.template one_dart_per_cell<0>().end();++it)
					    {
								...
								LCC_3 my_lcc_3;
								typename K::template Attribute_handle<0>::type ah;
								//std::cout << i << typeid((&ah)).name() << "\n";
								//ah=map.template create_vertex_attribute(//map.template vertex_attribute(it);
								if (typeid(&map)==typeid(&my_lcc_3)) {
									Point point=map.template point_of_vertex_attribute(map.template vertex_attribute(it));
									//ah=map.template create_vertex_attribute(map.template point_of_vertex_attribute(map.template vertex_attribute(it)));//map.template vertex_attribute(it);
									ah=map.template create_vertex_attribute(point);//map.template vertex_attribute(it)
								} else {
									//ah=map.template create__attribute();
								}
	...
	return minpath;
	 ...
	 /* wraps the boost implementation of the Dijkstra algorithm
	 	 	 ...
	 	 	 * g: an empty boost graph
	 	 	 * vertices : an empty vector of boost Vertex
	 	 	 *	This ones makes the graph of the dual
	 	 	 */

	 	 std::vector<Vertex> create_CGAL_dual_Graph2(Graph& g,int geometric,std::vector<int>& W) {
	 	 	 	...
	 	 	 	for (typename K::template One_dart_per_cell_range<2>::
	 	 	 	  ...
	 	 	 	  for (int k=0;k<vertices.size();k++){
	 	 	 		 ...
	 	 	 		 int itn=(int)(map.template info<2>(it));
	 	 	 		 ///boost::unordered_map<int,int> neigh=find_neighbors_vertex(it);
	 	 	 		 boost::unordered_map<int,int> neighmap=find_neighbors_face(it,mark0,valids);
	 	 	 		 std::vector<int> neigh;
	 	 	 		 for (boost::unordered_map<int,int>::iterator itm=neighmap.begin();itm!=neighmap.end();itm++) {
	 	 	 			 neigh.push_back(itm->second);
	 	 	 		 }
	 	 	 		  ...
	 	 	 		  for (std::vector<int>::iterator neighit=neigh.begin();
	 	 	 			  ...
	 	 	 			  if (geometric) {
	 	 	 				...
	 	 	 				Point p1=map.template point_of_vertex_attribute(facesvector[itn]);
	 	 	 				Point p2=map.template point_of_vertex_attribute(facesvector[*neighit]);
	 	 	 				  weight0=(double)(std::sqrt(std::pow(p2.x()-p1.x(),2)+
	 	 	 				   std::pow(p2.y()-p1.y(),2)+
	 	 	 				   std::pow(p2.z()-p1.z(),2)));
	 	 	 				   //weight0=W.at(itn).at(*neighit);
	 	 	 			  ...
	 	 	 			  if (W.size()==map.template count_all_cells()[1]) {
	 	 	 				  Dart_handle d1=facesvector[itn]->dart();
	 	 	 				  Dart_handle d2=facesvector[*neighit]->dart();
	 	 	 		...
	 	 	 		for (std::vector<int>::iterator neighit=neigh.begin();
	 		 	 	 ...
	 		 	 	 for(boost::graph_traits<Graph>::edge_iterator edgeIterator = edgeIteratorRange.first; edgeIterator != edgeIteratorRange.second; ++edgeIterator)
	 		 	 	 {
	 	 	 	 	 ...
	 	 	 	 	 std::cout << vertices.size() << " " << facesvector.size() << " vertex vertex\n";
	 	 	 	  return vertices;
	 	 	 }
 	...
 	void CGAL_dual_Dijkstra(Dart_handle& source,typename K::size_type& amark2,Graph& g,
 		 		 			std::vector<Vertex>& vertices
 		 					) {
 		 		typedef typename K::Vertex_attribute vertex;
 		 		  	  std::vector<Vertex> predecessors(boost::num_vertices(g)); // To store parents
 		 		 	  ...
 		 		 	  PredecessorMap predecessorMap(&predecessors[0], indexMap);
 		 		 	  DistanceMap distanceMap(&distances[0], indexMap);
 		  boost::dijkstra_shortest_paths(g, vertices[map.template info<2>(source)]
 		 														 , boost::distance_map(distanceMap).predecessor_map(predecessorMap));
 		 		   ...
 		 		   int k=0;
 		 		 	  ...
 		 		 	  for (int k=0;k<vertices.size();k++) {
 		 		 		  ...
 		 		 		  Vertex v=vertices[k];
 		 		 		  if (k!=map.template info<2>(source))
 		 		 			facesvector[k]->prev=atoi(nameMap[predecessorMap[vertices[v]]].c_str());
 		 		 		  else
 		 		 			facesvector[k]->prev=-1;
 		 		 		facesvector[k]->distance= (double)distanceMap[vertices[v]];
 		 		 			  ...
 		 		 			  it!=map.template darts().end();++it)
 		 		 	  {
 		 		 		  Dart_handle d1=it;
 		 		 		  count ++;
 		 		 		  int prev1=facesvector[map.template info<2>(it)]->prev;

 		 		 		 int prev2=facesvector[map.template info<2>(map.template alpha<2>(it))]->prev;
 		 		 		  int ancestors=0;
 		 		 			  if((prev1==map.template info<2>(map.template alpha<2>(it))||
 		 		 				  prev2==map.template info<2>(it))
 		 						  ){

 		 		 			  map.template mark(it,amark2);
 		 		 			  map.template mark(map.template alpha<0>(it),amark2);
 		 		 			  map.template mark(map.template alpha<2>(it),amark2);
 		 		 			  map.template mark(map.template alpha<0>(map.template alpha<2>(it)),amark2);
 		 		 		  }
 			...
 			void remove_edges() {
 		 		...
 		 		std::map<Dart_handle,int> oldindex;
 		 		std::map<Dart_handle,int> newindex;

 		 		/*for (typename K::Dart_range::iterator it=map.template darts().begin();it!=map.template darts().end();it++) {

 		 			oldindex.insert(std::make_pair(it,map.template info<1>(it)));
 		 		...
 		 		while (true) {
 		 			...
 		 			d1=map.template alpha<1>(map.template alpha<0>(d1));
 		 			d1=map.template alpha<1>(map.template alpha<2>(d1));
 		 			if (d1==d0)
 		 			 	break;
 		 		...
 		 		int dsommetssize=dsommets.size();
 		 		/*for (int k=0;k<dsommetssize;k++) {
 		 			dsommets.push_back(dsommets[k]);
 		 		}
 		 		for (int k=0;k<dsommetssize;k++) {
 		 			dsommets.push_back(dsommets[k]);
 		 		}*/

 		 		std::cout << dsommetssize << "\n";

 		 		/*for (int k=0;k<dsommetssize;k++) {

 		 			Dart_handle d1=dsommets[k];
 		 			map.template  contract_cell<1>(map.template alpha<1>(d1),true);

 		 		}*/

 		 		map.template unmark_all(this->mark0);
 		 		...
 		 		/*for (int k=0;k<dsommetssize;k=k+1) {
 		 			...
 		 			std::cout << k << "\n";
 		 			Dart_handle d10=map.template alpha<1>(map.template alpha<0>(dsommets[k]));
 		 			Dart_handle d20=map.template alpha<0>(dsommets[(k+2)%8]);
 		 			map.template link_alpha<0>(d10,d20,true);
 		 		}*/
 		 		int k=0;
 		 		for (typename K::Dart_range::iterator it=map.template darts().begin();it!=map.template darts().end();it++) {

 		 			if (map.template is_free<2>(it)) {
 		 				std::cout << k << "\n";
 		 			}
 		 			k++;
 		 		}
 		 		...
 		 		//std::cout <<map.template count_all_cells()[0] <<" quads finished\n";
 		 		map.template display_characteristics(std::cout);
 	...
 	void CGAL_breadth_first_search(Dart_handle& source,typename K::size_type& amark2,Graph& g,
	 	...
	 	void make_A_Map() {
	 		 		...
	 		 		Dart_handle d22=map.template alpha<0>(d12);
	 		 		Dart_handle d32=map.template alpha<1>(d3);
	 		 		Dart_handle d33=map.template alpha<0>(d32);

	 		 		map.template sew<2>(d2,d0);
	 		 		//map.template link_alpha<2>(d3,d4,false);
	 		 		map.template sew<2>(d12,d32);

	 		 		//map.template link_alpha<2>(d22,d33,false);
