/* Generalized_map_algorithmstest.h
 *
 *  Created on: 10 avr. 2017
 *
 */

#ifndef SRC_GENERALIZED_MAP_ALGORITHMStest2_H_
#define SRC_GENERALIZED_MAP_ALGORITHMStest2_H_
...
template<class K=LCC_3>
class Generalized_map_algorithmstest2
{
public :
	K map;
	...
	Generalized_map_algorithmstest2(K& g) {
	...
	Generalized_map_algorithmstest2() {
	...
	void compute_shortest_cycle_non_contractible(int geometric,std::map<int,int> boundary0,
													std::map<int,std::map<int,double> > W) {//std::map<int,std::map<int,double> >& W
			...
			Graph g;
			std::vector<Vertex> vertices=create_CGAL_Graph(g,geometric,W);
		...
		CGAL_Dijkstra(dh,amark2,g,vertices);
										...
										std::cout << k << "K\n";
										double* pmin=compute_shortest_cycle_with_dual_graph3(source,valids,amark2);
						...
						for (typename K::template One_dart_per_cell_range<0>::
							...
							V.clear();
								...
								for (std::vector<int>::iterator itn=neigh.begin();
									...
									int index2=*itn;
									Dart_handle d2=verticesvector[index2]->dart();//map.template info<0>(map.template alpha<0>(it));
									//if (index!=map.template info<0>(source)){
										//if (index2!=
										//	map.template info<0>(source)){
										if ((valids.find(map.template info<2>(d2))->second==1
												)&&
												valids.find(map.template info<2>(it))->second==1&&
											...
											if (p1size+p2size+1*0<pmin){//&&p1.size()+p2.size()+1>4){//&&p1size+p2size!=2&&p1size+p2size>4)
												pmin=p1size+p2size+1;
...
#endif /* SRC_GENERALIZED_MAP_ALGORITHMStest_H_ */
