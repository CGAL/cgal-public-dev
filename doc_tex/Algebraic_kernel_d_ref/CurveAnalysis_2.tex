...
\ccMethod{std::pair< size_type, size_type > 
  ...
  line as vertical asymptote and that approach $y=-\infty$. We distinguish
  between approaches from left (\ccc{pair.first}) and approaches from right
  (\ccc{pair.second}).
}
...
\ccPrecond{Each given polynomial must be square free.}
  ...
  provided by a model, but demanded by external source code, a compile error is
  reported.
\end{ccAdvanced}
...
\ccMethod{CGAL::Object asymptotic_value_of_arc(CGAL::Arr_parameter_space loc, 
  ...
  \ccc{Algebraic_real_1}, in case the x-asymptote of the arc is finite,
