...
{Check for validity of collapsing \ccc{e=(c,i,j)} onto the vertex of index \ccc{j} which geometrical position is changed to \ccc{p} if the second argument is given. This method returns false for infinite edges and arguments \ccc{p} which lie outside the cells incident to \ccc{e}. 
