...
{Check for validity of collapsing \ccc{e=(c,i,j)} onto the vertex of index \ccc{j} which geometrical position is changed to \ccc{p} if the second argument is given. These methods return false for infinite edges and edges collapsing which lead to inverted, flat or non-simplices. Otherwise they return true.
\ccPrecond{\ccVar.\ccc{dimension()} $\geq 1$.}
}
