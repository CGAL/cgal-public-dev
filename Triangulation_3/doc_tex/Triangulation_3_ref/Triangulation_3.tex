...
The edge collapse operation over a finite edge \ccc{e=(c,i,j)} preserves the vertex of index \ccc{j}. The geometrical position of this vertex is changed only if the method has an argument \ccc{Point p}. The vertex of index \ccc{i} and all the simplices of maximal dimension incident to \ccc{e} are removed. By removing such simplex its two neighbouring simplices which are not incident to \ccc{e} are made adjacent to each other. There are validity tests as collapsing some edges could produce non-simplices or inverted simplices.

\ccMethod{bool collapse(Edge e);}{}
\ccGlue
\ccMethod{bool collapse(Edge e, Point p);}
{Collapses \ccc{e=(c,i,j)} onto the vertex of index \ccc{j}. Before collapsing, this method invokes \ccc{is_collapsible(e)} (which is quite expensive) and returns \ccc{false} or \ccc{true} according to this test.
\ccPrecond{\ccVar.\ccc{dimension()} $\geq 1$.}
}
...
{Checks for topological and geometrical validity after collapsing \ccc{e}.
\ccPrecond{\ccVar.\ccc{dimension()} $\geq 1$.}
}
