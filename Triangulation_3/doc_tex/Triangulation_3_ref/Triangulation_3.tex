...
The edge collapse operation over a finite edge \ccc{e=(c,i,j)} preserves the vertex of index \ccc{j}. The vertex of index \ccc{i} and all the simplices of maximal dimension incident to \ccc{e} are removed. By removing such simplex its two neighbouring simplices which are not incident to \ccc{e} are made adjacent to each other. There are validity tests as collapsing some edges could produce non-simplices or inverted simplices.

\ccMethod{bool collapse(Edge e);}{}
\ccGlue
\ccMethod{bool collapse(Edge e, Point p);}
{Collapses \ccc{e=(c,i,j)} onto the vertex of index \ccc{j}. The geometrical position of this vertex is changed to \ccc{p} if the second argument is given. The return value is given by the collapsibility test made before eventually collapsing the edge.
\ccPrecond{\ccVar.\ccc{dimension()} $\geq 1$ and \ccc{e} is finite.}
}
...
{Should be preferred to the previous method when the edge is known to be collapsible.
...
{Checks for topological and geometrical validity after collapsing \ccc{e}. Position \ccc{p} is valid iff no degenerate simplices arrise after the collapse and \ccc{p} lies strictly inside a cell incident to \ccc{e} or on the vertex of index \eee{j}.
\ccPrecond{\ccVar.\ccc{dimension()} $\geq 1$.}
}
