...
{Collapses \ccc{e=(c,i,j)} preserving the vertex of index \ccc{j}. Removes the vertex of index \ccc{i} and all the simplices of maximal dimension incident to \ccc{e}. By removing such simplex its two neighbouring simplices which are not incident to \ccc{e} are made adjacent to each other. Before collapsing, this method invokes \ccc{is_collapsible(e)} (which is quite expensive) and returns \ccc{false} or \ccc{true} according to this test.
...
\ccMethod{bool is_collapsible(Edge e) const;}
{Checks whether the simlicial complex would remain valid (i.e. topologically valid) after collapsing \ccc{e}.
\ccPrecond{\ccVar.\ccc{dimension()} $\geq 1$.}
}
