...
The \ccc{edge collapse} operation over an edge \ccc{e=(c,i,j)} preserves the vertex of index \ccc{j}. The vertex of index \ccc{i} and all the simplices of maximal dimension incident to \ccc{e} are removed. By removing such simplex its two neighbouring simplices which are not incident to \ccc{e} are made adjacent to each other. No other changes are made. The \texttt{is\_collapsible} test guarantees that collapsing a specific edge would not invalidate the triangulation.

\ccMethod{bool collapse(Edge e);}
{Collapses \ccc{e=(c,i,j)} onto the vertex of index \ccc{j}. Before eventually collapsing, this method invokes \texttt{is\_collapsible} (which is quite expensive) and returns \ccc{false} or \ccc{true} according to this test.
...
{Checks for validity of collapsing \ccc{e=(c,i,j)} onto the vertex of index \ccc{j}. This method returns false for edges collapsing which lead to non-simplices. Otherwise it returns true.
