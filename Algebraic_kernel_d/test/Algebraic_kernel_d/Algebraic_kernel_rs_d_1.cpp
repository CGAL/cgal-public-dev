...
#include <CGAL/Algebraic_kernel_rs_d_1.h>
...
int test_fp(){
  ...
  std::cerr<<"*** TODO: floating-point polynomial test"<<std::endl;
#ifdef CGAL_RS_TEST_LOG_TIME
  std::cerr<<"*** floating-point test time: "<<
          (double)(clock()-start)/CLOCKS_PER_SEC<<" seconds"<<std::endl;
#endif
...
int main(){
        typedef CGAL::Algebraic_kernel_rs_d_1<CGAL::Polynomial<CGAL::Gmpfr> >
                                                                AK_generic;

        // test and return the result
        long result=0;
        std::cerr<<"*** testing generic RS AK_1:";
        result+=test_ak<AK_generic>();
        result+=2048*test_fp<AK_generic>();
