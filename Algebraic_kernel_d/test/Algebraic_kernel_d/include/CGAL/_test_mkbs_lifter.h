...
#include <CGAL/Algebraic_kernel_d/Curve_analysis_2_mkbs_lifter.h>
...
void test_mkbs_lifter() {
  ...
  typedef CGAL::internal::Bitstream_coefficient_kernel<Integer> BCK;
  
  typedef CGAL::internal::Mkbs_lifter<BCK> Mkbs_lifter;  
  ...
  { // Test for the classical Descartes method
    ...
    Mkbs_lifter lifter(f);
    
    assert(lifter.degree_of_gcd() == 0);
    assert(lifter.square_free_part() == f);
    int n = lifter.number_of_real_roots();
    
    assert(n==3);

    for(int i = 0; i<n;i++) {
      assert(lifter.is_certainly_simple_root(i));
      assert(! lifter.is_certainly_multiple_root(i));
    }

    for(int i =0; i<n; i++) {
      while((lifter.right_bound(i)-lifter.left_bound(i))
	    > Rational(1,10000)) 
	{
	  lifter.refine_interval(i);
	}
    }
    assert(Rational(-118,100)<lifter.left_bound(0));
    assert(Rational(-117,100)>lifter.right_bound(0));
    assert(Rational(-155,1000)<lifter.left_bound(1));
    assert(Rational(-154,1000)>lifter.right_bound(1));
    assert(Rational(116,100)<lifter.left_bound(2));
    assert(Rational(117,100)>lifter.right_bound(2));
  }

  { // Test for the square free case, created with tree from outside

    typedef typename Mkbs_lifter::Bitstream_tree Bitstream_tree;
    ...
    assert(Rational(-160,100)<lifter.left_bound(0));
    assert(Rational(-159,100)>lifter.right_bound(0));
    assert(Rational(-389,1000)<lifter.left_bound(1));
    assert(Rational(-387,1000)>lifter.right_bound(1));
    assert(Rational(-1,100)<lifter.left_bound(2));
    assert(Rational(1,100)>lifter.right_bound(2));
  }
    


  { // Test for the m-k-Lifter method
    ...
    Mkbs_lifter lifter(f, 4, 3);

    assert(lifter.degree_of_gcd() == 3);
    try {
      Poly_int1 sq_f = lifter.square_free_part();
      assert(false);
    ...
    assert(Rational(-104,100)<lifter.left_bound(0));
    assert(Rational(-103,100)>lifter.right_bound(0));
    assert(Rational(-106,1000)<lifter.left_bound(1));
    assert(Rational(-104,1000)>lifter.right_bound(1));
    assert(Rational(33,100)<lifter.left_bound(2));
    assert(Rational(34,100)>lifter.right_bound(2));
    assert(Rational(1776,100)<lifter.left_bound(3));
    assert(Rational(1777,100)>lifter.right_bound(3));
  ...
  { // Another test for the m-k-method
    ...
    assert(Rational(-101,100)<lifter.left_bound(0));
    assert(Rational(-99,100)>lifter.right_bound(0));
    assert(Rational(-1,1000)<lifter.left_bound(1));
    assert(Rational(1,1000)>lifter.right_bound(1));
    assert(Rational(199,100)<lifter.left_bound(2));
    assert(Rational(201,100)>lifter.right_bound(2));
  ...
  { // Test for the backshear method
    ...
    Mkbs_lifter lifter(g, 6, 3, event_refinement);

    try {
      int k = lifter.degree_of_gcd();
      (void)k;
      assert(false);
    } catch (CGAL::internal::Virtual_method_exception ex) {
      // Expected
    ...
    assert(! lifter.is_certainly_simple_root(0));
    assert(lifter.is_certainly_multiple_root(0));
    assert(Rational(-1528,1000)<lifter.left_bound(0));
    assert(Rational(-1527,1000)>lifter.right_bound(0));

    assert(! lifter.is_certainly_multiple_root(1));
    assert(Rational(-954,1000)<lifter.left_bound(1));
    assert(Rational(-953,1000)>lifter.right_bound(1));

    assert(! lifter.is_certainly_multiple_root(2));
    assert(Rational(-686,1000)<lifter.left_bound(2));
    assert(Rational(-685,1000)>lifter.right_bound(2));
    
    assert(! lifter.is_certainly_simple_root(3));
    assert(lifter.is_certainly_multiple_root(3));
    assert(Rational(-636,1000)<lifter.left_bound(3));
    assert(Rational(-635,1000)>lifter.right_bound(3));

    assert(! lifter.is_certainly_simple_root(4));
    assert(lifter.is_certainly_multiple_root(4));
    assert(Rational(889,1000)<lifter.left_bound(4));
    assert(Rational(891,1000)>lifter.right_bound(4));

    assert(! lifter.is_certainly_multiple_root(5));
    assert(Rational(1009,1000)<lifter.left_bound(5));
    assert(Rational(1010,1000)>lifter.right_bound(5));

  }
  { // Another test for the square free method with tree from outside

    // More complicated polynomials needed - change the bck class
    ...
    typedef CGAL::internal::Mkbs_lifter< Bitstream_coefficient_kernel > Mkbs_lifter;
    ...
    Bitstream_coefficient_kernel bck(&ak_1,alpha);

    Mkbs_lifter m_k_lifter(f,1,1,bck);

    assert(m_k_lifter.number_of_real_roots()==1);

    // The root is an "opening" x-extreme point

    // Refine the y-coordinate (just to make it harder...)
    while(m_k_lifter.right_bound(0) - m_k_lifter.left_bound(0) 
           > Rational(1,100000)) {
      m_k_lifter.refine_interval(0);
    }    

    // Get copy of the tree
    Bitstream_tree tree = m_k_lifter.tree().make_unique();
    ...
    Bitstream_coefficient_kernel new_bck(&ak_1,beta);

    Mkbs_lifter sq_free_lifter(f, tree, new_bck);

    assert(sq_free_lifter.number_of_real_roots() == 2);

    for( int i = 0 ; i<sq_free_lifter.number_of_real_roots(); i++ ) {
        // Both roots must be inside the isolating interval of the critical point
        assert(sq_free_lifter.left_bound(i) > m_k_lifter.left_bound(0));
        assert(sq_free_lifter.right_bound(i) < m_k_lifter.right_bound(0));
