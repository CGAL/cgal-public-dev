...
#ifndef CGAL_ALGEBRAIC_KERNEL_D_MK_LINE_BUILDER
#define CGAL_ALGEBRAIC_KERNEL_D_MK_LINE_BUILDER 1
...
public:
    ...
    //! Type of the coefficients of the input polynomial
    ...
    //! \c Vert_line specification for critical x-values 
 ...
 protected:
    ...
    typedef CGAL::internal::Bitstream_coefficient_kernel_at_alpha< Algebraic_kernel_with_analysis_2 > 
      Bitstream_coefficient_kernel;        
    
 public:
    
    //! type of isolator
    typedef CGAL::internal::Mkbs_lifter< Bitstream_coefficient_kernel > Status_line_isolator;
     ...
     */
    Mk_line_builder(Algebraic_kernel_with_analysis_2* kernel,
                    const Curve_analysis_2& curve,
                    const Polynomial_2& polynomial)
      throw(internal::Zero_resultant_exception<Polynomial_2>) 
      : _m_kernel(kernel), _m_curve(curve), _m_polynomial(polynomial)
    {}
    ...
    create_event_line(int id, const Algebraic_real_1& alpha,
        ...
        try {
            ...
            // TODO use smart ptr
            Status_line_isolator *lifter 
              = construct_lifter(alpha,k,root_of_resultant,mult,
...
*/
            ...
            int c = this->get_index_of_multiple_root(*lifter);
...
// we don't care about it for now!
              ...
              //event_flag=false;
                ...
                const Polynomial_2& f = _m_polynomial;
                ...
                if(c==-1 || k==0) {
                  //event_flag=false;
                    ...
                    //event_flag=
                    event_point_checker(*lifter,f,alpha,k,fx,fy);
                }
#endif
            }
	
            int root_number = lifter->number_of_real_roots();
            ...
            Status_line_1 vl(alpha, id, _m_curve, arcs_left, arcs_right, 
                             arc_container);
            vl.set_isolator(*lifter);
            ...
            if(kernel()->is_zero_at_1_object() 
               (CGAL::leading_coefficient(_m_polynomial),alpha)) {
                int n = CGAL::degree(_m_polynomial,1);
                CGAL_assertion(! kernel()->is_zero_at_1_object()
                                 (CGAL::get_coefficient(_m_polynomial,n-1),
                                  alpha));
                CGAL::Sign asym_sign 
                    = kernel()->sign_at_1_object()
                        (CGAL::get_coefficient(_m_polynomial,n-1),alpha)
                    * kernel()->sign_at_1_object()
                        (CGAL::canonicalize(CGAL::differentiate
                                            (CGAL::get_coefficient(_m_polynomial,n))),alpha);
    ...
    create_non_event_line(int id, const Algebraic_real_1& ar) {
      ...
      // TODO 2012 ar should be rational. Why using bitstream descartes by calling signature (f, bck)?
      //           if ar is rational use another lifter
      Status_line_isolator lifter(f, bck);
      
      size_t root_number = lifter.number_of_real_roots();
      
      Status_line_1 status_line(ar, id, _m_curve, root_number);
      
      CGAL_assertion(!status_line.is_event());

      status_line.set_isolator(lifter);
...
*/
     ...
     * is thrown.
     */
    Status_line_isolator* construct_lifter(const Algebraic_real_1& 
                                          alpha,
                                          int& k,
        ...
        if(root_of_resultant) {
               ...
               (CGAL::leading_coefficient(_m_polynomial),alpha)) {
                Polynomial_2 trunc_pol = 
                    CGAL::internal::poly_non_vanish_leading_term
                      (kernel(),_m_polynomial,alpha);
                CGAL_assertion(CGAL::degree(trunc_pol,1)+1 == 
                               CGAL::degree(_m_polynomial,1));
                return new Status_line_isolator(trunc_pol, bck);
            }
#endif
            ...
            if(mult==1) {
                ...
                std::pair<int,int> mk 
                    = compute_mk(alpha,
                                 _m_curve.principal_sturm_habicht_begin(),
                                 _m_curve.principal_sturm_habicht_end(),
                                 1);
...
#if CGAL_ACK_DEBUG_FLAG
            CGAL_ACK_DEBUG_PRINT << "Lifter..." << std::flush;
#endif
      
            

            return new Status_line_isolator(_m_polynomial, m, k, bck);
        } else {
            return new Status_line_isolator(_m_polynomial, bck);
        }
     ...
     * corresponds to the multiple root in the <tt>lifter</tt>-instance.
     ...
     */
    bool event_point_checker(Status_line_isolator& lifter,
			     const Polynomial_2& polynomial,
...
*/
      ...
      Polynomial_1 p = -_m_curve.coprincipal_sturm_habicht_of_primitive(k);
      Polynomial_1 q 
            = Coefficient(k)*_m_curve.principal_sturm_habicht_of_primitive(k);
/*
#if CGAL_ACK_DEBUG_FLAG
        CGAL_ACK_DEBUG_PRINT << k << " " << CGAL::to_double(alpha) 
                             << std::endl);
        CGAL_ACK_DEBUG_PRINT << p << " " << q << std::endl 
                             << _m_polynomial << std::endl;
        Bound a_d = alpha.low();
...
*/
        ...
        while(true) {
            ...
            BFI isol_iv 
                = CGAL::hull(CGAL::convert_to_bfi(lifter.left_bound(c)),
                             CGAL::convert_to_bfi(lifter.right_bound(c)));
        ...
        if(! CGAL::internal::zero_test_bivariate
	       <Algebraic_kernel_with_analysis_2>
	         (kernel(),alpha,_m_polynomial,p,q)) {
#if CGAL_ACK_DEBUG_FLAG
            CGAL_ACK_DEBUG_PRINT << "Detected non-generic position for alpha=" 
                                 << CGAL::to_double(alpha) << std::endl;
#endif
    ...
    int get_index_of_multiple_root(const Status_line_isolator& lifter) const {
        int n = lifter.number_of_real_roots();
        for(int i=0;i<n;i++) {
            if(! lifter.is_certainly_simple_root(i)) {
                return i;
            }
        }
        return -1;
    ...
    // friends
    friend class CGAL::Shear_transformation< Algebraic_kernel_with_analysis_2 >;
...
#endif // CGAL_ALGEBRAIC_KERNEL_D_MK_LINE_BUILDER
// EOF
