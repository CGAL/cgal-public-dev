...
public:
    ...
    mutable boost::optional< std::pair< double, double > > _m_approx_xy;
...
public:
    ...
    Algebraic_real_1 y() const {
        ...
        if (!this->ptr()->_m_y) {
            ...
            Roots y_roots;
            real_roots(y_pol, false, std::back_inserter(y_roots));
...
public:
    ...
    std::pair<double, double>& to_double() const {
        ...
        if(this->ptr()->_m_approx_xy)
            return *(this->ptr()->_m_approx_xy);
	...
	// rely on double conversion of the x-type
        ...
        this->ptr()->_m_approx_xy = std::make_pair(double_x, double_y);
        return *(this->ptr()->_m_approx_xy);
